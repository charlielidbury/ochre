...
`&mut (s : τ ↝ S)` means "across this boundary, a value of type S is owed"; `s` names the payload at entry, for use in S, which is checked at exit (`dllbc-arrows.md` §5.1). The mechanism:

- **Mint.** `seedTelescopeV` (`Machine.lean:3134`) walks a declaration's telescope. For a `borrowT` parameter it mints σ (the entry snapshot) and ℓ (the loan), binds the slot to `borrowₘ ℓ σ`, records `(x.id, σ)` in `St.entrySyms` — this is `old *v` — and computes `owed := S[s := σ]`, pushing an `Obligation ⟨arg, loan, owed, trivialOwed⟩` into `St.obligations`. The snapshot is *pinned at mint*: `Obligation.owed` holds the opened term, and it lives in `St` rather than being returned so that a §10 `Refl` refinement reaches it.
...
structure Debt where
  ...
  owed    : Term               -- S[s := σ]: the TYPE the payload must have at ℓ's end
...
```
fn Nth [i] (v : &mut (s : List Nat ~> List Nat = SetNth i (*res) s),
            i : Nat, p : Le (S i) (Len *v)) -> &mut Nat
```

Read: "when ℓ_v ends, its payload has type `List Nat`, and its value is `SetNth i (*res) s` — the entry list with position `i` replaced by whatever came back through the borrow I returned."
...
And the prohibition: **a caller may not strengthen a debt at a call.** Registering `~> S = e` on a loan being handed to a parameter typed `~> S′` with no pin would be asserting what nobody checks — the callee promised S′ and the group will release at S′. This is the M28 τ unsoundness wearing the caller's clothes, and the check that stops it is: at a call, the parameter's debt must **entail** every debt already on the loan. A caller may register a *weaker* debt at its own mint (a deliberate assertion checkpoint); it may not register a stronger one and have it believed.
---
...
```
Ω ⊢ v : &mut (s : τ ~> S)   ⟺   v = borrowₘ ℓ p,  and
                                 (a)  ⊢ p : τ          -- the PURE conjunct: Ω-independent given p
                                 (b)  ℓ carries a debt entailing S[s := its entry σ]   -- the STORE conjunct
```
...
| .borrowM ℓ p, .borrowT sn τ S =>
    if !(← hasType fuel p τ) then pure false
      ...
      | ds => pure (ds.any (fun d => Pure.convert fuel d.owed (open S at d.entry)))
```
...
| site | today | change |
| --- | --- | --- |
| `Uni.lean:535-552` | `&mut (s : τ ~> S)` → `.borrowT`; `&mut` in ⇒ position refused | adds the pin slot to the surface (D1's spelling) |
...
- **(a) Type only** (today's `~> S`, extended in scope). Smallest change; `hasType` is the only judgment needed. **Cannot express the get_mut law** — §2.4's two arguments.
...
- **(c) Both slots.** `&mut (s : τ ~> S = e)`, `e` optional, `e : S` checked at registration. `&mut (s : τ ~> S)` and `&mut τ` keep their exact current meaning.

**Recommend (c).** A type claim and a value claim are different claims, both already occur in the corpus, and (c) is the only option that is a strict superset of today. Surface spelling is bikesheddable — `~> S = e`, `~> S := e`, `~> S ↦ e` — and the choice should be made when `Uni.lean` is touched, not now.

### D2. How does the pinned release reach the caller?

- **(a) Definitional refinement.** Mint σ′ at `owed` as today, then `refineSym σ′ e`. Uses existing machinery; `pin = none` is byte-for-byte today's rule.
...
```
fn NthPin [i] (v : &mut (s : List Nat ~> List Nat = SetNth i (*res) s),
               i : Nat, p : Le (S i) (Len *v)) -> &mut Nat
```
