...
8. ▢ **Recursion and termination** — `[k]`, the snapshot-subterm guard, measures as the future escape hatch. *The guard is now operational (M23):* a self-call is admitted at the declared return type only when the actual at the declared decreasing position `[k]` is a strict structural subterm of that parameter's *current snapshot* — trivial to test precisely because the checker is a symbolic interpreter (inside `match n { S(m) ⇒ … }` the snapshot is already ⇜-refined to `S σₘ` and the actual is `σₘ`). Before the guard, an unguarded recursive-and-back-less declaration proved any postcondition from itself (`fn bad () -> Id Nat Z (S Z) { bad() }` was accepted); the hole never fired in the conformance era only because every recursive declaration carried a `back` whose convert-check did the real work. Three decisions with their reasons: `[k]` is *declared, not inferred* — "some argument decreases" is unsound (two branches can each shrink a different coordinate while growing the other, forever), and path-local exploration cannot commit to one inferred index consistently across paths; a *borrow* parameter may decrease through its payload snapshot — sound because snapshots are entry-knowledge that mutation never rewrites (§3.2), hence a fixed well-founded order, and it is the guard in its most literal form (a list cursor passes no counter at all); *mutual* recursion is rejected outright by call-graph reachability — per-declaration guards would admit `f → g → f` with nothing decreasing anywhere, the same hole through two doors. With a sufficiency hypothesis in the telescope (`hfuel : Le cnt fuel`) making the out-of-fuel path ⊥-dischargeable, guarded fuel recursion is *total* correctness; measures remain the future ergonomics that derive the fuel. Two of those three decisions have since been overtaken, by one move (M26): once `fn` elaborates to a recursor, `[k]` demotes to a **scrutinee-selection hint** and the guard *evaporates* rather than being removed — a recursive occurrence is the recursor's `ih`, a bound variable of the arm, and a binder cannot be a self-call, so there is no side condition left to check. `bad()` stays unwritable, and by a mechanism worth naming because it is not a recursion rule at all: a binding is not in scope in its own right-hand side, so the inner name resolves to nothing and falls through as the forward reference it is. What the demotion changes about the third decision is its *cost*. "A borrow parameter may decrease through its payload snapshot" is sound, and it was the natural thing to declare — a list cursor passes no counter — so nothing ever pressed an author to name a decreasing *index* that was also sitting in the telescope. Under a hint, naming the index is free and strictly better: it is what the elaboration can serve, and it costs one field of the declaration, no body change and no signature change. The corpus's `nth`/`nth2` are declared the old way, and that alone is why their families had no recursor form. A *genuine* payload decrease — a cursor with no other decreasing argument, `zero_all` — is accordingly rarer than the declarations suggest, and carries fuel until §9's borrow-mode eliminator exists.
9. ▢ **The comptime fragment as a type theory** — conversion (Ω-free over substituted terms), universes, K, elaboration of match to eliminators. Plus the kernel decisions accumulated by the mechanization, filed for this section: `nfV`-reduces-`letIn` (would make `let` mode-free, shrinking the bimodal inventory to `&mut` alone); comptime Σ-elimination (`sigmaRec` or comptime match — pure conjunctions currently build but never project; the quicksort postcondition's consolidated pain diary traces *four* structural detours to this one gap — the bounded-Π predicate encoding itself, the partition invariant split into three directional lemmas, the perm-survival keystone's separate bridges, and the out-of-range locality lemma split into `_lt`/`_gt` halves — with the add-order tax between index-first predicates and offset-first models a second-order cost of the same encoding); λ/neutral *family* typing (predicate families check only in applied form); and `Le` as a primitive former (recognition prerequisite for both the proof-copy extension and audit-rewrite-along-cited-bridges). One surface decision is filed with two independent milestones asking: `old` for *consumed* parameters — §5.4 lets a return type name a consumed argument's entry value and denies the body the same, so a body must *stage* the value in a runtime `let` while the argument is still live (M22: a proof consumed by a mutation; M23: a data argument consumed by the very call the proof is about — `append (old *tl) w` needed after `w` has moved). The mechanism exists (entry snapshots keyed by variable); the decision is only to extend it to the body. Pre-authorized to land the first time staging fails to reach a case. One machine *principle* joins the decisions, distilled from its fourth independent consumer (the recursion guard's tracked snapshot, after obligations, instantiated call types, and retTyVal/selfBack): any checker component that must observe a value *across* a refinement has to live in the σ-bearing state that `refineSym` sweeps — re-reading from Ω is precisely the staleness bug the M10 invariant exists to prevent, and after an owned match has consumed the slot it is not even possible.
...
*The mechanization diverged from this grammar and has been brought back (M27).* Worth recording because the correction runs the unusual direction. Two of the productions above were written as they are here from the start — application is juxtaposition `t t′`, and λ carries its domain, `λ (x : τ). t` — and the implementation drifted from both while building the runtime layer: M26-C introduced a Curry-style runtime λ whose binders carry names and no types (the ascription was the contract, so the types looked redundant) and an n-ary call form `f(a, b)` inherited from the declaration era's telescopes. The ratified function model restores the written grammar. Annotated binders make checking a seal ONE conversion — synthesize the λ's Π, compare it against the ascription — instead of a bidirectional descent, and juxtaposition says what the type already said, since a multi-argument function's type is a nested Π and the comma form reads as passing a tuple. The n-ary *call event* stays atomic regardless; what changes is how it is written, not how many things happen.
...
The shelved unification has since happened, and the grammar above is the historical one. M26 collapsed `fn` into a `let` of a sealed λ and M27 finished the job: a program is an arbitrary term, a declaration is a `let`, and the `D` production is gone along with the record it built. Three of this section's clauses were overtaken rather than refined, and it is worth being precise about which, because each was load-bearing somewhere else in this document. The **decreasing-argument index** survives only as the elaboration's hint for which parameter becomes a recursor's scrutinee — the guard it named EVAPORATED, since a recursive occurrence is the `ih` binder and a binder cannot be a self-call. The **backward specification** a function could declare is deleted outright (§6.2). And the ratified function model settles the residue the unification left open: functions are ⇝-resolved **names**, never runtime values, so there is no function-typed runtime slot at all; λ binders are annotated (Church-style), which makes checking a seal ONE conversion between the synthesized Π and the ascription; and the call event stays atomic, so saturation stays a rule rather than becoming a consequence of currying.
...
One refinement, adopted when the mechanization met index arithmetic (every reuse of a loop index was a use-after-move): a ⇒-read of an **index-kind** value — a concrete `Nat`/`Bool`/`Unit` tree, any pure-former value (a proof, a type, a λ), or a σ whose type is one of these — is a *copy*: the owner keeps it, and the trace above reads `x ↦ 3, y ↦ 3`. On index-kind values the ownership machinery is doubly vacuous: they are marker-free *and* erasure-bound (§1.3), so consuming them polices nothing. Data proper — `Cons`-trees, user constructors, anything with an aggregate inside it — moves even when marker-free: value semantics would make copying it *sound*, but silently duplicating aggregates is exactly the cost-opacity Rust's move discipline exists to prevent, and this calculus keeps Rust's line (use-after-move of data remains an error; §7.2's generated `T.copy` remains the honest price of duplicating it). Copy-or-move is decided by value shape — the σ-context's type for symbolic values — not by a declared trait.

A **Σ pack is index-kind exactly when every component is**, counting a component as free when it is ERASED: the pack's type marks that position comptime (a capital Σ binder marks the domain, `Σ0` marks the tail) *and* the position is erasure-bound in the sense above — index-kind, or a type whose constructor set the machine does not know, which is what a stuck proposition spine like `Le n MAX` is. An erased position costs nothing at runtime, so duplicating it duplicates nothing; the second conjunct is what keeps the two machines agreeing, since a CONCRETE pack carries no mode on its components and is classified by shape alone. This is the one place the paragraph above draws its line differently from Rust's, and it draws it the same way Rust does: `#[derive(Copy)]` on a struct of scalars, read off the type rather than declared. So `Σ0 (n : Nat). Le n MAX` — a bounded machine integer, a runtime `Nat` beside an erased bound — copies, and `Σ (a : Nat). List Nat` does not. The dependent tail is decided with the Σ's binder opaque, so a component whose kind depends on what the binder turned out to be (`Σ (n : Nat). VecF Nat n`) takes the conservative answer and moves. And a pack holding a **borrow** — `Σ (c : Nat). &mut (Array c T)`, the runtime-length slice of ¶4 — moves, necessarily: a borrow is neither erased nor index-kind, and copying the pack would produce two owners of one payload.
...
Out-of-bounds is unrepresentable: `Fin (len *b)` replaces the error monad. The division of labor is exact: **`*b` means the payload now** (⇝-projected wherever the type is consulted); **`s` means the payload at entry** (bound once, for the one position — the obligation — that must outlive its moment). And since `Σ (l : &mut List T). Fin (len *l)` is likewise well-formed, the telescope pattern "a borrow, plus data typed by its payload" is an ordinary first-class type. This is a payoff of the value semantics: a borrow *carries* its payload, so `*l` in a type is a pure projection — `*(borrowₘ ℓ v) ⇝ v` — never a store lookup, never stale. One proviso: the projection is defined only on a proper payload — a *suspended* borrow, its payload holding loan markers mid-§3.3, has no meaningful snapshot, and comptime deref is stuck on it until the reborrows end. "Stuck" must be *made* true, and the mechanization earned the general statement one silent bug at a time (M23): **every demand collapses first** — any rule that *reads* a place ends the suspensions parked there before it looks, and the sites are instances of that one rule, not separate rules: the runtime var-move, `&mut`, the match scrutinee, the comptime deref through the pure lift (where a body proves something about a call it just made and `*tl` is the only name for the released value), the take (`let lo = *v` after a call), and the bare comptime `.var` naming a local whose loan a call still holds. The last three were each found as a live defect — the projection or the take silently returning the parked `loanₘ` itself, a state artifact riding into a proof term (the same silent-marker class as §3.2's `⊥`-into-a-pure-value) — which is the evidence for stating the rule once: every *unlisted* site was a bug waiting for the first body that read through it. A seventh site joined once a program became a term (M26), and it is the one that is not a read at all: the **end of the program**. A program that lends a local to a call and never looks at it again leaves the loan parked — the rule releases a group when something demands it, and nothing ever does — while the concrete machine releases a frame's loans on the way out of it, so the two end in visibly different environments on an ordinary program that neither is wrong about. Ending the scope is what the boundary audit does for an argument borrow and what frame exit does for a callee, arriving where there is no caller left to hand anything back to; and it can legitimately *fail*, so it belongs to checking rather than to observation. What is worth recording is the instrument: every earlier site was found by a body reading through it, and this one could not be — there is no read — so it was found by the differential instead, the two machines disagreeing about an Ω rather than about a value. Same rule, same evidence, the other instrument. One scoping stands: the ⇝ demand fires on the pure *lift*, where a body reads live state; ⇝ on types and specs remains the read-only projection documented here — the lazy-ending rule gained premise-sites, the comptime fragment did not gain a side effect. And one convention the mechanization pinned down: a later parameter's type mentions an earlier argument by its *runtime variable*, resolved to the snapshot by ⇝ when the type is consulted — the telescope performs no cross-parameter substitution at seed time. A pure binder in that position would sit unsubstituted, whnf to a rigid neutral, and read as rigid at, say, a refl-match that should have been flex. At a call site the same convention holds with the actuals in place of the parameters — a borrow argument's name denotes the *borrow*, so `*b` peels to the payload snapshot; a bare `b` in a type position, the borrow itself as an index, is outside the unrestricted fragment and unspecified (indices must be unrestricted, §7.1).

### 5.3 Calls: consume and promise

A call is checked against the signature alone — recursion forces this (the checker cannot unroll), and all calls get it uniformly. The rule: ⇒-consume the arguments; for each argument borrow, annotate its loan with the owed type from the signature, instantiated at the actual arguments; mint a fresh symbolic value for the result. Instantiation is caller-side telescope discipline, symmetric to the callee's seeding: each checked actual is substituted into the remaining parameter types before they are consulted, so `Fin (len *b)` at a call site means the `b` just passed. (This section states the *checking* rule; the mechanization's concrete counterpart — the operational semantics any simulation proof relates it to — inlines the callee's body into a fresh frame over the shared environment, so borrows propagate. One invariant of that semantics, learned by counterexample: any executing-mode operation that hands out a value peeled from a demand-ended suspension must drive it to *owned-marker-free normal form* first — the collapse must be transitive over owned field-loans, or a later comptime deref through the reborrow reads a stale partial view while the owner, whose move-cascade always collapsed fully, stays correct. Checking mode gets this for free from atomic group-ends; executing formers must each preserve it. Recorded here until the metatheory owns it.) Left-to-right consumption has a practical corollary the mechanization hit: a later argument expression may not mention a borrow an earlier argument of the same call consumed — in particular, a bounds proof about `*v` must be `let`-bound *before* the call that consumes `v`. Derive the bounds, then make the call.
...
The trichotomy has since stopped being a way of *indexing* precision and become syntax (M26). Position three — the sealed spec — is one AST node, `.seal t u`: checking it **is** this section's audit, performed at the node rather than at a declaration, after which a fresh symbolic value at the ascribed type is the whole downstream view, and what a caller keeps is exactly what the programmer wrote in the ascription. What that buys beyond tidiness is that the position stops being a property of a *declaration form*: a seal is legal anywhere the runtime arrow evaluates, so a program is an arbitrary term whose module structure is a chain of `let`s, and a callee is a binding lexically above its caller rather than an entry in a table — which also makes forward references unwritable rather than rejected, since a let-chain cannot reference downward. The mechanization then turned this section's own sentence into a fact about the machine: factoring the checking-mode call rule out yields ONE function by which a sealed function, a recursor's induction hypothesis, and a declaration-table entry are all called, because a symbolic value whose signature is a borrow-moded Π *is* a callee whose telescope and return type are known and whose body is not — which is exactly what a table entry always was. Nothing new sits underneath it. And the opacity is load-bearing for *expressiveness* rather than for hygiene, which is this calculus's novel claim in the space: the array era's carve machinery — where one body cannot both select a pivot (forcing a match that rigidifies the length) and carve at the returned index (needing the length flex), so the call boundary's re-mint is the only way to reset the rigidity regime — transferred to a sealed binding without one adjustment, and those programs are unwritable if that is false.
...
The propositional main line has its first full instance: an in-place quicksort whose return type *is* the postcondition — `Σ (SortedR cnt lo (*v)). Π n. Id (count n (*v)) (count n (old *v))`, sortedness and permutation over the exit snapshot — checked end to end, with the two nearest lies (the count shifted by one, the sorted range widened by one) rejected at the audit. The instance sharpened the architecture's one structural fact. An *inline* pointer-write's exit is opaque — the collapse mints issued payloads as fresh existentials — so a direct-proving body delegates its mutation to a back-carrying callee and cites pure lemmas about that callee's model: direct proving eliminates conversion-*verification*, not the model functions, which survive as the only provable exit shapes. A three-feature arc was filed forward here — pin the issued payload, prove the bridging equation, let the audit rewrite along cited bridges — and M23's leaf stage *collapsed most of it*, by refining the diagnosis it rested on: the opacity is a property of borrows **issued by a call**, not of inline mutation. `buildResult` mints a call-issued borrow's payload as a fresh σ because signature-only checking cannot say what it holds; but a leaf doing its own cursor work through the body's *own* match-field borrows writes into a suspension the audit itself collapses, so its exit is a constructor tree over known snapshots — fully provable, nothing minted. The route to a provable inline leaf is thus a *program-level* choice — walk the structure yourself instead of calling a cursor — and the bridging equation is cited as an ordinary lemma in the body (which also keeps the ensures on the observation side of the vocabulary line: `swapL` is admissible there *because* the bridge defines it from `set`/`nth`). Issued-payload pinning survives at reduced scope, only for bodies that insist on calling a cursor rather than being one; the audit-rewrite close for reformulated backs stands on its own merits. And the successor question is already posed: whether the model functions themselves can go — the callee's *postcondition* as the caller's only knowledge of the exit, self-framing ensures replacing declared backs outright, the recursion checked against its own declared postcondition under a decreasing measure. That removal is where the imperative/dependent interaction stops being deferrable.

The removal is done (M23). `fn quicksort [fuel] (fuel, v : &mut List Nat, hfuel : Le (len *v) fuel) -> Σ (Sorted (*v)). Π n. Id (count n (*v)) (count n (old *v))` checks with *zero* declared backs in its call tree — every callee described only by its ensures, every invariant proven inductively in the body from the recursive call's own postcondition, the split-off/append-back ownership discipline standing in for slices, and the one non-glue step — bound survival across the recursive sorts — routed through the multiset exactly as M22's keystone was (`Ub` restated by counts is manifestly permutation-invariant; one `id_trans` for the invariance, the work in the two crossings). Two facts about the result deserve equal billing with it. The check costs **21 milliseconds** where M22's model-conformance quicksort cost 21.8 seconds tuned and 38 minutes untuned — three orders against the tuned number with no checker performance work at all. Stated honestly, the ~1000× is a property of the *pair* — how the problem is posed, program and statement together: the positional `SortedR cnt lo`/bounded-Π stratum built enormous normal forms where the whole-list structural `Sorted` (a Σ the caller projects — the capability whose absence forced the positional detours) keeps them small, but the two quicksorts also differ in partition program, and the differences are *coupled* (the positional spec exists because a swap-based scan mutates a range; the whole-list spec is what makes a two-list partition natural) — no experiment here isolates encoding from program. What stands unqualified: the natural posing is also the cheap one; naturalness and cost were never in tension. And the staging tax got its precise statement: a *Nat-valued* fact is named once and survives (`len rest`); a *family* over an index (`Π n. count n rest`) cannot be, because the lemma wants the consumed *list* itself — which is exactly what a snapshot-naming binder (`old x` for any consumed binder; six independent filings across two milestones) is for, and exactly what four staged builder-λ's doing no mathematical work currently simulate.

**And the mechanism itself is gone (M27).** This section's declared backward specs — the callee check that converts a suspension tree with holes against a stated spec, the caller's computed group-end release, the four facts about composition, the value-returning dual — are deleted from the calculus and from the checker. Everything above stands as the record of what they were and why they were sound; none of it describes anything the language now has. Three things about the removal are worth stating here rather than in a log, because each corrects something this section says.

The removal was **not a port**, and the difference was measured rather than argued. Giving a cursor the ensures its callers need while leaving its body a cursor is *rejected*: the exit is a fresh existential minted by the sub-call's own group, which is this section's opacity sentence arriving as an error message. So a body that delegates its cursor work to a call cannot state what its exit is, whatever return type it carries — and the route to a provable exit was always the program-level choice this section already names, walking the structure yourself. M23 had made that choice; its `swap_at` proves, from a chain of ensures-carrying writes, precisely the statement a declared `back = swapL i j *v` used to assert. The stratum was superseded, not portable, and its replacement was already in the corpus and already green.

What the deletion costs is exactly one thing, and it is the thing this section calls the payoff: **a caller no longer recovers the exact written value in checking mode.** That is by design rather than by regression — what a caller keeps across a boundary is what the callee ascribed, and an opaque call ascribes nothing about the payload. The *behaviour* is untouched: the executing machine still writes what it always wrote, and the differential still says so. Opacity was never a fact about what happens; it is a fact about what was promised.

And the trichotomy's third position, which an earlier fold recorded as becoming syntax, has finished the journey: the sealed spec is not a *position on a dial* at all but the only form there is. A caller sees a signature because that is what a seal yields; there is no dial left, and the "precision spectrum" this section is named for describes a choice the calculus no longer offers.
