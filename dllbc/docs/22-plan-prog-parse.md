...
* else `s` a kernel constant (`Dllbc.constNames`) → `.const s` — reached by an `.ident` built by hand, since the parser classifies constants itself; and a `.const s` the splice site binds as a local is rewritten to that binder (§6.1);
...
* **A lowercase kernel constant the splice site shadows is the splice site's binder** (found by the §H conversion, not foreseen; settled twice). `k` is the `Id` eliminator and the name every `S(k) =>` arm binds; inline the local wins because `resolveName` looks it up first, and a fragment has no local to find. The first cut left a lowercase `constSet` name as `.ident` in parse mode for `bindIdent` to decide. That broke on rebase, when `prog_parse` became the parse-stage brace (below): hundreds of its sites hand a term like `arrCat 2 1 …` straight to the normalizer (`pv`, KernelFloor) with no boundary in between, where an `.ident` resolves to nothing. So constants are classified at parse as they always were, and `bindFree`'s `.const` row lets the splice site's binders claim one — pure binder → `.pvar`, runtime local → `.var`, a `fn` slot or nothing → the constant stays. Same answer as inline, reached from the other side. The table moved to the kernel (`Dllbc.constNames`) because `rejectFree`'s empty-context bind resolves a leftover `.ident` against it. Only the lowercase constants can ever match — the capitalized ones and all constructors are reserved as binders — and the one lowercase constructor, `unit`, is classified at parse with no shadowing row: nothing in the corpus binds it.
* **`ident_or_free%` asks known-or-not, not `resolveId?`**: the latter throws on an ambiguous global (`Append` the Lean class against `StdLemmas.Append`), where the bare identifier it replaces went through `elabTerm`'s overload resolution against the expected type. Found by Direct.lean on rebase.
