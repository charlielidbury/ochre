...
6. **Consumer migration — measured, not yet executed (2026-08-22).** Three probes settled the shape:
   ...
   * Remaining order: placeholder resolution for bare imported identifiers in deferred blocks (if a consumer needs one); inline the 69 raw proof bodies into the chain (paste, not splice); move the formers in as capital `let`s; fan out one agent per consumer file; delete `StdLemmas.lean` and shrink `Std.lean`.
