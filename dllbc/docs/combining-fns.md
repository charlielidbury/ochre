...
4. **One guard mode has no easy recursor form**: decreasing through a borrow's *payload* (the `zero_all` shape — no counter at all). Fuel-threading is the always-available fallback; the principled successor is §9.
