...
1. **`back` under a seal — and it is what `Decl`'s deletion waits on.** The question, stated as one: **what replaces `back` for these eight declarations, given that M23 retired the mechanism?** §6.2's declared backward specs are a `Decl` mechanism with no seal counterpart, because the ensures IS the contract (§5 point 4), and M23's own corpus declares none, deliberately. The scope is measured rather than guessed: of 224 corpus declarations, 73 have no program form, and **exactly eight are blocked by `back` alone**. The blockers COMPOSE through the cohort closure — 73 → 68 with two hints corrected, → 65 with `back` stripped, → **20 with both** — and one leaf with *both* problems (S17's `nth`) is why: fixing either leaves it declining and everything above it declines with it. **"Fix the biggest class first" is exactly the wrong strategy against a closure**; the marginal return on removing one blocker is near zero until the last one on a path comes off. This is a decision about what the repo keeps as its M17-era comparison baseline, not a migration task.
2. **The borrow-eliminator's design** (§9): cursor-indexed motive shape, the consequence/weakening rule at the audit, derive-vs-compute motives. Filed with its survey; unblocked now that §7's machinery exists. **One correction to its billing**: the eliminator is worth building for `zero_all`'s shape and for naturalness, but it is NOT what stands between here and deleting `Decl` — the measured map says that is eight `back`s and a handful of hints. The earlier reading of the same evidence said the opposite, and the difference was `fnElab` reporting the FIRST reason it declines.
3. **`recDeep` — §7's genuine expressiveness limit.** A recursion two constructors down (`match n { Z => …, S(a) => match a { Z => …, S(b) => f(b) } }`) is legal under the guard and has no single-recursor form: an arm gets `ih` at the *immediate* predecessor and nothing below it. Either the elaboration learns to nest recursors, or such a function threads fuel. Filed, not urgent — one corpus function, and it is a guard test.
4. **Adopting `[i]` over `[v]` in `S14Bounds`/`S17Spec`** — a user call, with the recommendation to adopt: the hint is a claim about *why* the recursion terminates, that claim is load-bearing now that it selects the eliminator, and era-documentation lives in git history. The corrected twins are in `Tests/S26Fuel.lean`; the shared corpus files were deliberately not touched.
5. **Stored and returned Σ-of-borrows**: exercised in one direction only; needs probing with negative controls before the calling convention leans on it.
6. **δ-constants granularity**, if and when adopted: per-use unfold (Lean's `unfold`) for proofs about pure functions.
7. **Closures and capture** — deferred wholesale to the main note's §10. §8's globals do NOT breach it: a body may name the FUNCTIONS bound above it (a function is *called*, and calling locates its callee rather than moving it), while data, borrows and sealed proofs are refused. A body that wants a proof takes it as a capital parameter, which is what §6 built.
