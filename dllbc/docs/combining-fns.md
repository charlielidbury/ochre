...
| **the correction** | *Most of the class was never a decision-8 cost.* §7 demotes `[k]` to a scrutinee-selection HINT, and the declaration-era guard was happy with `[v]` whenever the payload decreases — so nobody had a reason to name anything else. But `nth`/`nth2` also decrease on their INDEX, which is a `Nat` the macro serves directly, and correcting the hint is FREE: same body, same telescope, same return type, no fuel, no bound, no dead branch, no caller change. |

**Still open**:

1. **`back` under a seal — and it is what `Decl`'s deletion waits on.** §6.2's declared backward specs are a `Decl` mechanism with no seal counterpart, because the ensures IS the contract (§5 point 4). The scope is now measured rather than guessed: of 224 corpus declarations, 73 have no program form, and **exactly eight are blocked by `back` alone**. The blockers COMPOSE through the cohort closure — 73 → 68 with two hints corrected, → 65 with `back` stripped, → **20 with both** — so attacking the largest class first barely moves the number, and the residue after both is 20. This is a decision about what the repo keeps as its M17-era comparison baseline, not a migration task.
...
4. **Stored and returned Σ-of-borrows**: exercised in one direction only; needs probing with negative controls before the calling convention leans on it.
5. **δ-constants granularity**, if and when adopted: per-use unfold (Lean's `unfold`) for proofs about pure functions.
6. **Closures and capture** — deferred wholesale to the main note's §10. §8's globals do NOT breach it: a body may name the FUNCTIONS bound above it (a function is *called*, and calling locates its callee rather than moving it), while data, borrows and sealed proofs are refused. A body that wants a proof takes it as a capital parameter, which is what §6 built.
