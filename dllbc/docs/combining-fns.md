...
**Resolved in this design round** (2026-08-04, recorded so the next reader knows what is settled):

1. The opacity mechanism is the **`.seal` node** — legal under ⇒ anywhere, absent from the pure grammar; checking it is the audit. (Not: a reducibility attribute; not: a const table — δ remains a separate optional representation design for *transparent* names only.)
2. **`fn` is a macro** over recursor + sealed binding; motive derived from the signature; `[k]` demoted to scrutinee-selection hint; the guard deleted for recursor-expressed functions.
3. **Modes are per-binder, marked by case** (capital = comptime, extending to `let X`), kind-constrained (borrows lowercase); the fence checks capital binders into ⇝-positions only. Constructor names are reserved keywords, so capitalisation is unambiguous. Congruence is decided by judgment + type, never by the flag.
4. **Application is saturated** (Σ-package or spine — no runtime partial applications).
5. **Runtime calls do not opt into remembered-spine semantics** — door kept closed, noted as a door (the M22 history is the evidence both sides are coherent).
6. **A program is a term** (§8): declarations are `let`s, `Decl` is deleted with `fn`, scope is the call table, and checking a program is one symbolic ⇒-walk.
7. **The executing machine is built first or in lockstep, never after** (§7 cost 5) — effectful ι, λ-application under ⇒, and closed `ih` closures land with the differential running from the first commit, per the polarity doctrine.
8. **The `[v]` payload-decrease regression is accepted** (§7 cost 4): fuel-threading is the blessed interim for the `zero_all`/`split_off` shape; §9 is the successor and stays filed.

**Still open**:

1. **The borrow-eliminator's design** (§9): cursor-indexed motive shape, the consequence/weakening rule at the audit, derive-vs-compute motives. Filed with its survey; blocked on §7's machinery, not on further discussion.
2. **Stored and returned Σ-of-borrows**: exercised in one direction only; needs probing with negative controls before the calling convention leans on it.
3. **δ-constants granularity**, if and when adopted: per-use unfold (Lean's `unfold`) for proofs about pure functions.
4. **The intersection smell test**: confirm at implementation time that the audit of a no-borrow sealed λ degenerates to exactly `hasType` (§4 predicts it must).
5. **Closures and capture** — deferred wholesale to the main note's §10; nothing above depends on it.
