...
## Why the callee's exit is the assertion site (and not the loan's end)

The Rust-shaped objection — "`fn g<'a>(&'a mut self) -> &'a mut i32`'s check belongs where `'a` ends, not at `g`'s return" — has a precise answer here, and it is a fact about who knows what. The ignorance at this boundary is symmetric: the CALLER never learns which cell the returned borrow reaches (calls are checked against signatures alone, so the caller-side machine carries no position information — two `G`s with identical signatures may lend a value cell or a key cell), and the CALLEE never learns which value the caller will write. Soundness needs both quantifiers covered, and each party can only quantify over the other's unknown. So the obligation cannot be asserted, unaccompanied, at the group end: the party present there does not know the position. What CAN be done is for the callee — the party that knows the position — to discharge the obligation for EVERY value at its own exit. That is the opaque fill below: the lent place abstracted to a fresh σ, the checker's native ∀.

This also restores Rust's modularity guarantee rather than merely imitating its check placement. In Rust, writing freely through a returned `&'a mut i32` is justified because the borrow checker verified the callee under the assumption that the caller writes anything. Here likewise: `-> &mut Nat` out of a callee that CHECKED means every Nat the caller writes preserves everything the callee's types claim — the caller needs no value-cell-versus-key-cell knowledge, because a key-cell-returning `G` does not check. The deferral the Rust framing suggests is not closed off; it is the `12-` pin: the callee hands the position along as a FUNCTION of the issued borrow's final payload, and the loan's end plugs in the actual value — buying, beyond safety, what the write MEANS. The fill is that pin's ∀-weakening, available without new machinery.
...
## Status, and superseded readings (kept out of the way above)

The campaign ran overnight 2026-08-18; `15-packed-borrows-review.md` is the review guide. The fill and the segment-folding prerequisite both merged (as soundness fixes, under the session's delegation — the "nothing merges" line above was written before the probe found four live unsoundness holes independent of the hashmap). The acceptance above was half met: value-cell borrows check at index-0 carves, the key-cell control holds. The residual is the `gmValMin`/`gmValMid` family: SAFE callees refused, because the ∀-value check is a *normalization* — the invariant's `arrRec` must actually run far enough to show it drops the abstracted cell, and it sticks on an opaque head whenever the carve sits at index ≥ 1. Routes out: the `12-` pin (declare the contract; stop needing the fold to run), or an invariant fold the checker can evaluate from a symbolic prefix.

Two readings died en route and are recorded so they do not return: (1) asserting the owed type at the GROUP END without callee preparation — refuted by the position-ignorance argument above; (2) the residual as a spine-association/self-normalization failure of the carve itself — excluded by exact A/Bs (`midSplit3`, `midCarveEscN`, pinned in `Tests/OpaqueFill.lean` §7.5).
