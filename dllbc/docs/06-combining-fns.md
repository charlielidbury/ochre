...
**And a block may end in its `fn`.** The let-chain "ending in an expression" has one form that ends in a *binding* instead: a `fn` written with no `; rest` is the last statement of its block, and the block's value is `()`. That is not a second kind of block — a `let` in tail position is a term of this calculus already, and both arrows read one (⇒ takes it as a unit-valued statement, ⇝ binds it and answers `()`), so what the surface drops is the obligatory `; ()` and nothing besides. `fn` is the only statement form permitted to end a block this way: `let x = e` still owes its tail, and a block ending in one is refused by the grammar.
