...
**The return type is optional, and omitting it omits the seal.** `fn F (x : Nat) { x }` is `let F = λ (x : Nat) { x }` — the bare λ, with no Π written and so nothing to seal at. This is not an inferred signature; it is a different program. Everything §5 lists as what the seal delivers is what an unsealed `fn` gives up: the body is not audited at the binding, and callers are held to no contract. What it gets instead is the transparency §5's first reading reserves for execution — the λ β-reduces at each call site, so the checker meets the body *there*, and the body is checked once **per call site** and not at all when there is no call site. `fn G (x : Nat) { Bogus(x) }` with `Bogus` bound nowhere is accepted while it stays uncalled and refused at the first call; the same body with `-> Nat` is refused at the declaration, before any call exists.
...
**And a block may end in a statement.** The let-chain "ending in an expression" has a second form, which ends in a *statement* instead: any statement written with no `; rest` is the last of its block, and the block's value is `()`. That is not a second kind of block — a `let` or an `assign` in tail position is a term of this calculus already, and both arrows read one (⇒ takes it as a unit-valued statement, ⇝ binds it and answers `()`), so what the surface drops is the obligatory `; ()` and nothing besides. `fn` had this first and no longer has it alone: `let x = e`, `p := e` and `show x` end a block on the same terms, and the grammar gives every statement row the same optional tail. The `show` case is the one that took a decision rather than an unwrapping — its row is erased, emitting its tail and nothing else, so with no tail it emits `()`, which makes `show x` and `show x ; ()` the same term.
