...
The one system that has the same idea — a *takeover marker living inside the aggregate's own typed value* — is RefinedRust (PLDI'24), and it is worth naming because it is the limit case this design generalizes. There an array is `array_n T` refined by a list of element types; borrowing element `i` out of it rewrites the array's type to `blocked'b (array_c (maybe_init T))` with the borrowed element replaced by the prophecy observer `*γᵢ`, recombined when the lifetime `'b` ends. That is `Arr⟨…, 1 ▷ loanₘ ℓ, …⟩` in different notation: the aggregate stays whole, and the hole is *in it*, not described beside it. RefinedRust does this **one element at a time**, with the side conditions going to Coq tactics. ¶3's contribution over it is the step from an element to a range, and the step from a tactic to a checker premise.
...
where `l ↦∗{dq} vs` is *definitionally* the iterated points-to `[∗ list] i ↦ v ∈ vs, (l +ₗ i) ↦{dq} v`. Three features, and ¶3 inherits all three. It is a **bi-entailment**, so split and rejoin are one law read in two directions — which is why ¶3.3 has no rejoin rule. It is stated **at a split point**, not for an arbitrary subrange; a middle range is obtained by iterating it, and the residue comes out as the explicit pair `l ↦∗ take lo vs ∗ (l +ₗ hi) ↦∗ drop hi vs`. And **disjointness is not a side condition at all** — it is a theorem of pointer arithmetic, baked into `+ₗ length vs` and absorbed by `∗`. ¶3.1's "the tree does the aliasing, the proofs do the arithmetic" is that division transplanted: the segment partition plays `∗`, and premise (2) plays the in-bounds obligation.
...
**VST** has the range-parameterized form natively, and it is the closest sep-logic shape to `&mut a[lo ; cnt]` as a *type*: `array_at sh t gfs lo hi v p` is an assertion about a subrange of one object, with `split2_array_at` (`lo ≤ mid ≤ hi ⊢ array_at lo hi = array_at lo mid ∗ array_at mid hi`), `split3seg_array_at` (three-way, `[lo,ml) ∗ [ml,mr) ∗ [mr,hi)`) and `split3_array_at` (prefix ∗ single cell ∗ suffix) in `floyd/field_at.v:685-731`. Side conditions are pure arithmetic discharged by `lia`; rejoin is the same equation right-to-left. If ¶3's segment list has a direct ancestor, `split3seg_array_at` is it.
...
Two honest qualifications, both important. First: **Low\* has no exclusivity to protect.** Buffers may overlap freely; nothing stops a program holding two overlapping sub-buffers. Bounds and liveness are the safety obligations, and disjointness is needed only to prove *functional* facts through `modifies`. So Low\* supplies concern (i) and (iii) of ¶0.1 and not (ii) — it is not a borrow discipline, and its disjointness lemma is not enforcing anything. Second: **the arithmetic is not where the burden lives.** The `.fsti` carries a handful of `gsub` lemmas and forty-odd `modifies_*` lemmas — `modifies_trans`, `modifies_loc_includes`, `modifies_buffer_elim`, `modifies_loc_buffer_from_to_intro`, and so on. Framing *composition* is what a Low\* development pays for. That machinery exists because there is a heap to frame over; ¶3.6 is the argument that a heapless ownership tree does not need it, and it is the design's one substantive claim of improvement over its nearest neighbour — entirely downstream of §0's no-heap decision rather than of anything invented here.
...
So rustc *already performs interval-disjointness reasoning* — it is a real, safe, static split — but only where the bounds are compile-time constants, which in practice means slice patterns (`[a, b, rest @ ..]`). The moment an index is a variable it gives up conservatively, and there is no channel by which a proof could reach the decision: the place language has syntax, not propositions. **Proof-conditioned disjointness is precisely the replacement of that hardwired constant-only test with a user-suppliable derivation.**
...
On the proposal landscape, briefly, since "someone must have proposed this" is the natural next thought. **View types / partial borrows** (Matsakis 2021; rfcs#1215) are at field-of-struct granularity, not ranges, and have not landed. **Generativity / branded indices** (the `indexing` crate; GhostCell, POPL'21) tie an `Index<'id>` to one container by a lifetime brand, making in-bounds a type-level invariant and eliminating bounds checks — but the brand explicitly does *not* track disjointness or exclusivity (indices and ranges are freely `Copy`), so it solves the **bounds** half of premise (2) and none of the aliasing half. **Polonius/NLL** refine *when* borrows are live, not *whether* places overlap. Nothing pending makes `split_at_mut` derivable.
...
*In the mechanized semantics there are no aggregates at all.* The in-repo `aeneas/` subtree (the `mechanized-llbc` development for the 2024 symbolic-semantics paper) has value grammar `VBottom | VInt | VBool | VMutLoan ℓ | VMutBorrow ℓ v` (`LLBC.v:16-23`), HLPL+ adding only `VLoc | VPtr` (`HLPL_plus.v:19-27`), and a projection grammar of exactly `Deref | Field(nat)` (`lang.v:9-11`). A case-insensitive grep for `array`, `slice`, `split_at` or `subslice` across all 15,816 lines of `.v` returns **nothing**. So the in-repo prior art on this topic ends at the `PathToSubtree` trichotomy quoted in ¶3.1 — which is genuinely the right structural ancestor, and is the reason ¶3.1 can claim there is no disjointness *test*.

*The borrow machinery is not "over whole values"* — this was the draft's error. LLBC borrows at **tree-node** granularity: you may borrow a field deep inside a value, and `eval_proj`'s single rule (`E_Deref_MutBorrow`, `LLBC.v:155-161`) walks into a value tree node by node. What is missing is not sub-value borrowing but *range-of-children* borrowing: there is no way to borrow "children `[lo, lo+cnt)` of one node." That is a much narrower gap than "no partial borrows," and it is exactly the gap ¶2's range step fills.

*Upstream, arrays are a modelling decision that bypasses places entirely.* Charon eliminates MIR's `ProjectionElem::Index` and `Subslice` in a micro-pass (`--index-to-function-calls`), turning them into **builtin function calls** — `ArrayToSliceMut`, `SliceSubSliceMut : (&mut [T], usize, usize) -> &mut [T]`, `Index(BuiltinIndexOp)`. Array element access never reaches the interpreter's place projection at all (`InterpPaths.ml`'s `project_value` handles only `Field` and `Deref`). The backend then models `Slice α := { l : List α // l.length ≤ Usize.max }` and gives `split_at_mut` the type `Slice T → Usize → Result ((Slice T × Slice T) × ((Slice T × Slice T) → Slice T))`: a `take`/`drop` pair plus a backward function that re-appends when the lengths still match. A second draft error to correct: this is **not an axiom**. In the current Lean backend it is a definition with *proved* spec theorems. What is trusted is the modelling step itself — nothing connects that definition to any borrow-and-loan semantics.

The consequence is the sharpest sentence in this section: **in Aeneas, the two halves of `split_at_mut` are never simultaneously-live borrows in any semantics.** The functional translation replaces them with "produce two pure lists and a recombining continuation," so disjointness never arises as a proposition anywhere. That is not a criticism of Aeneas — punting is a legitimate design choice for a translation-based tool — but it settles this project's positioning: the range-place design is not a reimplementation of something Aeneas has. It is the thing Aeneas assumes.

**Verus.** Two layers, and conflating them would flatter the comparison. At the **safe-Rust layer**, `&mut [T]` is viewed functionally as `slice@ : Seq<T>` and `split_at_mut` is an *assumed* (`external_body`) spec — `requires 0 ≤ mid ≤ len`, `ensures ret.0@ == old@.subrange(0,mid)`, `ret.1@ == old@.subrange(mid,len)`, and the recombination `final(slice)@ == final(ret.0)@ + final(ret.1)@`. The prophecy-flavoured rejoin is right there in the spec, but soundness rides entirely on rustc's borrow checker; Verus proves nothing about aliasing, and there is no `vstd` primitive for "two `&mut` subranges given a user `Le` proof." At the **tracked-permission layer**, `PointsToRaw::split`/`join` (¶3.3) is the real thing: an affine token over an address set, split by a user-discharged `range.subset_of(self.dom())`, residue by set difference, coexistence free by construction. This is the closest existing instance of "proof-conditioned disjointness," and ¶3.3 adopts its shape. Its limitation for our purposes is placement, not power: it lives in ghost proof code around raw pointers, never in the type of a surface `&mut`.
...
**Creusot / Prusti.** Creusot gives `&mut T` a prophecy pair (current, final) and models slices as `Seq`; `split_at_mut`'s spec splits the prophecy pointwise across the halves (`(^self)@.subsequence(0,mid) == (^l)@`, and dually), and single-index borrows frame by a spec-level quantifier `resolve_elsewhere : ∀ i ≠ ix, old[i] == fin[i]` — a quantifier, not a capability. Like Verus's safe layer: rustc guarantees the aliasing, Creusot only translates it. Prusti's backend (Viper) *can* express range footprints as quantified permissions with an injectivity side condition, but Prusti's place-capability encoding did not ship slice/array support, so it is not a competitor on this exact point.

**RefinedRust** is treated in ¶1.1, where it belongs: it is the only system that puts the takeover marker *inside* the aggregate's typed value, and it does so one element at a time.

**What none of them does.** Line the systems up against ¶0.1's three concerns and the gap is a hole in a table, not a missing idea. Rust hardwires (i) for constants only and otherwise checks at run time. Iris, VST and RustBelt discharge (i) in the meta-logic, by proofs written in a proof mode. Low\* has (i) and (iii) but no (ii) to enforce. Verus-tracked has (ii) and (iii) with genuine user-discharged obligations, but on ghost tokens detached from surface borrows. Aeneas, Creusot and Prusti trust the primitive. **No prior system makes program-supplied dependent evidence license the coexistence of two surface mutable borrows inside the operational semantics and the borrow checker itself.** That is available here only because DLLBC has a comptime fragment inside the same language as its ownership discipline — which is the whole reason the calculus exists, and is therefore the right place for this design to be tested.
