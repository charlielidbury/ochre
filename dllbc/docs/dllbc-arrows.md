...
*No `copy`, no `take`, no `reborrow`.* All three are derived. `take` is `*x` read under ⇒ (destructive read through a borrow, leaving a hole). `reborrow` is `&mut *x`. `copy` is a *generated function* `T.copy : T → T × T` (consume-and-rebuild, §7), used via the idiom `match T.copy x { Pair(l, r) => { x := l ; …r… } }` — and rarely needed at all, because type-level reads go through snapshots (⇝) at no runtime cost. (In practice, superseded by §2.1's copy-on-read refinement: hereditarily marker-free values are read non-destructively, so the generated copy's remaining role is as the unrestrictedness witness.)
...
One refinement, adopted when the mechanization met index arithmetic (every reuse of a loop index was a use-after-move): a ⇒-read *consumes* only when it must. A value that is **hereditarily marker-free** — no borrow, no loan, no hole anywhere in its tree, which includes every symbolic σ by construction — is read by *copy*, the owner keeping it: on such values the ownership machinery is vacuous (§0), and vacuous machinery should not bite. Under the refinement, the trace above reads `x ↦ 3, y ↦ 3` — the drawn `x ↦ ⊥` shows the uniform-move semantics that survives only for values that *do* carry markers, which is where every move/⊥/refill mechanic in this section genuinely operates (§2.4's takes produce the holes). This is Rust's `Copy` derived as a value-level fact rather than declared as a trait, and it retires §7.2's generated `T.copy` from practical duty — it remains the classification witness (a type is unrestricted iff its copy function is generatable).
