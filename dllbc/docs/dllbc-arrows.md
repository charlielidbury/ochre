...
The ending discipline is the group's whole content: **every issued borrow ends first** (its payload surrendered and discarded), **then the group ends**, releasing each captured loan with a fresh, unconstrained existential. The ordering is the soundness argument made structural — `a` cannot recover while `r` lives, because ℓₐ is held by the group and the group cannot end before ℓᵣ. A wire is the degenerate group — one captured, one issued, existential constrained to the surrendered value — so §5 and §6 are one mechanism at two precisions. Two provisos on that constrained release, both from the mechanization. It presupposes the issued borrow spans the captured loan's *whole* payload: a function returning a *field* reborrow — `advance`, the cursor step — surrenders only the field, and constraining the owner's release to it would be value-wrong. And it cannot be signature-inferred: `through (b : &mut List T) → &mut List T = b` and an `advance` of the same signature differ only in body, which signature-only checking cannot see — so under opaque calls the constrained wire is *unavailable*, every captured release is a fresh existential, and `through`'s precision is recoverable only from §6.2's transparent or spec group-ends. (v0 briefly inferred constrained from the signature; that was unsound and is removed in the soundness pass.)
