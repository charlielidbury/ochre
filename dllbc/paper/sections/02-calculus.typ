...
The write `*b := 7` peels one indirection under $arrow.l.double$ and replaces the payload in place; no contract governs it, because inside the body borrowing is contract-free. The last line is where laziness pays out: the move needs to read `x`, but `x` holds a loan and no rule applies, so the environment reorganizes first. `End-Mut` (@fig-reorg) plugs the borrow's current payload back where the marker sits and kills the borrow --- itself just a $arrow.l.double$-fill aimed at the environment's own bookkeeping --- after which the move proceeds. Nothing marked _when_ $ell$ had to end; the demand for `x` did.
