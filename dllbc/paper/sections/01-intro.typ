...
@calculus presents the grammar, the four arrows, and the first programs run on the machine; @identification develops the identification of refinement with dependent matching and the fording library it makes possible. The remainder of the paper takes up the boundary discipline and the audit at return, works the quicksort case study, reports the differential harness and the verified invariants, sets the two verification architectures side by side, and closes with lessons and related work.

DLLBC is the mutation-stage vehicle of a longer program. It supersedes an earlier staging --- a pure core with equirecursive subtyping and self-types, then an ownership layer, then mutation --- by carrying ownership and dependency together from the start, which is where their interaction, the object of study, actually lives.
