...
def zeroAll : Decl :=
  { name := "zero_all", retType := .const "Unit",
...
def toNat : Decl :=
  { name := "to_nat", retType := .const "Unit",
    telescope := [("v", .borrowT boolT natT)],
    body := dllbcWith [v] { *v := 0; () } }
...
def toNatCaller : Decl :=
  { name := "caller", retType := natT, telescope := [],
    body := dllbcWith [] { let x = True; let b = &mut x; to_nat(b); let y = x; y } }
...
def rbCaller : Decl :=
  { name := "caller", retType := listNatT, telescope := [],
    body := dllbcWith [] { let x = Cons(1, Nil); let b = &mut x; push(7, &mut *b); let y = x; y } }
...
example : checkFnErr
  { name := "c", retType := .const "Unit", telescope := [],
    body := dllbcWith [] { let x = Cons(1, Nil); let b = &mut x; push(True, b); () } }
  "parameter type" [pushList] = true := by native_decide

-- A non-borrow where a borrow argument is expected.
example : checkFnErr
  { name := "c", retType := .const "Unit", telescope := [],
    body := dllbcWith [] { let x = Cons(1, Nil); push(7, x); () } }
...
example : checkFnErr
  { name := "c", retType := .const "Unit", telescope := [],
    body := dllbcWith [] { let x = Cons(1, Nil); let b = &mut x; push(7, b); let z = b; () } }
