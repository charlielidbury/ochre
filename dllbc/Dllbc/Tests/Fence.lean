...
`prog_parse { … }` suppresses only the elaboration-time check. Rejection
...
/-! ## (1) The fence changes no value
    ...
    `prog_parse { … }` and `ty{ … }` on the same source must build the
    same `Term`; `ty{ }` is the non-checking brace below the checker. -/

example :
    ((prog_parse { fn F (b : Bool) -> Unit { () }; let r = F(True); () })
