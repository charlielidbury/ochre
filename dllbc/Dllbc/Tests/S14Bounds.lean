...
def nth2 : Decl :=
  decl{ fn nth2 (v : &mut List Nat, i : Nat, j : Nat, pij : Le (S i) j, p2 : Le (S j) (len *v)) -> Σ (e : &mut Nat) → &mut Nat {
    match v {
      Nil => botElim Unit p2,
      Cons(hd, tl) => { match i {
        Z => { match j {
...
def swap : Decl :=
  decl{ fn swap (v : &mut List Nat, i : Nat, j : Nat, pij : Le (S i) j, p2 : Le (S j) (len *v)) -> Unit {
...
def sc : Decl := decl{ fn sc () -> Unit {
  let x = Cons(1, Cons(2, Cons(3, Nil)));
  let bb = &mut x;
  swap(bb, 0, 2, (), ());
  let y = x;
  () } }
example : checkFnOk sc ([nth, nth2, swap, sc]) = true := by native_decide

-- CONCRETELY: `swap(v, 0, 2)` on `[1,2,3]` yields `[3,2,1]`.
example :
    (match Dllbc.Tests.S9Diff.runExec [nth, nth2, swap] sc.body with
...
def oob : Decl := decl{ fn oob () -> Unit {
  let x = Cons(1, Cons(2, Cons(3, Nil)));
  let bb = &mut x;
  swap(bb, 0, 4, (), ());
  let y = x;
  () } }
example : checkFnErr oob "does not have its parameter type"
  ([nth, nth2, swap, oob]) = true := by native_decide
...
def cascade : Decl := decl{ fn cascade () -> Unit {
  let x = Cons(1, Cons(2, Cons(3, Nil)));
  let bb = &mut x;
  let pp = nth2(bb, 0, 2, (), ());
...
def rejectProbe : Decl := decl{ fn reject () -> Unit {
  let x = Cons(1, Cons(2, Cons(3, Nil)));
  let bb = &mut x;
  let pp = nth2(bb, 0, 2, (), ());
...
example : Dllbc.Tests.S9Diff.diffV2 false [nth, nth2, swap] sc.body = true := by native_decide
