...
/-! ## Σ binds with a DOT, and the dot is the only spelling
    ...
    **Consequence for branches written before this**, measured against
    `origin/main` rather than assumed. Five in-flight branches add a file that
    spells Σ with an arrow, and those files will fail to PARSE at merge — not
    fail a check, which is a friendlier failure than it sounds, since a parse
    error names its line:

      hm-probe-opt/…/Tests/HmProbeOpt.lean           107
      hm-finite-ints/…/Tests/Finite.lean              38
    ...
    189 sites, and the fix for each is one run of `dllbc/scripts/sigmadot.py`
    ...
    Two more branches need NO such run — `hm-option-kernel`'s `Tests/OptionT.lean`
    and `audit-exemption-fix`'s `Tests/AuditExemption.lean` are already
    arrow-free — but both modify files the migration rewrote (`Uni.lean`,
    `Machine.lean`, `Pure.lean`, and this file), so their merges want reading
