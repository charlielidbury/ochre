...
/-- **Check a program** (§8): one symbolic ⇒-walk, auditing each path's result at
    ...
    table. -/
def checkProgramDiag (t : Term) (retType? : Option Term := some (.const "Unit"))
    (table : List FnDef := []) : Except Diag Unit :=
  let paths :=
    (exploreD defaultFuel (pushContinuations t) { initSt with decls := table }).map
      (fun r => r.bind (fun p =>
...
def checkProgram (t : Term) (retType : Term := .const "Unit")
  ...
  Except.mapError Diag.msg (checkProgramDiag t (some retType) table)
