...
/-- **Which arrow evaluates a `let`'s right-hand side** (M31 Stage A).
    ...
    it is why a capital binder is non-consuming. The seal is the ONE exception,
    and the reason is that it is not a reading at all: `(t : T)` is an EVENT (it
    mints a fresh σ), and `readC` refuses it by name for precisely that reason —
    "minting a fresh σ needs an event and ⇝ has none".

    So the arrow here is a property of the RIGHT-HAND SIDE, not only of the
    binder: the seal's arrow is ⇒ whatever the binder's case, and what the capital
    binder changes is the BINDING — erased, ⇝-readable, never ⇒-consumed — not the
    event that produced its value. This is what lets `fn F …` desugar, exactly as
    §2.1 says it does, to a comptime `let` of a λ ascribed its Π: the declaration
    is still the event it always was, and only its mode moved.
