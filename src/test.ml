...
let _ =
  QCheck_runner.run_tests
    [
      QCheck.Test.make ~name:"sequence returns rhs"
        QCheck.(pair int int)
        (fun (x, y) ->
          let program = Prim (PInt x) <+> Prim (PInt y) in

          let result, _end_state = eval program Scope.empty in

          match result with PInt yy -> y == yy | _ -> false);
      QCheck.Test.make ~name:"declarations add to scope"
        QCheck.(int)
        (fun x ->
          let program = Ident "x" <=> Prim (PInt x) in
