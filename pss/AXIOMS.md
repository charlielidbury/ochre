...
the axioms / named hypotheses below.

## Two-tier dependency model

After the de Bruijn refactor, the headline theorems live in two parallel
forms — the **primary** de Bruijn `_proved` endpoints and the
**documentation-only** LN-side endpoints. The two tiers track different
kinds of obligations.

* **De Bruijn `_proved` endpoints depend on kernel axioms + named
  Prop/Type residual hypotheses.** Residual hypotheses are explicit
  arguments to the theorem; they are NOT Lean `axiom` declarations.
  `#print axioms` on a `_proved` endpoint shows the kernel three
  (`propext`, `Quot.sound`, `Classical.choice`) only. There are
  **zero** custom axioms on the de Bruijn side.

* **LN-side endpoints depend on actual Lean `axiom` declarations.**
  These remain in the audit as documentation / paper-faithfulness
  references during the de Bruijn refactor; they will be retired once
  the LN side is removed.

This file is organized to reflect that split:

1. **De Bruijn primary headlines (the actual proof status)** — kernel
   axioms only, plus named residual hypotheses.
2. **LN-side actual `axiom` declarations** — 12 axioms (1 permanent,
   9 active outstanding in LN-side headlines, 2 inactive). These still
   exist in the source tree; the de Bruijn side does not depend on any
   of them.

**LN-side axiom count: 12** (1 permanent, 9 active outstanding in
LN-side headline closures, 2 inactive outstanding).

## De Bruijn primary headlines (post-iter pss-20260506-130802)
...
### The eight de Bruijn `_proved` endpoints
...
#### Group A — Standing transitivity-elimination residuals

These are the open metatheoretic problems on which the headline
closures are conditional — universal single-step companions of the
relations the headlines lift to chain output.

* `UniformStrongCommutes : Prop` — universal single-step
  `StrongCommutes Γ s` at every extended context. Defined at
  `DeBruijnTypeSafety.lean:21202`. Consumed by Lemma 1 / Theorem 3 /
  Theorem 4 closures. The standing transitivity-elimination problem;
  discharge requires closing the full `MSubRed × MEqRed` case grid
  unconditionally. The de Bruijn switch eliminated the
  alpha-equivariance wall; what remains is structural recursion plus
  the per-cell transports in Group C below. Per-cell case progress is
  in `DeBruijnTransitivityElim.lean`'s `StrongCommutes.*` namespace.

* `UniformEqDiamonds : Prop` — universal single-step `EqDiamonds Γ s`
  at every extended context. Defined at
  `DeBruijnTypeSafety.lean:20215`. Consumed by Lemma 2 closures.
  Smaller case grid than `UniformStrongCommutes`; many cells closed
...
propagate well-formedness across a single operational step.

* `BetaInstantiationPreservesWfM : Type` — de Bruijn analogue of paper
  Lemma 7. Defined at `DeBruijnTypeSafety.lean:193`. Substituting an
  argument that is a well-subtype of the abstraction bound preserves
  body well-formedness. Partial discharge `MEqRedPreservesWfM_partial`
  ships the easy three constructor arms (`top`, `var`, `tAp`)
  unconditionally; `pro`, `bet`, `app`, `fun_` arms remain explicit
  hypotheses (commit `4b547d0`).

* `AbsFunctionBoundInversion : Type` — function-bound inversion: if
  `WSubMStar Γ (.abs bound body) (.abs result .top)`, then bounds are
  transitively well-equivalent. Defined at
  `DeBruijnTypeSafety.lean:4043`. A weaker variant
  `AbsFunctionBoundInversionUnderWfCtx` (with `WfCtxEqu Γ` premise)
  matches the actual invariant available in contextual preservation.

* `WfMSubHeadReplaceOfNewWf : Type` — sharpened `.sub`-head replacement
  payload (assumes the replacement bound is already well-formed).
  Defined at `DeBruijnTypeSafety.lean:10890`.

#### Group C — Per-cell transport residuals (unblock Group A)
...
* `MEqRedArgTransportPayload` (Type) — argument-position transport
  across argument chain. Used by Lemma 2 chain assembly.
  **Restricted variants proved**: `MEqRedArgTransportPayloadRestricted_proved`
  (`Term.NoBinders` arg + `Term.AbsFree` body),
  `MEqRedArgTransportPayloadNoBinders_proved` (`Term.NoBinders` arg,
  body unconstrained).

* `MEqRedOpStackHeadTransportPayload` (Type) — operator stack-head
  transport for `MEqRed`. Used by `app × app` chain assembly.
  **Restricted variant proved**: `MEqRedOpStackHeadTransportPayloadRestricted_proved`.

* `MEqRedSubBridgePayload` (Type) — `.equ → .sub` head bridge for
  `MEqRed` bodies. Used by cross-β cells (`bet × app`, `app × bet`,
  `equ × bet`). **Restricted variant proved**:
  `MEqRedSubBridgePayloadNoBinders_proved` (`Term.NoBinders` body
  source).

* `MSubRedOpStackHeadTransportPayload` (Prop) — operator stack-head
  transport for `MSubRed`. Used by `app × app` strong-commutes
  assembly. **Restricted variant proved**:
  `MSubRedOpStackHeadTransportPayloadRestricted_proved`.

* `MSubBridgePayload` (Type) — cross-relation, cross-head bridge:
  collapses `MSubRed` body step under `.equ`-head into `MEqRed` body
  step under `.sub`-head with the original abstraction bound. Strictly
  more demanding than `MEqRedSubBridgePayload` because it must collapse
  `MSubRed`'s extra constructors (`Ms-Pro`, `Ms-Top`, `Ms-FOp`) into
  `MEqRed`. **Restricted variants proved**:
  `MSubBridgePayloadEquNoBinders_proved`,
  `MSubBridgePayloadNoBindersAsSub_proved`.

* `MSubBodyNarrowPayload` (Type) — `.sub`-head abstraction-bound
  narrowing for `MSubRed` bodies. Used by the `fun_fun_of` cell to
  migrate the right-edge body subtype step across the abstraction-bound
  diamond. **Restricted variant proved**:
  `MSubBodyNarrowPayloadNoBinders_proved`.
...
| `MEqRedArgTransportPayloadNoBinders_proved` | proved (`NoBinders` arg, body unconstrained) |
...
| `MEqRedPreservesWfM_partial` | three constructor arms (`top`, `var`, `tAp`) unconditional; four (`pro`, `bet`, `app`, `fun_`) as named hypotheses (commit `4b547d0`) |
...
The full audit trail of partial-discharge commits is searchable via
`git log --oneline --all --grep="_NoBinders_\|_AbsFree_\|_partial\|_Restricted_proved"`.
The per-payload `#print axioms` audit is wired up in
`Pss/DeBruijnSanity.lean`.

### Terminology — "residual hypothesis" vs "axiom"

"Residual hypothesis" is *not* a synonym for "axiom" in this section.
Each named residual is a `def` whose body is a `Prop` or `Type`;
passing one in as an explicit argument records the open obligation in
the theorem signature without committing the build to a Lean `axiom`
declaration. `#print axioms` on a `_proved` endpoint shows the kernel
three only, not the residual names. Compare:

* **Actual axiom** — Lean `axiom` declaration; appears in the
  transitive `#print axioms` closure of any theorem that depends on
  it. The 12 LN-side axioms below are this.
* **Named residual hypothesis** — `def` returning a `Prop` or `Type`;
  passed as an explicit theorem argument. Does NOT appear in
  `#print axioms` output of theorems that take it. The de Bruijn
  primary headlines depend only on these.

DO NOT introduce any of the named residuals (`Uniform*`,
`*TransportPayload`, `*BridgePayload`, `*NarrowPayload`,
`BetaInstantiationPreservesWfM`, `AbsFunctionBoundInversion`,
`WfMSubHeadReplaceOfNewWf`) as Lean `axiom` declarations under any
circumstance. The current shape is the correct one — when the campaign
discharges a residual, the corresponding `_of` lemma is wired in and
the `_proved` endpoint becomes unconditional in lockstep, with the
hypothesis disappearing from its signature.
...
Per CLAUDE.md §5a "Hardest case first":

1. **`UniformStrongCommutes`** — closing the `MSubRed × MEqRed` case
   grid unconditionally. Falls Theorem 3 and Theorem 4 in lockstep.
2. **`UniformEqDiamonds`** — same shape on the `MEqRed × MEqRed` side;
   smaller case grid; cross-β cells routed through Group C.
3. **Group C transport residuals** — widening each restricted variant
   to its general form. Hardest-first: `MSubBridgePayload` (cross-
   relation cross-head bridge collapsing `Ms-Pro`/`Ms-Top`/`Ms-FOp`
   into `MEqRed`).
4. **`BetaInstantiationPreservesWfM`** — completing the four hard
   constructor arms left as hypotheses on `MEqRedPreservesWfM_partial`.
   Hardest arm: `bet` (subject-reduction-class substitution lemma).
5. **`AbsFunctionBoundInversion`, `WfMSubHeadReplaceOfNewWf`** —
   smaller, structurally independent of (1)–(4).
...
* `Pss/Mpss/DeBruijnTypeSafety.lean` — merged the under-heads
  ...
  `Pss/DeBruijnSanity.lean`. No axiom-count change.
* `Pss/Mpss/DeBruijnTypeSafety.lean` — added
  `EqDiamonds.bet_bet_chain_ArgNoBinders_of`, a `Me-Bet × Me-Bet`
  chain-output cell that needs only `Term.NoBinders` for the β argument
  and no `AbsFree`/`NoBinders` premise on the joined body. It uses
...
* `Pss/Mpss/DeBruijnTypeSafety.lean` — added
  ...
  No axiom-count change.
* `Pss/Mpss/DeBruijnTypeSafety.lean` — added
  `MSubRed.preserves_noBinders`, `MSubRedJ.preserves_noBinders`, and
  `MSubRedStar.preserves_noBinders`, giving the subtype-reduction side the
  same binder-free shape tracking already available for `MEqRed`. Added
  the endpoints to `Pss/DeBruijnSanity.lean`. No axiom-count change.
* `Pss/Mpss/DeBruijnTypeSafety.lean` — added
  `MSubRedStar.lift_to_any_context_stack_of_NoBinders`, the chain-level
...
* `Pss/Mpss/DeBruijnTypeSafety.lean` — added
  ...
  `Pss/DeBruijnSanity.lean`. No axiom-count change.
* `Pss/Mpss/DeBruijnTypeSafety.lean` — added
  `MEqRedStar.op_stack_head_transport_of_NoBinders`, a chain-level
  restricted operator stack-head transport theorem. It retargets a
...
* `Pss/Mpss/DeBruijnTypeSafety.lean` — added
  ...
  `Pss/DeBruijnSanity.lean`. No axiom-count change.
* `Pss/Mpss/DeBruijnTypeSafety.lean` — added
  `StrongCommutes_proved`, the top-level chain-output Lemma 1 closure
  assembled from the de Bruijn strong-commutation cells. It remains
  conditional on the existing `MEqRed` transport/bridge residuals plus
  the new `MSubRedOpStackHeadTransportPayload`, `MSubBridgePayload`,
...
* `Pss/Mpss/DeBruijnTypeSafety.lean` — added
  ...
  axiom-count change.
* `Pss/Mpss/DeBruijnTypeSafety.lean` — added
  `StrongCommutes_proved_of_split_branch_handlers`, a chain-output
  Lemma 1 closure whose app/bet and fun/fun branch residuals are expanded
  to constructor-local payloads. It composes the split branch-handler
  builders and keeps the structural app/app transports plus
  `MEqRedArgTransportPayload`, `MEqRedSubBridgePayload`,
...
* `Pss/Mpss/DeBruijnTypeSafety.lean` — proved
  ...
  the endpoints to `Pss/DeBruijnSanity.lean`. No axiom-count change.
* `Pss/Mpss/DeBruijnTypeSafety.lean` — proved
  `StrongCommutesFunFunBodyAppAppSubEquChainPayload.proved` from
  `UniformEqDiamonds`. The constructor-local `Ms-Equ` subtype-operator
  app/app case is handled as an ordinary equivalence body diamond under
  the original abstraction bound, then transported to the joined bound.
  Added
  `StrongCommutes_proved_of_split_chain_fun_app_sub_cases_top_equ_handlers`,
...
* `Pss/Mpss/DeBruijnTypeSafety.lean` — split the constructor-local
  ...
  Added the endpoints to `Pss/DeBruijnSanity.lean`. No axiom-count change.
* `Pss/Mpss/DeBruijnTypeSafety.lean` — split the constructor-local
  fun/fun structural app/app nested `Ms-App` subtype-operator case into
  nested `Ms-Pro`, `Ms-Top`, `Ms-Equ`, `Ms-App`, and `Ms-FOp` payloads;
  proved the nested `Ms-Top` and `Ms-Equ` leaves as
  `StrongCommutesFunFunBodyAppAppSubAppTopChainPayload.proved` and
  `StrongCommutesFunFunBodyAppAppSubAppEquChainPayload.proved` from
  `UniformEqDiamonds`. Added
  `StrongCommutesFunFunBodyAppAppSubAppChainPayload.of_nested_cases` and
  `StrongCommutes_proved_of_split_chain_fun_app_sub_cases_nested_app_handlers`.
  The nested `Ms-Pro`, recursive nested `Ms-App`, and nested `Ms-FOp`
  leaves remain explicit residuals. Added the endpoints to
  `Pss/DeBruijnSanity.lean`. No axiom-count change.
* `Pss/Mpss/DeBruijnTypeSafety.lean` — split the nested `Ms-Pro` leaf of
  the fun/fun structural app/app nested `Ms-App` subtype-operator case into
  `StrongCommutesFunFunBodyAppAppSubAppProHeadChainPayload` and
  `StrongCommutesFunFunBodyAppAppSubAppProSuccChainPayload`; proved the
  stable successor case as
  `StrongCommutesFunFunBodyAppAppSubAppProSuccChainPayload.proved` from
  `UniformEqDiamonds`. Added
  `StrongCommutesFunFunBodyAppAppSubAppChainPayload.of_nested_cases_pro_split`
  and rewired
  `StrongCommutes_proved_of_split_chain_fun_app_sub_cases_nested_app_handlers`
  so the live route exposes only the changed-head nested `Ms-Pro`, recursive
  nested `Ms-App`, and nested `Ms-FOp` residuals. The changed-head nested
...
* `Pss/Mpss/DeBruijnTypeSafety.lean` — proved the nested-recursive
  ...
  No axiom-count change.
* `Pss/Mpss/DeBruijnTypeSafety.lean` — split the nested-recursive `Ms-Pro`
  leaf into
  `StrongCommutesFunFunBodyAppAppSubAppAppProHeadChainPayload` and
  `StrongCommutesFunFunBodyAppAppSubAppAppProSuccChainPayload`; proved the
  stable successor case as
  `StrongCommutesFunFunBodyAppAppSubAppAppProSuccChainPayload.proved` from
  `UniformEqDiamonds`. Added
  `StrongCommutesFunFunBodyAppAppSubAppAppChainPayload.of_nested_cases_pro_split`
  and rewired
  `StrongCommutes_proved_of_split_chain_fun_app_sub_cases_nested_app_handlers`
  so the live recursive nested residuals are now the changed-head
  nested-recursive `Ms-Pro`, nested-recursive `Ms-App`, and nested-recursive
  `Ms-FOp`, plus the sibling nested `Ms-FOp` and shallower changed-head
  `Ms-Pro` cases. Added the endpoints to `Pss/DeBruijnSanity.lean`.
  No axiom-count change.
* `Pss/Mpss/DeBruijnTypeSafety.lean` — split the one-deeper
  nested-recursive `Ms-App` residual
  `StrongCommutesFunFunBodyAppAppSubAppAppAppChainPayload` by its next
  subtype-operator constructor, including a further split of its `Ms-Pro`
  leaf into changed-head and stable-successor payloads. Added
  `StrongCommutesFunFunBodyAppAppSubAppAppAppChainPayload.of_nested_cases`
  and
  `StrongCommutesFunFunBodyAppAppSubAppAppAppChainPayload.of_nested_cases_pro_split`,
  and rewired
  `StrongCommutes_proved_of_split_chain_fun_app_sub_cases_nested_app_handlers`
  ...
  `Pss/DeBruijnSanity.lean`.
  No axiom-count change.
* `Pss/Mpss/DeBruijnTypeSafety.lean` — split the two-deeper recursive
  `Ms-App` residual
  `StrongCommutesFunFunBodyAppAppSubAppAppAppAppChainPayload` by its next
  subtype-operator constructor, again splitting the local `Ms-Pro` leaf
  into changed-head and stable-successor payloads. Added
  `StrongCommutesFunFunBodyAppAppSubAppAppAppAppChainPayload.of_nested_cases`
  and
  `StrongCommutesFunFunBodyAppAppSubAppAppAppAppChainPayload.of_nested_cases_pro_split`,
  and rewired
  `StrongCommutes_proved_of_split_chain_fun_app_sub_cases_nested_app_handlers`
  so the two-deeper recursive `Ms-App` case is no longer opaque. The live
  two-deeper residuals are changed-head `Ms-Pro`, stable-successor
  `Ms-Pro`, `Ms-Top`, `Ms-Equ`, recursive `Ms-App`, and `Ms-FOp`, alongside
  the remaining one-deeper changed-head `Ms-Pro`, one-deeper `Ms-FOp`,
  nested-recursive `Ms-FOp`, sibling nested `Ms-FOp`, and higher
  changed-head `Ms-Pro` cases. Added the endpoints to
  `Pss/DeBruijnSanity.lean`. No axiom-count change.
* `Pss/Mpss/DeBruijnTypeSafety.lean` — discharged the exposed two-deeper
  recursive `Ms-Top` leaf as
  `StrongCommutesFunFunBodyAppAppSubAppAppAppAppTopChainPayload.proved`.
  The proof extends the shallower `Top` pattern by one application layer:
  after the fun-bound join it collapses the left nested `Top` spine with
  repeated `Me-TAp` steps and closes the right side with `Ms-Top`.
  `StrongCommutes_proved_of_split_chain_fun_app_sub_cases_nested_app_handlers`
  now supplies this proof directly, so the live two-deeper residuals are
  changed-head `Ms-Pro`, stable-successor `Ms-Pro`, `Ms-Equ`, recursive
  `Ms-App`, and `Ms-FOp`. Added the endpoint to
  `Pss/DeBruijnSanity.lean`. No axiom-count change.
* `Pss/Mpss/DeBruijnTypeSafety.lean` — discharged the exposed two-deeper
  recursive `Ms-Equ` leaf as
  `StrongCommutesFunFunBodyAppAppSubAppAppAppAppEquChainPayload.proved`.
  This reuses the old-bound equivalence-body diamond after building the
  extra structural application layer, then transports the joined body
  across the fun-bound join. The nested-app closure now supplies this proof
  directly, leaving live two-deeper residuals at changed-head `Ms-Pro`,
  stable-successor `Ms-Pro`, recursive `Ms-App`, and `Ms-FOp`. Added the
  endpoint to `Pss/DeBruijnSanity.lean`. No axiom-count change.
* `Pss/Mpss/DeBruijnTypeSafety.lean` — discharged the exposed two-deeper
  stable-successor `Ms-Pro` leaf as
  `StrongCommutesFunFunBodyAppAppSubAppAppAppAppProSuccChainPayload.proved`.
  The proof extends the successor-lookup pattern through four structural
  application layers, rules out an equivalence binding at the shared
  successor head, transports the lookup across the joined fun bound, and
  advances the four arguments plus the final body argument on the left.
  `StrongCommutes_proved_of_split_chain_fun_app_sub_cases_nested_app_handlers`
...
* `Pss/Mpss/DeBruijnTypeSafety.lean` — split the remaining two-deeper
  ...
  endpoints to `Pss/DeBruijnSanity.lean`. No axiom-count change.
* `Pss/Mpss/DeBruijnTypeSafety.lean` — discharged the three-deeper
  nested-recursive `Ms-Equ` leaf
  `StrongCommutesFunFunBodyAppAppSubAppAppAppAppAppEquChainPayload.proved`.
  The proof extends the old-bound equivalence body diamond through six
  ...
  directly, leaving the live three-deeper residuals at changed-head
...
* `Pss/Mpss/DeBruijnTypeSafety.lean` — discharged the three-deeper
  ...
  The proof extends the explicit top-collapse chain through six
  application layers after the joined abstraction bound. The nested-app
  handler now supplies this proof directly, leaving the live three-deeper
  residuals at changed-head `Ms-Pro`, stable-successor `Ms-Pro`, recursive
  `Ms-App`, and `Ms-FOp`. Added the proved endpoint to
  `Pss/DeBruijnSanity.lean`. No axiom-count change.
* `Pss/Mpss/DeBruijnTypeSafety.lean` — discharged the three-deeper
  stable-successor `Ms-Pro` leaf
  `StrongCommutesFunFunBodyAppAppSubAppAppAppAppAppProSuccChainPayload.proved`.
  The proof extends the successor-lookup transport through five structural
  arguments plus the final body argument, rules out an equivalence binding
  at the shared successor head, and rebuilds the transported subtype
  application spine at the joined abstraction bound. The nested-app handler
  now supplies this proof directly, leaving the live three-deeper residuals
  at changed-head `Ms-Pro`, recursive `Ms-App`, and `Ms-FOp`. Added the
  proved endpoint to `Pss/DeBruijnSanity.lean`. No axiom-count change.
* `Pss/Mpss/DeBruijnTypeSafety.lean` — split the remaining three-deeper
  recursive `Ms-App` leaf
  `StrongCommutesFunFunBodyAppAppSubAppAppAppAppAppAppChainPayload` by its
  next subtype-operator constructor, again separating the local `Ms-Pro`
  leaf into changed-head and stable-successor payloads. Added
  `StrongCommutesFunFunBodyAppAppSubAppAppAppAppAppAppChainPayload.of_nested_cases`
  and
  `StrongCommutesFunFunBodyAppAppSubAppAppAppAppAppAppChainPayload.of_nested_cases_pro_split`,
  and rewired the nested-app handler so the four-deeper recursive `Ms-App`
  case is no longer opaque. The live four-deeper residuals are changed-head
  `Ms-Pro`, stable-successor `Ms-Pro`, `Ms-Top`, `Ms-Equ`, recursive
...
* `Pss/Mpss/DeBruijnTypeSafety.lean` — discharged the four-deeper
  nested-recursive `Ms-Top` leaf
  `StrongCommutesFunFunBodyAppAppSubAppAppAppAppAppAppTopChainPayload.proved`.
  The proof extends the explicit top-collapse chain through seven
  application layers and closes the right side with `Ms-Top` after the
  joined abstraction bound. The nested-app handler now supplies this proof
  directly, leaving the live four-deeper residuals at changed-head
...
* `Pss/Mpss/DeBruijnTypeSafety.lean` — discharged the four-deeper
  ...
  proved endpoint to `Pss/DeBruijnSanity.lean`. No axiom-count change.
* `Pss/Mpss/DeBruijnTypeSafety.lean` — split the remaining four-deeper
  recursive `Ms-App` leaf
  `StrongCommutesFunFunBodyAppAppSubAppAppAppAppAppAppAppChainPayload` by
  its next subtype-operator constructor, again separating the local
  `Ms-Pro` leaf into changed-head and stable-successor payloads. Added
  `StrongCommutesFunFunBodyAppAppSubAppAppAppAppAppAppAppChainPayload.of_nested_cases`
  and
  `StrongCommutesFunFunBodyAppAppSubAppAppAppAppAppAppAppChainPayload.of_nested_cases_pro_split`,
  and rewired the nested-app handler so the five-deeper recursive `Ms-App`
  case is no longer opaque. The live five-deeper residuals are changed-head
  `Ms-Pro`, stable-successor `Ms-Pro`, `Ms-Top`, `Ms-Equ`, recursive
  `Ms-App`, and `Ms-FOp`, alongside the remaining shallower changed-head
  `Ms-Pro` and `Ms-FOp` residuals. Added the endpoints to
  `Pss/DeBruijnSanity.lean`. No axiom-count change.
* `Pss/Mpss/DeBruijnTypeSafety.lean` — discharged the five-deeper
  nested-recursive `Ms-Equ` leaf
  `StrongCommutesFunFunBodyAppAppSubAppAppAppAppAppAppAppEquChainPayload.proved`.
  The proof extends the old-bound equivalence body diamond through eight
  application layers and then replaces the old subtype head along the
  joined abstraction bound. The nested-app handler now supplies this proof
  directly, leaving the live five-deeper residuals at changed-head
  `Ms-Pro`, stable-successor `Ms-Pro`, `Ms-Top`, recursive `Ms-App`, and
  `Ms-FOp`. Added the proved endpoint to `Pss/DeBruijnSanity.lean`. No
  axiom-count change.
* `Pss/Mpss/DeBruijnTypeSafety.lean` — discharged the five-deeper
  nested-recursive `Ms-Top` leaf
  `StrongCommutesFunFunBodyAppAppSubAppAppAppAppAppAppAppTopChainPayload.proved`.
  The proof reuses the four-deeper top-collapse proof after absorbing the
  leading argument into the operator, then prefixes the left side by the
  extra `Top`-application collapse. The nested-app handler now supplies
  this proof directly, leaving the live five-deeper residuals at
...
* `Pss/Mpss/DeBruijnTypeSafety.lean` — discharged the five-deeper
  stable-successor nested-recursive `Ms-Pro` leaf
  `StrongCommutesFunFunBodyAppAppSubAppAppAppAppAppAppAppProSuccChainPayload.proved`.
  The proof extends the successor-lookup transport pattern through eight
  application layers, transports every equivalence-rewritten argument
  across the joined abstraction bound, and rebuilds the right-hand subtype
  spine with the preserved successor lookup. The nested-app handler now
  supplies this proof directly, leaving the live five-deeper residuals at
  changed-head `Ms-Pro`, recursive `Ms-App`, and `Ms-FOp`. Added the
  proved endpoint to `Pss/DeBruijnSanity.lean`. No axiom-count change.
* `Pss/Mpss/DeBruijnTypeSafety.lean` — split the remaining five-deeper
  recursive `Ms-App` leaf
  `StrongCommutesFunFunBodyAppAppSubAppAppAppAppAppAppAppAppChainPayload`
  by its next subtype-operator constructor, again separating the local
  `Ms-Pro` leaf into changed-head and stable-successor payloads. Added
  `StrongCommutesFunFunBodyAppAppSubAppAppAppAppAppAppAppAppChainPayload.of_nested_cases`
  and
  `StrongCommutesFunFunBodyAppAppSubAppAppAppAppAppAppAppAppChainPayload.of_nested_cases_pro_split`,
  and rewired the nested-app handler so the six-deeper recursive `Ms-App`
  case is no longer opaque. The live six-deeper residuals are changed-head
  `Ms-Pro`, stable-successor `Ms-Pro`, `Ms-Top`, `Ms-Equ`, recursive
  `Ms-App`, and `Ms-FOp`, alongside the remaining shallower changed-head
  `Ms-Pro` and `Ms-FOp` residuals. Added the endpoints to
  `Pss/DeBruijnSanity.lean`. No axiom-count change.
* `Pss/Mpss/DeBruijnTypeSafety.lean` — discharged the six-deeper
  nested-recursive `Ms-Equ` leaf
  `StrongCommutesFunFunBodyAppAppSubAppAppAppAppAppAppAppAppEquChainPayload.proved`.
  The proof reuses the five-deeper old-bound equivalence proof after
  absorbing the leading argument into the operator. The nested-app handler
  now supplies this proof directly, leaving the live six-deeper residuals
  at changed-head `Ms-Pro`, stable-successor `Ms-Pro`, `Ms-Top`,
  recursive `Ms-App`, and `Ms-FOp`. Added the proved endpoint to
  `Pss/DeBruijnSanity.lean`. No axiom-count change.
* `Pss/Mpss/DeBruijnTypeSafety.lean` — discharged the six-deeper
  nested-recursive `Ms-Top` leaf
  `StrongCommutesFunFunBodyAppAppSubAppAppAppAppAppAppAppAppTopChainPayload.proved`.
  The proof reuses the five-deeper top-collapse proof after absorbing the
  leading argument into the operator, then prefixes the left edge with the
  extra `Top`-application collapse. The nested-app handler now supplies
  this proof directly, leaving the live six-deeper residuals at
  changed-head `Ms-Pro`, stable-successor `Ms-Pro`, recursive `Ms-App`,
  and `Ms-FOp`. Added the proved endpoint to `Pss/DeBruijnSanity.lean`.
  No axiom-count change.
* `Pss/Mpss/DeBruijnTypeSafety.lean` — split the remaining six-deeper
  recursive `Ms-App` leaf by the next subtype-operator constructor. Added
  seven-deeper payload surfaces for constructor-local `Ms-Pro`, changed-head
  `Ms-Pro`, stable-successor `Ms-Pro`, `Ms-Top`, `Ms-Equ`, recursive
  `Ms-App`, and `Ms-FOp`, plus the dispatchers
  `StrongCommutesFunFunBodyAppAppSubAppAppAppAppAppAppAppAppAppChainPayload.of_nested_cases`
  and
  `StrongCommutesFunFunBodyAppAppSubAppAppAppAppAppAppAppAppAppChainPayload.of_nested_cases_pro_split`.
  The top-level nested-app handler now exposes the seven-deeper recursive
  app residual instead of carrying the broad six-deeper `Ms-App` premise.
  Added the endpoints to `Pss/DeBruijnSanity.lean`. No axiom-count change.
* `Pss/Mpss/DeBruijnTypeSafety.lean` — discharged the seven-deeper
  ...
  The proof reuses the six-deeper old-bound equivalence proof after
  absorbing the leading argument into the operator. The nested-app handler
  now supplies this proof directly, leaving the live seven-deeper residuals
  at changed-head `Ms-Pro`, stable-successor `Ms-Pro`, `Ms-Top`,
  recursive `Ms-App`, and `Ms-FOp`. Added the proved endpoint to
  `Pss/DeBruijnSanity.lean`. No axiom-count change.
* `Pss/Mpss/DeBruijnTypeSafety.lean` — discharged the seven-deeper
...
* `Pss/Mpss/DeBruijnTypeSafety.lean` — discharged that eight-deeper
  stable-successor `Ms-Pro` leaf
  `StrongCommutesFunFunBodyAppAppSubAppAppAppAppAppAppAppAppProSuccChainPayload.proved`.
  The proof extends the existing successor-lookup transport by one
  structural application layer, explicitly decomposing the additional
  `Me-App`, retargeting all argument reductions under the joined
  abstraction bound, and rebuilding the final `Ms-Pro` chain. The nested
  handler now supplies this proof directly; the deepest successor
  `Ms-Pro` residual remains live alongside changed-head `Ms-Pro`,
  recursive `Ms-App`, and `Ms-FOp` residuals. Added the proved endpoint
  to `Pss/DeBruijnSanity.lean`. No axiom-count change.
* `Pss/Mpss/DeBruijnTypeSafety.lean` — discharged the deepest exposed
  stable-successor `Ms-Pro` leaf
  `StrongCommutesFunFunBodyAppAppSubAppAppAppAppAppAppAppAppAppProSuccChainPayload.proved`.
  This is the same explicit successor-lookup transport pattern extended
  by the final application layer in the current nested split. The nested
  handler now supplies all stable-successor `Ms-Pro` proofs directly;
...
* `Pss/Mpss/DeBruijnTypeSafety.lean` — split the newly exposed deepest
  ...
  axiom-count change.
* `Pss/Mpss/DeBruijnTypeSafety.lean` — split the one-more recursive
  `Ms-App` residual
  `StrongCommutesFunFunBodyAppAppSubAppAppAppAppAppAppAppAppAppAppAppChainPayload`
  by its subtype-operator constructor and then immediately split its
  `Ms-Pro` leaf into changed-head and stable-successor leaves. Added
  `StrongCommutesFunFunBodyAppAppSubAppAppAppAppAppAppAppAppAppAppAppChainPayload.of_nested_cases_pro_split`
  and rewired the nested handler so this level now consumes the explicit
  constructor-local leaves instead of the opaque one-more `Ms-App`
  premise. This exposes the next deeper recursive `Ms-App` residual and
...
* `Pss/Mpss/DeBruijnTypeSafety.lean` — discharged the newly exposed
  deepest, one-more, and two-more `Ms-Equ` leaves as
  ...
  Each proof reuses the previous old-bound equivalence proof after
  absorbing the leading argument into the operator with `Me-App`. The
  nested handler now supplies these three `Ms-Equ` proofs directly; the
  corresponding live residuals remain changed-head `Ms-Pro`,
  stable-successor `Ms-Pro`, `Ms-Top`, recursive `Ms-App`, and `Ms-FOp`.
  Added the proved endpoints to `Pss/DeBruijnSanity.lean`. No axiom-count
  change.
* `Pss/Mpss/DeBruijnTypeSafety.lean` — discharged the newly exposed
  deepest `Ms-Top` leaf as
  `StrongCommutesFunFunBodyAppAppSubAppAppAppAppAppAppAppAppAppAppTopChainPayload.proved`.
  The proof reuses the previous top-collapse proof after absorbing the
  leading argument into the operator, then prefixes the left side with a
  chain-level `Me-TAp` collapse through the extra application layer. The
  nested handler now supplies this proof directly; the corresponding live
  residuals are changed-head `Ms-Pro`, stable-successor `Ms-Pro`,
  recursive `Ms-App`, and `Ms-FOp`. Added the proved endpoint to
  `Pss/DeBruijnSanity.lean`. No axiom-count change.
* `Pss/Mpss/DeBruijnTypeSafety.lean` — discharged the one-more exposed
  `Ms-Top` leaf as
  `StrongCommutesFunFunBodyAppAppSubAppAppAppAppAppAppAppAppAppAppAppTopChainPayload.proved`.
  The proof reuses the deepest top-collapse proof after absorbing the
  leading argument into the operator, then prefixes the left side with a
  chain-level `Me-TAp` collapse through the extra application layer. The
  nested handler now supplies this proof directly; the corresponding live
  residuals remain changed-head `Ms-Pro`, stable-successor `Ms-Pro`,
  recursive `Ms-App`, and `Ms-FOp`. Added the proved endpoint to
  `Pss/DeBruijnSanity.lean`. No axiom-count change.
* `Pss/Mpss/DeBruijnTypeSafety.lean` — discharged the two-more exposed
  `Ms-Top` leaf as
  `StrongCommutesFunFunBodyAppAppSubAppAppAppAppAppAppAppAppAppAppAppAppTopChainPayload.proved`.
  The proof reuses the one-more top-collapse proof after absorbing the
  leading argument into the operator, then prefixes the left side with a
  chain-level `Me-TAp` collapse through the extra application layer. The
  nested handler now supplies this proof directly; the corresponding live
  residuals remain changed-head `Ms-Pro`, stable-successor `Ms-Pro`,
  recursive `Ms-App`, and `Ms-FOp`. Added the proved endpoint to
  `Pss/DeBruijnSanity.lean`. No axiom-count change.
* `Pss/Mpss/DeBruijnTypeSafety.lean` — exposed and discharged the
  three-more `Ms-Top` leaf as
  `StrongCommutesFunFunBodyAppAppSubAppAppAppAppAppAppAppAppAppAppAppAppAppTopChainPayload.proved`.
  The proof reuses the two-more top-collapse proof after absorbing the
  leading argument into the operator, then prefixes the left side with a
  chain-level `Me-TAp` collapse through the extra application layer. This
  endpoint is ready for the next recursive `Ms-App` splitter; live
  residuals remain changed-head `Ms-Pro`, stable-successor `Ms-Pro`,
  recursive `Ms-App`, and `Ms-FOp`. Added the proved endpoint to
  `Pss/DeBruijnSanity.lean`. No axiom-count change.
* `Pss/Mpss/DeBruijnTypeSafety.lean` — split the three-more recursive
  `Ms-App` leaf into constructor-local `Ms-Pro`, `Ms-Top`, `Ms-Equ`,
  recursive `Ms-App`, and `Ms-FOp` payloads via
  `StrongCommutesFunFunBodyAppAppSubAppAppAppAppAppAppAppAppAppAppAppAppAppChainPayload.of_nested_cases`.
  The nested handler now supplies the already-proved three-more `Ms-Top`
...
* `Pss/Mpss/DeBruijnTypeSafety.lean` — added
  ...
  endpoint to `Pss/DeBruijnSanity.lean`. No axiom-count change.
* `Pss/Mpss/DeBruijnTypeSafety.lean` — rewired the next two
  stable-successor `Ms-Pro` application-spine leaves,
  `StrongCommutesFunFunBodyAppAppSubAppProSuccChainPayload.proved` and
  `StrongCommutesFunFunBodyAppAppSubAppAppProSuccChainPayload.proved`,
  through `stableSuccProAppSpineJoin`. This removes the hand-built
  two- and three-argument successor-spine joins and confirms the helper
  scales beyond the shallow one-argument case. No axiom-count change.
...
* `Pss/Mpss/DeBruijnTypeSafety.lean` — rewired
  `StrongCommutesFunFunBodyAppAppSubAppAppAppAppAppAppProSuccChainPayload.proved`
  through `stableSuccProAppSpineJoin`, removing the hand-built
  seven-argument successor-spine join. No axiom-count change.
* `Pss/Mpss/DeBruijnTypeSafety.lean` — rewired
  `StrongCommutesFunFunBodyAppAppSubAppAppAppAppAppAppAppProSuccChainPayload.proved`
  through `stableSuccProAppSpineJoin`, removing the hand-built
  eight-argument successor-spine join. No axiom-count change.
* `Pss/Mpss/DeBruijnTypeSafety.lean` — rewired
  `StrongCommutesFunFunBodyAppAppSubAppAppAppAppAppAppAppAppProSuccChainPayload.proved`
  through `stableSuccProAppSpineJoin`, removing the hand-built
  nine-argument successor-spine join. No axiom-count change.
* `Pss/Mpss/DeBruijnTypeSafety.lean` — added
  ...
  `StrongCommutes_proved_of_split_chain_fun_app_sub_cases_nested_app_handlers`,
  removing one more explicit stable-successor residual parameter from the
  top-level Lemma 1 assembly. No axiom-count change.
* `Pss/Mpss/DeBruijnTypeSafety.lean` — added
  `StrongCommutesFunFunBodyAppAppSubAppAppAppAppAppAppAppAppAppAppAppProSuccChainPayload.proved`
  through `stableSuccProAppSpineJoin` and wired it into
  `StrongCommutes_proved_of_split_chain_fun_app_sub_cases_nested_app_handlers`,
  removing the next explicit stable-successor residual parameter from the
  top-level Lemma 1 assembly. No axiom-count change.
* `Pss/Mpss/DeBruijnTypeSafety.lean` — added
  `StrongCommutesFunFunBodyAppAppSubAppAppAppAppAppAppAppAppAppAppAppAppProSuccChainPayload.proved`
  through `stableSuccProAppSpineJoin` and wired it into
  `StrongCommutes_proved_of_split_chain_fun_app_sub_cases_nested_app_handlers`,
...
* `Pss/Mpss/DeBruijnTypeSafety.lean` — added
  ...
  the two-argument nested changed-head `Ms-Pro` app/app leaf, again using
  `changedHeadProAppSpineJoin` after splitting the right `Me-App`
  operator step through two applications. Wired the first three
  changed-head leaves in
  `StrongCommutes_proved_of_split_chain_fun_app_sub_cases_nested_app_handlers`
  through `MEqRedStarStackAppendPayload`, replacing three explicit
  changed-head residual parameters with the single stack-append residual.
  No axiom-count change.
* `Pss/Mpss/DeBruijnTypeSafety.lean` — added
  `StrongCommutesFunFunBodyAppAppSubAppAppAppProHeadChainPayload.proved_of_stack_append`,
  the three-argument nested changed-head `Ms-Pro` app/app leaf, using
  `changedHeadProAppSpineJoin` after splitting the right `Me-App`
  operator step through three applications. Wired it into
  `StrongCommutes_proved_of_split_chain_fun_app_sub_cases_nested_app_handlers`,
  removing one more explicit changed-head residual parameter in favor of
  `MEqRedStarStackAppendPayload`. No axiom-count change.
* `Pss/Mpss/DeBruijnTypeSafety.lean` — added
  `StrongCommutesFunFunBodyAppAppSubAppAppAppAppProHeadChainPayload.proved_of_stack_append`,
  the four-argument nested changed-head `Ms-Pro` app/app leaf, using
  `changedHeadProAppSpineJoin` after splitting the right `Me-App`
  operator step through four applications. Wired it into
  `StrongCommutes_proved_of_split_chain_fun_app_sub_cases_nested_app_handlers`,
  removing the next explicit changed-head residual parameter in favor of
  `MEqRedStarStackAppendPayload`. No axiom-count change.
* `Pss/Mpss/DeBruijnTypeSafety.lean` — added
  `StrongCommutesFunFunBodyAppAppSubAppAppAppAppAppProHeadChainPayload.proved_of_stack_append`,
  the five-argument nested changed-head `Ms-Pro` app/app leaf, using
  `changedHeadProAppSpineJoin` after splitting the right `Me-App`
  operator step through five applications. Wired it into
  `StrongCommutes_proved_of_split_chain_fun_app_sub_cases_nested_app_handlers`,
  replacing another explicit changed-head residual parameter with
  `MEqRedStarStackAppendPayload`. No axiom-count change.
* `Pss/Mpss/DeBruijnTypeSafety.lean` — added
  `StrongCommutesFunFunBodyAppAppSubAppAppAppAppAppAppProHeadChainPayload.proved_of_stack_append`,
  the six-argument nested changed-head `Ms-Pro` app/app leaf, using
  `changedHeadProAppSpineJoin` after splitting the right `Me-App`
  operator step through six applications. Wired it into
  `StrongCommutes_proved_of_split_chain_fun_app_sub_cases_nested_app_handlers`,
  replacing another explicit changed-head residual parameter with
  `MEqRedStarStackAppendPayload`. No axiom-count change.
* `Pss/Mpss/DeBruijnTypeSafety.lean` — added
  `StrongCommutesFunFunBodyAppAppSubAppAppAppAppAppAppAppProHeadChainPayload.proved_of_stack_append`,
  the seven-argument nested changed-head `Ms-Pro` app/app leaf, using
  `changedHeadProAppSpineJoin` after splitting the right `Me-App`
  operator step through seven applications. Wired it into
  `StrongCommutes_proved_of_split_chain_fun_app_sub_cases_nested_app_handlers`,
  replacing another explicit changed-head residual parameter with
  `MEqRedStarStackAppendPayload`. No axiom-count change.
* `Pss/Mpss/DeBruijnTypeSafety.lean` — added
  `StrongCommutesFunFunBodyAppAppSubAppAppAppAppAppAppAppAppProHeadChainPayload.proved_of_stack_append`,
  the eight-argument nested changed-head `Ms-Pro` app/app leaf, using
  `changedHeadProAppSpineJoin` after splitting the right `Me-App`
  operator step through eight applications. Wired it into
  `StrongCommutes_proved_of_split_chain_fun_app_sub_cases_nested_app_handlers`,
  replacing another explicit changed-head residual parameter with
  `MEqRedStarStackAppendPayload`. No axiom-count change.
* `Pss/Mpss/DeBruijnTypeSafety.lean` — added
  `StrongCommutesFunFunBodyAppAppSubAppAppAppAppAppAppAppAppAppProHeadChainPayload.proved_of_stack_append`,
  the nine-argument nested changed-head `Ms-Pro` app/app leaf, using
  `changedHeadProAppSpineJoin` after splitting the right `Me-App`
  operator step through nine applications. Wired it into
  `StrongCommutes_proved_of_split_chain_fun_app_sub_cases_nested_app_handlers`,
  replacing another explicit changed-head residual parameter with
  `MEqRedStarStackAppendPayload`. No axiom-count change.
* `Pss/Mpss/DeBruijnTypeSafety.lean` — added
  `StrongCommutesFunFunBodyAppAppSubAppAppAppAppAppAppAppAppAppAppProHeadChainPayload.proved_of_stack_append`,
  the ten-argument nested changed-head `Ms-Pro` app/app leaf, using
  `changedHeadProAppSpineJoin` after splitting the right `Me-App`
  operator step through ten applications. Wired it into
  `StrongCommutes_proved_of_split_chain_fun_app_sub_cases_nested_app_handlers`,
  replacing another explicit changed-head residual parameter with
  `MEqRedStarStackAppendPayload`. No axiom-count change.
* `Pss/Mpss/DeBruijnTypeSafety.lean` — added
  `StrongCommutesFunFunBodyAppAppSubAppAppAppAppAppAppAppAppAppAppAppProHeadChainPayload.proved_of_stack_append`,
  the eleven-argument nested changed-head `Ms-Pro` app/app leaf, using
  `changedHeadProAppSpineJoin` after splitting the right `Me-App`
  operator step through eleven applications. Wired it into
  `StrongCommutes_proved_of_split_chain_fun_app_sub_cases_nested_app_handlers`,
  replacing another explicit changed-head residual parameter with
  `MEqRedStarStackAppendPayload`. No axiom-count change.
* `Pss/Mpss/DeBruijnTypeSafety.lean` — added
  `StrongCommutesFunFunBodyAppAppSubAppAppAppAppAppAppAppAppAppAppAppAppProHeadChainPayload.proved_of_stack_append`,
  the twelve-argument nested changed-head `Ms-Pro` app/app leaf, using
  `changedHeadProAppSpineJoin` after splitting the right `Me-App`
  operator step through twelve applications. Wired it into
  `StrongCommutes_proved_of_split_chain_fun_app_sub_cases_nested_app_handlers`,
  replacing another explicit changed-head residual parameter with
  `MEqRedStarStackAppendPayload`. No axiom-count change.
* `Pss/Mpss/DeBruijnTypeSafety.lean` — added
