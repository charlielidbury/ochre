...
> **Status of this document.** Self-contained apart from references to Hutchins, *Pure Subtype Systems* (POPL 2010); it supersedes `03-revised-proof.md`, whose §6 DS-APP case and §4 proof of Lemma 4.5 contain quantifier-budget errors (found during verification of that document, and analyzed here). Everything stated below as *proven* has a complete proof, reproduced or sketched in place; the document's purpose is to present the one remaining obstruction — **the depth-transport problem**, §8 — precisely enough to be attacked, branched on, or refuted. §10 lists the concrete questions.
...
| Member conversion (Lemma 4.5 of the predecessor document) | **Open** — reduced to Problem D1 (§4.4, §8); consumed by *nothing* downstream |
...
| Soundness of DS-APP(≤) | **Open** — Problem D2 (§8); conjectured true |
...
Both open items are instances of a single phenomenon: the model budgets the *member's* computation and λ-depth but never the *type's*, and certain proofs need a value's self-description (its MATCH data) at depths the diagonal budget cannot reach. We call this the **depth-transport problem**. Both instances are conjectured true; the evidence and the failed attacks are catalogued in §8.
...
So any true version of member conversion must use the *context* in which the transport is demanded — the goodness of `c` and the ∀-index MATCH of the source value — essentially, not as index bookkeeping. Everything in the broken proof *except* tier (t1) is sound; the lemma therefore reduces to:

**Problem D1 (Tier-Transport).** *Let `q =β β′`, suppose `MATCH_{n+1}(λx≤α′.β′, λx≤a.b)` for every `n`, and let `c ∈ ⟨a⟩_{j₁}`. Show `[x↦c]q ∈ ⟦[x↦c]b⟧_{j₁}`.*

Tier-Transport implies the full member-conversion lemma, with no residual index induction ((t1) was the proof's only recursive step; the bound clause and (t2) transport unconditionally, (t2) by 4.4's set equality composed with the supplied tier's inclusion at exactly `j₁`). Conversely it is an instance of it. Discussion in §8. **Nothing downstream consumes member conversion**: its single intended use (§6, DS-APP step (d)) is derivable without it, as shown in §7.
...
**Problem D1 (Tier-Transport — restated from §4.4).** `q =β β′`, `MATCH_{n+1}(λx≤α′.β′, λx≤a.b)` for every `n`, `c ∈ ⟨a⟩_{j₁}` ⟹ `[x↦c]q ∈ ⟦[x↦c]b⟧_{j₁}`.

**Problem D2 (soundness of DS-APP(≤)).** Under the premises `Γ ⊨ t ≤ t′`, `Γ ⊨ u ≡ u′`, and the four carried instrumentations: `Γ ⊨ t(u) ≤ t′(u′)` — specifically the inclusion half `⟦T(U)⟧ₖ ⊆ ⟦T′(U′)⟧ₖ` at each `(k, γ)`.
...
**Why truth is conjectured — the countermodel constraint system.** Extensive search for a falsifying configuration fails in a *patterned* way; the obstructions are themselves informative:
...
**The desideratum.** What neither direction possesses is a quantitative lemma linking *goodness depth*, *evaluation length*, and *value self-description* — a **cap anatomy**: e.g., a convergent `c ∈ ⟨a⟩_K` has its defects (observations separating it from `⟦a⟧`-membership) only at observation-depth-plus-steps `> K`, so that defects invisible in budget are unreachable along any converging evaluation. Such a lemma would (i) discharge D1 directly, (ii) break the self-MATCH wall of D2, and (iii) explain *why* the four obstructions are exhaustive. A promising but unverified angle on D1: its self-similarity suggests an induction on the lexicographic measure (goal index, remaining member steps), where the type-side tier re-entry rides the ∀-index MATCH's own (t2) inclusions (whose supply is index-free) rather than fresh goodness; the open question is whether the measure genuinely decreases at the re-entry.
...
2. **(Cap anatomy.)** Prove or refute: a convergent `c ∈ ⟨a⟩_K` has all its defects at observation-depth-plus-steps `> K` ("in-budget-invisible defects are unreachable along converging evaluations"). Either resolution settles D1 and likely D2.
3. **(D1 measure.)** Does the self-similar unfolding of Tier-Transport decrease under (goal index, remaining member steps), with type-side re-entry riding the ∀-MATCH's own (t2) inclusions? Identify the exact re-entry point and what it consumes.
4. **(Model variant.)** Is the type-side-budgeted MATCH (`MATCH_{k−j−j_T}`) viable — do 4.1–4.4 and W-APP survive it?
5. **(Countermodel.)** Exhibit a hypothesis-consistent configuration realizing the junk-tower arithmetic (§8) syntactically — i.e., evade all four obstructions simultaneously — or prove the obstructions exhaustive (which is Question 2 in disguise).

A resolution of D2 in either direction completes the program: *true* gives unconditional type safety for λ⊲ with transitivity elimination retired; *false* identifies the precise model repair the approach always needed.
