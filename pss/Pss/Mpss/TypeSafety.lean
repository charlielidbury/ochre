...
/-! ## §8. WfM preservation under `MEqRed`

This section discharges the obstruction that blocks the mechanization of
`Lemma_10_Inversion` (see `Pss/Mpss/WellFormed.lean` §7 audit notes):
producing `WfM Γ t_w` along an `MEqRedStar Γ [] t t_w` chain extracted via
the `_WSubMStar_toMSub` strip.

The lemma is a parallel of `Lemma_6_EvaluationPreservesWf` but for
`MEqRed` (instead of `Step`). Conditional on the existing paper axioms
`Lemma_10_Inversion` (β arm) and `Conjecture_8_WellSubtypingContextIndependent`
(`Me-App` inner-stack arm). No NEW axioms are introduced.

A `WfCtx Γ` premise tracks well-formedness of `≡`-bound entries
(needed for the `Me-Pro` arm, which extracts the bound term's WfM
to feed the IH). -/

/-- A context is **WfM-context-well-formed** iff every `≡`-bound entry has
a well-formed bound term in the full context. (Sub-bound entries only
require `Prevalid`-level scope/LC, already carried by `Prevalid Γ`.) -/
def WfCtx (Γ : Ctx) : Prop :=
  ∀ x α, Γ.equBinds x α → WfM Γ α

namespace WfCtx

/-- Empty context: vacuous. -/
theorem nil_wfctx : WfCtx [] := by
  intro x α hb
  simp [Ctx.equBinds, Ctx.lookupEqu] at hb

/-- Extending with a `.sub`-binding preserves `WfCtx`. The new binding
is `.sub` so it doesn't add a new `≡`-bound entry; existing entries are
weakened via `WfM.weaken_append`. -/
theorem cons_sub {Γ : Ctx} {x : String} {t : Term}
    (hwfctx : WfCtx Γ) (hpv' : Prevalid (⟨x, t, .sub⟩ :: Γ)) :
    WfCtx (⟨x, t, .sub⟩ :: Γ) := by
  intro y α hb
  -- equBinds (⟨x, t, .sub⟩ :: Γ) y α: head is .sub so lookupEqu skips
  -- the head, falling through to lookupEqu Γ y = some α.
  have hb_in : Γ.equBinds y α := by
    simp [Ctx.equBinds, Ctx.lookupEqu] at hb
    exact hb.2
  have hwfα : WfM Γ α := hwfctx y α hb_in
  exact WfM.weaken_append (Δ := [⟨x, t, .sub⟩]) hwfα (by simpa using hpv')
...
* `Me-Pro` (variable promotion under `≡`-binding): WfCtx supplies the
  bound term's WfM; recursion handles the inner reduction.
* `Me-Top`, `Me-Var`, `Me-TAp`: trivial (target is `.top` or `.fvar y`).
* `Me-Fun` (descend under unapplied abstraction): IH-body in the .sub-extended
  context, then `Lemma_23_NarrowingWf` to update the bound annotation.

**Obstructed cases (3 of 8) — flagged with structural notes below:**

* `Me-Bet` (β-step): Lemma 10's β-substitution chain is constructible
  (Lemma 10 + 15 + 16 + Lemma 7 with `α = v0`), giving
  `WfM Γ (Term.opening v0 body)`. But the conclusion is
  `WfM Γ (Term.opening v0' body')`, which requires preservation of
  `WfM` along the body's inner MEqRed. The body MEqRed is at `Γ` (with
  `body^[x]` containing `x ∉ Γ.dom` as an open name); weakening it to
  `(⟨x, tBound, .sub⟩ :: Γ)` to apply IH at the extended context fails
  because `Lemma 22` requires `fv body^[x] ⊆ Γ.dom` which doesn't hold
  for `x ∈ fv body^[x]`.
* `Me-App` (application congruence): inner u-MEqRed is at stack `v :: s`
  (non-empty); IH gives `WfM Γ u'`, but `WSubMStar Γ u' (.abs t .top)`
  requires `MEqRed Γ [] u u'` (empty stack), which is NOT in general
  derivable from `MEqRed Γ (v :: s) u u'` (Me-FOp fires only at non-empty
  stack).
* `Me-FOp` (descend under applied abstraction by popping stack): body
  MEqRed is in `(⟨x, α, .equ⟩ :: Γ)`, but Wf-Fun inversion gives WfM in
  `(⟨x, tBound, .sub⟩ :: Γ)`. No `.sub`-to-`.equ` "kind narrowing" lemma
  exists, and `Lemma 23` only handles `.sub`-to-`.sub` narrowing.

The remaining cases are flagged TODO and require either:
(a) a stack-conversion lemma `MEqRed Γ (v :: s) u u' → MEqRed Γ [] u u'`
    under suitable side conditions (likely false in general),
(b) a `.sub`/`.equ` kind-narrowing lemma for `WfM`,
(c) restructuring the lemma's premise to track per-subterm `WfM`-style
    reduction structure (rather than just top-level WfM),
(d) or a measure-based recursion that operates on substituted terms
    (e.g. `Term.opening v0 body`) rather than the open `body^[x]`.

Conditional on `Lemma_10_Inversion` (existing paper axiom) for the β arm. -/
theorem WfM.preservation_MEqRed
    ...
    WfM Γ t' := by
  induction h with
  | @pro Γ s y α α' hpv hb hα ihα =>
    -- t = .fvar y, t' = α'. Use WfCtx to get WfM Γ α; recurse.
    have hwfα : WfM Γ α := hwfctx y α hb
    exact ihα hwfctx hwfα
  | @top Γ s hpv =>
    -- t = .top, t' = .top. Trivial.
    exact hwf
  | @var Γ s y hpv =>
    -- t = .fvar y, t' = .fvar y. Trivial.
    exact hwf
  | @tAp Γ s u hpv hLCu hfvU =>
  ...
  | @fun_ Γ tBound tBound' body body' L ht hbody iht ihbody =>
    ...
    cases hwf with
    | @fun_ _ _ _ L₀ hwfT hwfBody =>
      -- IH on bound annotation: WfM Γ tBound' from WfM Γ tBound.
      have hwfT' : WfM Γ tBound' := iht hwfctx hwfT
      -- Build cofinite WfM (⟨x, tBound', .sub⟩ :: Γ) (body'^[x]).
      have hLCT : Term.LC tBound := WfM.lc hwfT
      have hLCT' : Term.LC tBound' := WfM.lc hwfT'
      have hfvT : Term.fv tBound ⊆ Γ.dom := WfM.fv_subset hwfT
      have hfvT' : Term.fv tBound' ⊆ Γ.dom := WfM.fv_subset hwfT'
      have hpvΓ : Prevalid Γ := prevalid_of_wfM hwfT
      classical
      refine WfM.fun_ (L ∪ L₀ ∪ Γ.dom) hwfT' ?_
      intro x hx
      have hxL : x ∉ L := fun hh => hx (by
        apply Finset.mem_union.mpr; left
        apply Finset.mem_union.mpr; left; exact hh)
      have hxL₀ : x ∉ L₀ := fun hh => hx (by
        apply Finset.mem_union.mpr; left
        apply Finset.mem_union.mpr; right; exact hh)
      have hxΓ : x ∉ Γ.dom := fun hh => hx (by
        apply Finset.mem_union.mpr; right; exact hh)
      -- Step 1: WfM (⟨x, tBound, .sub⟩ :: Γ) (body^[x]).
      have hwfBody_x : WfM (⟨x, tBound, .sub⟩ :: Γ) (Term.opening (.fvar x) body) :=
        hwfBody x hxL₀
      -- Step 2: Apply IH-body in extended (.sub) context.
      have hpvExt : Prevalid (⟨x, tBound, .sub⟩ :: Γ) :=
        Prevalid.sub hpvΓ hxΓ hfvT hLCT
      have hwfctxExt : WfCtx (⟨x, tBound, .sub⟩ :: Γ) :=
        WfCtx.cons_sub hwfctx hpvExt
      have hwfBody'_x : WfM (⟨x, tBound, .sub⟩ :: Γ) (Term.opening (.fvar x) body') :=
        ihbody x hxL hwfctxExt hwfBody_x
      -- Step 3: Narrow from tBound to tBound' via Lemma 23.
...
theorem WfM.preservation_MEqRedStar
  ...
  | tail _ hStep ih =>
    obtain ⟨hStep'⟩ := hStep
    exact WfM.preservation_MEqRed hwfctx ih hStep'
