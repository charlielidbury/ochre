...
/-- Final assembly for the changing-bound/changing-body `Fun` abstraction
commutation cell. The recursive obligation is isolated in `hBodyJoin`: after
choosing a bound join, the left body target moves by equivalence and the
right body target moves by subtype under the joined `.sub` head. -/
theorem commute_abs_fun_targets_of_bound_body_joins_from_left {Γ : Ctx}
    {bound body bound₁ body₁ bound₂ body₂ : Term}
    (hLeft : MSubRed Γ [] (.abs bound body) (.abs bound₁ body₁))
    (hRight : MEqRed Γ [] (.abs bound body) (.abs bound₂ body₂))
    (hBoundJoin :
    ...
    (hBodyJoin :
      ∀ {bound₃ : Term},
        ...
        ∃ body₃,
          MEqRedStar ({ bound := bound₃, kind := .sub } :: Γ) [] body₁ body₃ ∧
            MSubRedStar ({ bound := bound₃, kind := .sub } :: Γ) [] body₂ body₃) :
    ∃ t₃,
      MEqRedStar Γ [] (.abs bound₁ body₁) t₃ ∧
        MSubRedStar Γ [] (.abs bound₂ body₂) t₃ := by
  obtain ⟨bound₃, hBound₁₃, hBound₂₃⟩ := hBoundJoin
  ...
  exact ⟨.abs bound₃ body₃,
    MEqRedStar.trans
      ...
      (meqRedStar_abs_fun_body_fixed_bound hLeft.prevalidExt hBound₃Scoped
        hBody₁₃),
    MSubRedStar.trans
      (msubRedStar_abs_fun_bound_fixed_body hRight.prevalidExt hBody₂Scoped
        (MEqRedStar.single hBound₂₃.some))
...
The `Ms-Top` and `Ms-Equ` body branches close by the dedicated helpers above;
the remaining body constructors are exposed as handlers because they require
recursive commutation/replacement infrastructure. -/
theorem commute_abs_fun_fun_body_from_handlers_of {Γ : Ctx}
    {bound body bound₁ body₁ bound₂ body₂ : Term}
    (hpvNil : PrevalidExt Γ [])
    (hdiamondBound : EqDiamonds Γ [])
    (hdiamondBody : EqDiamonds ({ bound := bound, kind := .sub } :: Γ) [])
    ...
    (hProVar :
      ∀ {i : Nat} {t : Term},
        Ctx.subBinds ({ bound := bound, kind := .sub } :: Γ) i t →
        ∃ t₃,
...
/-- Body-constructor dispatcher for the `Ms-Fun × Me-Fun` commutation cell
with the structural `Ms-App × Me-App` body branch wired through
`commute_abs_fun_fun_app_body_app_from_operator_join_of`. The remaining
explicit handlers are beta `Ms-App × Me-Bet` and nested `Ms-Fun`; the
structural app branch is reduced to the precise operator replacement and
changed-argument transport obligations. -/
theorem commute_abs_fun_fun_body_from_operator_join_app_cases_fun_handlers_of
    ...
    ∃ t₃,
      MEqRedStar Γ [] (.abs bound₁ body₁) t₃ ∧
        MSubRedStar Γ [] (.abs bound₂ body₂) t₃ :=
  commute_abs_fun_fun_body_from_app_cases_fun_handlers_of hpvNil hdiamondBound
    hdiamondBody hSubBound hSubBody hEqBound hEqBody
    (fun hSubOp hArgScoped hEqOp hEqArg =>
      commute_abs_fun_fun_app_body_app_from_operator_join_of hpvNil hdiamondBound
        hSubBound hSubOp hArgScoped hEqBound hEqOp hEqArg
        (fun {_bound₃} hBound₁₃ hBound₂₃ =>
          hAppAppComm hSubOp hArgScoped hEqOp hEqArg hBound₁₃ hBound₂₃)
        (fun {_bound₃} hBound₁₃ hBound₂₃ =>
          hAppAppSubReplace hSubOp hArgScoped hEqOp hEqArg hBound₁₃ hBound₂₃)
        (fun {_bound₃} hBound₁₃ hBound₂₃ =>
          hAppAppEqOpReplace hSubOp hArgScoped hEqOp hEqArg hBound₁₃ hBound₂₃)
        (fun {_bound₃} hBound₁₃ hBound₂₃ =>
          hAppAppEqArgReplace hSubOp hArgScoped hEqOp hEqArg hBound₁₃ hBound₂₃)
        (fun {_bound₃} hBound₁₃ hBound₂₃ =>
          hAppAppTransport hSubOp hArgScoped hEqOp hEqArg hBound₁₃ hBound₂₃))
    hAppBet hFun

/-- Dispatcher variant that also internalizes the equivalence-premise
replacement in the structural `Ms-App × Me-App` body branch. The original
body equivalence premises are moved to the joined abstraction bound by
`MEqRed.sub_head_replace_two_step`, via the right branch bound. The remaining
structural app obligations are operator strong commutativity, subtype-premise
replacement, and changed-argument stack-head transport. -/
theorem commute_abs_fun_fun_body_from_operator_join_app_cases_eq_replaced_of
    {Γ : Ctx} {bound body bound₁ body₁ bound₂ body₂ : Term}
    (hpvNil : PrevalidExt Γ [])
    (hdiamondBound : EqDiamonds Γ [])
    (hdiamondBody : EqDiamonds ({ bound := bound, kind := .sub } :: Γ) [])
    ...
    ∃ t₃,
      MEqRedStar Γ [] (.abs bound₁ body₁) t₃ ∧
        MSubRedStar Γ [] (.abs bound₂ body₂) t₃ := by
  refine
    commute_abs_fun_fun_body_from_operator_join_app_cases_fun_handlers_of hpvNil
      hdiamondBound hdiamondBody hSubBound hSubBody hEqBound hEqBody
      hAppAppComm hAppAppSubReplace ?_ ?_ hAppAppTransport hAppBet hFun
  · intro u u' v u₂ v₂ _hSubOp _hArgScoped hEqOp _hEqArg bound₃ _hBound₁₃ hBound₂₃
    exact hEqOp.sub_head_replace_two_step hEqBound hBound₂₃.some
...
/-- Dispatcher variant that splits the remaining one-step changed-argument
stack-head transport in the structural `Ms-App × Me-App` body branch by the
subtype constructor. Stable `Ms-Pro`/`Ms-Top` leaves are rebuilt internally;
the remaining explicit handlers are the equivalence-origin, nested app, and
`FOp` cases. -/
theorem commute_abs_fun_fun_body_from_operator_join_app_cases_stack_handlers_of
    {Γ : Ctx} {bound body bound₁ body₁ bound₂ body₂ : Term}
    (hpvNil : PrevalidExt Γ [])
    (hdiamondBound : EqDiamonds Γ [])
    (hdiamondBody : EqDiamonds ({ bound := bound, kind := .sub } :: Γ) [])
    ...
    (hAppAppStepEqu :
      ∀ {u u' v u₂ v₂ : Term},
        ...
        ∀ {bound₃ : Term},
          ...
          MEqRedJ Γ [] bound₂ bound₃ →
          ∀ {a b : Term},
            MEqRed ({ bound := bound₃, kind := .sub } :: Γ) (v :: []) a b →
            MSubRedStar ({ bound := bound₃, kind := .sub } :: Γ) (v₂ :: []) a b)
    (hAppAppStepApp :
      ∀ {u u' v u₂ v₂ : Term},
        MSubRed ({ bound := bound, kind := .sub } :: Γ) (v :: []) u u' →
        ...
        ∀ {bound₃ : Term},
          ...
          ∀ {op op' arg : Term},
            ...
            MSubRedStar ({ bound := bound₃, kind := .sub } :: Γ) (v₂ :: [])
        ...
        ∀ {bound₃ : Term},
          ...
          ∀ {funBound body body' : Term},
            ...
            MSubRed ({ bound := v, kind := .equ } ::
              ...
              body body' →
            MSubRedStar ({ bound := bound₃, kind := .sub } :: Γ) (v₂ :: [])
              (.abs funBound body) (.abs funBound body'))
    (hAppBet :
      ∀ {t v v' inner inner' u' : Term},
        Term.Scoped (Ctx.depth ({ bound := bound, kind := .sub } :: Γ)) t →
        MSubRed ({ bound := bound, kind := .sub } :: Γ) (v :: []) (.abs t inner) u' →
  ...
  have hpvNew : PrevalidExt Γ₃ (v₂ :: []) :=
    PrevalidExt.push (PrevalidExt.nil (Prevalid.sub (PrevalidExt.ctx hpvNil)
      hBound₂₃.some.scoped_right)) hEqArg₃.scoped_right
  exact MSubRed.stack_head_replace_from_handlers hpvNew
    (fun hEq =>
      hAppAppStepEqu hSubOp hArgScoped hEqOp hEqArg hBound₁₃ hBound₂₃ hEq)
    (fun hOp hArg =>
      hAppAppStepApp hSubOp hArgScoped hEqOp hEqArg hBound₁₃ hBound₂₃ hOp hArg)
    (fun hBound hOldScoped hBody =>
      hAppAppStepFOp hSubOp hArgScoped hEqOp hEqArg hBound₁₃ hBound₂₃
        hBound hOldScoped hBody)
    hStep

/-- Dispatcher variant that also splits the equivalence-origin changed-stack
handler in the structural `Ms-App × Me-App` body branch. Stable
`Me-Top`/`Me-Var`/`Me-TAp` leaves are rebuilt internally; the remaining
equivalence-origin premises are exactly the recursive `Me-Pro`, `Me-App`,
`Me-Bet`, and `Me-FOp` cases. -/
theorem commute_abs_fun_fun_body_from_operator_join_app_cases_equ_stack_handlers_of
    {Γ : Ctx} {bound body bound₁ body₁ bound₂ body₂ : Term}
    (hpvNil : PrevalidExt Γ [])
    (hdiamondBound : EqDiamonds Γ [])
    (hdiamondBody : EqDiamonds ({ bound := bound, kind := .sub } :: Γ) [])
    ...
    (hAppAppStepEquPro :
      ∀ {u u' v u₂ v₂ : Term},
        ...
        ∀ {bound₃ : Term},
          ...
          MEqRedJ Γ [] bound₂ bound₃ →
          ∀ {i : Nat} {α α' : Term},
            Ctx.equBinds ({ bound := bound₃, kind := .sub } :: Γ) i α →
    ...
    (hAppAppStepEquBet :
      ∀ {u u' v u₂ v₂ : Term},
        ...
        ∀ {bound₃ : Term},
          MEqRedJ Γ [] bound₁ bound₃ →
          MEqRedJ Γ [] bound₂ bound₃ →
          ∀ {funBound arg arg' inner inner' : Term},
            Term.Scoped (Ctx.depth ({ bound := bound₃, kind := .sub } :: Γ))
              funBound →
            ...
            MEqRed ({ bound := bound₃, kind := .sub } :: Γ) [] arg arg' →
            MSubRedStar ({ bound := bound₃, kind := .sub } :: Γ) (v₂ :: [])
              (.app (.abs funBound inner) arg) (Term.instantiate 0 arg' inner'))
    (hAppAppStepEquFOp :
      ∀ {u u' v u₂ v₂ : Term},
        MSubRed ({ bound := bound, kind := .sub } :: Γ) (v :: []) u u' →
        Term.Scoped (Ctx.depth ({ bound := bound, kind := .sub } :: Γ)) v →
        ...
        ∀ {bound₃ : Term},
          ...
          ∀ {funBound funBound' inner inner' : Term},
            ...
            MEqRed ({ bound := v, kind := .equ } ::
              ...
              inner inner' →
            MSubRedStar ({ bound := bound₃, kind := .sub } :: Γ) (v₂ :: [])
              (.abs funBound inner) (.abs funBound' inner'))
    (hAppAppStepApp :
      ∀ {u u' v u₂ v₂ : Term},
        MSubRed ({ bound := bound, kind := .sub } :: Γ) (v :: []) u u' →
        Term.Scoped (Ctx.depth ({ bound := bound, kind := .sub } :: Γ)) v →
  ...
  refine
    commute_abs_fun_fun_body_from_operator_join_app_cases_stack_handlers_of
      hpvNil hdiamondBound hdiamondBody hSubBound hSubBody hEqBound hEqBody
      hAppAppComm hAppAppSubReplace ?_ hAppAppStepApp hAppAppStepFOp
      hAppBet hFun
  intro u u' v u₂ v₂ hSubOp hArgScoped hEqOp hEqArg bound₃ hBound₁₃ hBound₂₃
    a b hEqStep
  let Γ₃ : Ctx := { bound := bound₃, kind := .sub } :: Γ
  ...
  have hpvNew : PrevalidExt Γ₃ (v₂ :: []) :=
    PrevalidExt.push (PrevalidExt.nil (Prevalid.sub (PrevalidExt.ctx hpvNil)
      hBound₂₃.some.scoped_right)) hEqArg₃.scoped_right
  exact MEqRed.stack_head_subtype_replace_from_handlers hpvNew
    (fun hb hα =>
      hAppAppStepEquPro hSubOp hArgScoped hEqOp hEqArg hBound₁₃ hBound₂₃ hb hα)
    (fun hOp hArg =>
      hAppAppStepEquApp hSubOp hArgScoped hEqOp hEqArg hBound₁₃ hBound₂₃ hOp hArg)
    (fun hBound hBody hArg =>
      hAppAppStepEquBet hSubOp hArgScoped hEqOp hEqArg hBound₁₃ hBound₂₃
        hBound hBody hArg)
    (fun hBound hOldScoped hBody =>
      hAppAppStepEquFOp hSubOp hArgScoped hEqOp hEqArg hBound₁₃ hBound₂₃
        hBound hOldScoped hBody)
    hEqStep

/-- Dispatcher variant that wires the stable recursive app/pro handlers in the
structural `Ms-App × Me-App` body branch. `Me-Pro` now only needs transport of
its equivalence payload; `Me-App`/`Ms-App` now only need recursive operator
transport under the fixed nested argument stack. The β and `FOp` residuals
remain explicit. -/
theorem commute_abs_fun_fun_body_from_operator_join_app_cases_recursive_stack_handlers_of
    {Γ : Ctx} {bound body bound₁ body₁ bound₂ body₂ : Term}
    (hpvNil : PrevalidExt Γ [])
    (hdiamondBound : EqDiamonds Γ [])
    (hdiamondBody : EqDiamonds ({ bound := bound, kind := .sub } :: Γ) [])
  ...
  refine
    commute_abs_fun_fun_body_from_operator_join_app_cases_equ_stack_handlers_of
      hpvNil hdiamondBound hdiamondBody hSubBound hSubBody hEqBound hEqBody
      hAppAppComm hAppAppSubReplace ?_ ?_ hAppAppStepEquBet
      hAppAppStepEquFOp ?_ hAppAppStepFOp hAppBet hFun
  · intro u u' v u₂ v₂ hSubOp hArgScoped hEqOp hEqArg bound₃ hBound₁₃ hBound₂₃
      i α α' hb hPayload
    let Γ₃ : Ctx := { bound := bound₃, kind := .sub } :: Γ
    have hEqArg₃ : MEqRed Γ₃ [] v v₂ :=
    ...
    have hαScoped : Term.Scoped Γ₃.depth α :=
      Prevalid.scoped_lookupEqu (PrevalidExt.ctx hpvNew) hb
    have hαRefl : MEqRed Γ₃ (v₂ :: []) α α :=
      MEqRed.refl hpvNew hαScoped
    exact MSubRedStar.trans
      (MSubRedStar.single (MSubRed.equ hpvNew (MEqRed.pro hpvNew hb hαRefl)))
      (hAppAppStepEquProPayload hSubOp hArgScoped hEqOp hEqArg
        hBound₁₃ hBound₂₃ hPayload)
  · intro u u' v u₂ v₂ hSubOp hArgScoped hEqOp hEqArg bound₃ hBound₁₃ hBound₂₃
      op op' arg arg' hOp hArg
    let Γ₃ : Ctx := { bound := bound₃, kind := .sub } :: Γ
    have hEqArg₃ : MEqRed Γ₃ [] v v₂ :=
      hEqArg.sub_head_replace_two_step hEqBound hBound₂₃.some
    have hpvNew : PrevalidExt Γ₃ (v₂ :: []) :=
      PrevalidExt.push (PrevalidExt.nil (Prevalid.sub (PrevalidExt.ctx hpvNil)
        hBound₂₃.some.scoped_right)) hEqArg₃.scoped_right
    have hpvArgNew : PrevalidExt Γ₃ (arg :: v₂ :: []) :=
      PrevalidExt.push hpvNew hArg.scoped_left
    have hOpRefl : MEqRed Γ₃ (arg :: v₂ :: []) op' op' :=
      MEqRed.refl hpvArgNew hOp.scoped_right
    exact MSubRedStar.trans
      (msubRedStar_app_fixed_arg hArg.scoped_left
        (hAppAppStepEquAppOp hSubOp hArgScoped hEqOp hEqArg hBound₁₃
          hBound₂₃ hOp hArg))
      (MSubRedStar.single (MSubRed.equ hpvNew (MEqRed.app hOpRefl hArg)))
  · intro u u' v u₂ v₂ hSubOp hArgScoped hEqOp hEqArg bound₃ hBound₁₃ hBound₂₃
      op op' arg hOp hArg
    exact msubRedStar_app_fixed_arg hArg
      (hAppAppStepAppOp hSubOp hArgScoped hEqOp hEqArg hBound₁₃ hBound₂₃ hOp hArg)

/-- Dispatcher variant that reduces the remaining β/`FOp` residual stack
handlers in the structural `Ms-App × Me-App` body branch to body-premise
transports. Once those body premises are moved to the changed argument head,
the outer `Me-Bet`, `Me-FOp`, or `Ms-FOp` constructor is rebuilt directly. -/
theorem commute_abs_fun_fun_body_from_operator_join_app_cases_residual_body_handlers_of
    {Γ : Ctx} {bound body bound₁ body₁ bound₂ body₂ : Term}
    (hpvNil : PrevalidExt Γ [])
    (hdiamondBound : EqDiamonds Γ [])
    (hdiamondBody : EqDiamonds ({ bound := bound, kind := .sub } :: Γ) [])
  ...
  refine
    commute_abs_fun_fun_body_from_operator_join_app_cases_recursive_stack_handlers_of
      ...
      hAppAppStepEquAppOp ?_ ?_ hAppAppStepAppOp ?_ hAppBet hFun
  · intro u u' v u₂ v₂ hSubOp hArgScoped hEqOp hEqArg bound₃ hBound₁₃ hBound₂₃
    ...
    have hpvNew : PrevalidExt Γ₃ (v₂ :: []) :=
      PrevalidExt.push (PrevalidExt.nil (Prevalid.sub (PrevalidExt.ctx hpvNil)
        hBound₂₃.some.scoped_right)) hEqArg₃.scoped_right
    exact MSubRedStar.single (MSubRed.equ hpvNew
      (MEqRed.bet hFunBound
        (hAppAppStepEquBetBody hSubOp hArgScoped hEqOp hEqArg hBound₁₃ hBound₂₃
          hFunBound hBody hArg)
        hArg))
  · intro u u' v u₂ v₂ hSubOp hArgScoped hEqOp hEqArg bound₃ hBound₁₃ hBound₂₃
      funBound funBound' inner inner' hBound hOldScoped hBody
    let Γ₃ : Ctx := { bound := bound₃, kind := .sub } :: Γ
    have hEqArg₃ : MEqRed Γ₃ [] v v₂ :=
      hEqArg.sub_head_replace_two_step hEqBound hBound₂₃.some
    have hpvNew : PrevalidExt Γ₃ (v₂ :: []) :=
      PrevalidExt.push (PrevalidExt.nil (Prevalid.sub (PrevalidExt.ctx hpvNil)
        hBound₂₃.some.scoped_right)) hEqArg₃.scoped_right
    exact MSubRedStar.single (MSubRed.equ hpvNew
      (MEqRed.fOp hBound hEqArg₃.scoped_right
        (hAppAppStepEquFOpBody hSubOp hArgScoped hEqOp hEqArg hBound₁₃ hBound₂₃
          hBound hOldScoped hBody)))
  · intro u u' v u₂ v₂ hSubOp hArgScoped hEqOp hEqArg bound₃ hBound₁₃ hBound₂₃
      funBound body body' hFunBound hOldScoped hBody
    let Γ₃ : Ctx := { bound := bound₃, kind := .sub } :: Γ
    have hEqArg₃ : MEqRed Γ₃ [] v v₂ :=
      hEqArg.sub_head_replace_two_step hEqBound hBound₂₃.some
    exact MSubRedStar.single
      (MSubRed.fOp hFunBound hEqArg₃.scoped_right
        (hAppAppStepFOpBody hSubOp hArgScoped hEqOp hEqArg hBound₁₃ hBound₂₃
          hFunBound hOldScoped hBody))

/-- Dispatcher variant whose residual `Ms-FOp` body transport is already a
raw subtype-reduction chain. This keeps recursive body replacement from being
forced back to a one-step reduction and lifts the chain through the fixed-bound
`FOp` abstraction. -/
theorem commute_abs_fun_fun_body_from_operator_join_app_cases_residual_body_star_handlers_of
    {Γ : Ctx} {bound body bound₁ body₁ bound₂ body₂ : Term}
    (hpvNil : PrevalidExt Γ [])
    (hdiamondBound : EqDiamonds Γ [])
    (hdiamondBody : EqDiamonds ({ bound := bound, kind := .sub } :: Γ) [])
  ...
  · intro u u' v u₂ v₂ hSubOp hArgScoped hEqOp hEqArg bound₃ hBound₁₃ hBound₂₃
    ...
    simpa [Γ₃, Stack.shift] using
      msubRedStar_abs_fOp_body_fixed_bound hFunBound hEqArg₃.scoped_right
        (hAppAppStepFOpBody hSubOp hArgScoped hEqOp hEqArg hBound₁₃ hBound₂₃
          hFunBound hOldScoped hBody)
...
/-- Dispatcher variant that splits the residual `Me-FOp` and raw `Ms-FOp`
...
theorem msubRedStar_equ_under_sub_head_sub_tail_nil_replace_from_raw_handlers
    ...
    (hEq :
      ∀ {u v : Term},
        MEqRed ({ bound := headBound, kind := .sub } ::
          { bound := old, kind := .equ } :: { bound := tailBound, kind := .sub } ::
          Γ) [] u v →
        MSubRedStar ({ bound := headBound, kind := .sub } ::
  ...
  have hpvNew : PrevalidExt ({ bound := headBound, kind := .sub } ::
    ...
    simpa [CtxEntry.ScopedIn, Ctx.depth] using
      PrevalidExt.replaceAt_equ_same
        (Γ := { bound := headBound, kind := .sub } ::
          { bound := old, kind := .equ } :: { bound := tailBound, kind := .sub } ::
          Γ)
        (s := []) (cutoff := 1) (old := old) (new := new)
        hpv (by simp [Ctx.depth])
        (by simpa [CtxEntry.ScopedIn, Ctx.depth] using hnew)
  cases h with
  | pro _ hb =>
    exact MSubRedStar.single (by
      simpa [CtxEntry.ScopedIn, Ctx.depth] using
        MSubRed.pro_replaceAt_equ
          (Γ := { bound := headBound, kind := .sub } ::
            { bound := old, kind := .equ } :: { bound := tailBound, kind := .sub } ::
            Γ)
          (s := []) (cutoff := 1) (old := old) (new := new)
          hpv (by simp [Ctx.depth])
          (by simpa [CtxEntry.ScopedIn, Ctx.depth] using hnew) hb)
  | top _ hu =>
    exact MSubRedStar.single (MSubRed.top hpvNew (by
      simpa [Ctx.depth] using hu))
  | equ _ heq =>
    exact hEq heq
  | app hOp hArg =>
    exact hApp hOp hArg
  | fun_ hBoundScoped hBound hBody =>
    exact hFun hBoundScoped hBound hBody

/-- Consume a raw subtype-reduction chain in the empty-stack `{sub}, {equ},
{sub}` shape when each old-context step has a raw-subtype-chain replacement
in the new context. -/
theorem msubRedStar_equ_under_sub_head_sub_tail_nil_replace_from_raw_handlers_star
    {Γ : Ctx} {headBound tailBound old new u v : Term}
    (hpv : PrevalidExt ({ bound := headBound, kind := .sub } ::
      { bound := old, kind := .equ } :: { bound := tailBound, kind := .sub } ::
      Γ) [])
  ...
  | @tail mid v hStar hLast ih =>
    exact MSubRedStar.trans ih
      (msubRedStar_equ_under_sub_head_sub_tail_nil_replace_from_raw_handlers
        hpv hnew hEq hApp hFun hLast.some)
...
theorem commute_abs_fun_fun_body_from_operator_join_app_cases_fop_body_equ_handlers_of
      ...
      ∀ {u u' v u₂ v₂ : Term},
        ...
        ∀ {bound₃ : Term},
          ...
          ∀ {target : Term},
            MEqRed ({ bound := v, kind := .equ } ::
              { bound := bound₃, kind := .sub } :: Γ) (Stack.shift 0 [])
              (Term.shift 0 v) target →
            MEqRed ({ bound := v₂, kind := .equ } ::
              { bound := bound₃, kind := .sub } :: Γ) (Stack.shift 0 [])
              (.bvar 0) target)
    (hAppAppStepEquFOpProTail :
      ∀ {u u' v u₂ v₂ : Term},
        MSubRed ({ bound := bound, kind := .sub } :: Γ) (v :: []) u u' →
        Term.Scoped (Ctx.depth ({ bound := bound, kind := .sub } :: Γ)) v →
        MEqRed ({ bound := bound, kind := .sub } :: Γ) (v :: []) u u₂ →
        ...
        ∀ {bound₃ : Term},
          ...
          MEqRedJ Γ [] bound₂ bound₃ →
          ∀ {i : Nat} {α α' : Term},
            Ctx.equBinds ({ bound := v, kind := .equ } ::
              { bound := bound₃, kind := .sub } :: Γ) (i + 1) α →
            MEqRed ({ bound := v, kind := .equ } ::
              { bound := bound₃, kind := .sub } :: Γ) (Stack.shift 0 []) α α' →
            MEqRed ({ bound := v₂, kind := .equ } ::
              { bound := bound₃, kind := .sub } :: Γ) (Stack.shift 0 [])
              (.bvar (i + 1)) α')
    (hAppAppStepEquFOpApp :
      ∀ {u u' v u₂ v₂ : Term},
        MSubRed ({ bound := bound, kind := .sub } :: Γ) (v :: []) u u' →
        Term.Scoped (Ctx.depth ({ bound := bound, kind := .sub } :: Γ)) v →
        MEqRed ({ bound := bound, kind := .sub } :: Γ) (v :: []) u u₂ →
        ...
        ∀ {bound₃ : Term},
          ...
          ∀ {op op' arg arg' : Term},
            MEqRed ({ bound := v, kind := .equ } ::
              { bound := bound₃, kind := .sub } :: Γ) (arg :: Stack.shift 0 [])
              op op' →
            MEqRed ({ bound := v, kind := .equ } ::
              { bound := bound₃, kind := .sub } :: Γ) [] arg arg' →
            MEqRed ({ bound := v₂, kind := .equ } ::
    ...
    (hAppAppStepEquFOpFun :
      ∀ {u u' v u₂ v₂ : Term},
        ...
        ∀ {bound₃ : Term},
          ...
          ∀ {funBound funBound' inner inner' : Term},
            MEqRed ({ bound := v, kind := .equ } ::
              { bound := bound₃, kind := .sub } :: Γ) [] funBound funBound' →
            MEqRed ({ bound := funBound, kind := .sub } ::
              { bound := v, kind := .equ } ::
              { bound := bound₃, kind := .sub } :: Γ) [] inner inner' →
            MEqRed ({ bound := v₂, kind := .equ } ::
    ...
    (hAppAppStepEquFOpBet :
      ∀ {u u' v u₂ v₂ : Term},
        ...
        ∀ {bound₃ : Term},
          ...
          MEqRedJ Γ [] bound₂ bound₃ →
          ∀ {funBound arg arg' inner inner' : Term},
            Term.Scoped (Ctx.depth ({ bound := v, kind := .equ } ::
              { bound := bound₃, kind := .sub } :: Γ)) funBound →
            MEqRed ({ bound := funBound, kind := .sub } ::
              { bound := v, kind := .equ } ::
              { bound := bound₃, kind := .sub } :: Γ) (Stack.shift 0 [])
              inner inner' →
        ...
        ∀ {bound₃ : Term},
          ...
          ∀ {op op' arg : Term},
            MSubRed ({ bound := v, kind := .equ } ::
              ...
              op op' →
            Term.Scoped (Ctx.depth ({ bound := v, kind := .equ } ::
              { bound := bound₃, kind := .sub } :: Γ)) arg →
            MSubRed ({ bound := v₂, kind := .equ } ::
              { bound := bound₃, kind := .sub } :: Γ) (Stack.shift 0 [])
              (.app op arg) (.app op' arg))
    (hAppAppStepFOpFunEquProOne :
      ∀ {u u' v u₂ v₂ : Term},
        MSubRed ({ bound := bound, kind := .sub } :: Γ) (v :: []) u u' →
        Term.Scoped (Ctx.depth ({ bound := bound, kind := .sub } :: Γ)) v →
        MEqRed ({ bound := bound, kind := .sub } :: Γ) (v :: []) u u₂ →
        ...
        ∀ {bound₃ : Term},
          ...
          ∀ {funBound funBound' target : Term},
            ...
            MEqRed ({ bound := funBound, kind := .sub } ::
              { bound := v, kind := .equ } ::
              { bound := bound₃, kind := .sub } :: Γ) []
              (Term.shift 0 (Term.shift 0 v)) target →
            MEqRed ({ bound := funBound, kind := .sub } ::
              { bound := v₂, kind := .equ } ::
              { bound := bound₃, kind := .sub } :: Γ) [] (.bvar 1) target)
    (hAppAppStepFOpFunEquProTail :
      ∀ {u u' v u₂ v₂ : Term},
        MSubRed ({ bound := bound, kind := .sub } :: Γ) (v :: []) u u' →
        Term.Scoped (Ctx.depth ({ bound := bound, kind := .sub } :: Γ)) v →
        MEqRed ({ bound := bound, kind := .sub } :: Γ) (v :: []) u u₂ →
    ...
    (hAppAppStepFOpFunEquApp :
      ∀ {u u' v u₂ v₂ : Term},
        ...
        ∀ {bound₃ : Term},
          MEqRedJ Γ [] bound₁ bound₃ →
          MEqRedJ Γ [] bound₂ bound₃ →
          ∀ {funBound funBound' op op' arg arg' : Term},
            Term.Scoped (Ctx.depth ({ bound := v, kind := .equ } ::
              { bound := bound₃, kind := .sub } :: Γ)) funBound →
            MEqRed ({ bound := v, kind := .equ } ::
              { bound := bound₃, kind := .sub } :: Γ) [] funBound funBound' →
            MEqRed ({ bound := funBound, kind := .sub } ::
              { bound := v, kind := .equ } ::
              { bound := bound₃, kind := .sub } :: Γ) (arg :: []) op op' →
            MEqRed ({ bound := funBound, kind := .sub } ::
              { bound := v, kind := .equ } ::
              { bound := bound₃, kind := .sub } :: Γ) [] arg arg' →
            MEqRed ({ bound := funBound, kind := .sub } ::
    ...
    (hAppAppStepFOpFunEquFun :
      ∀ {u u' v u₂ v₂ : Term},
        ...
        ∀ {bound₃ : Term},
          MEqRedJ Γ [] bound₁ bound₃ →
          MEqRedJ Γ [] bound₂ bound₃ →
          ∀ {funBound funBound' innerBound innerBound' inner inner' : Term},
            Term.Scoped (Ctx.depth ({ bound := v, kind := .equ } ::
              { bound := bound₃, kind := .sub } :: Γ)) funBound →
            MEqRed ({ bound := v, kind := .equ } ::
              { bound := bound₃, kind := .sub } :: Γ) [] funBound funBound' →
    ...
    (hAppAppStepFOpFunEquBet :
      ∀ {u u' v u₂ v₂ : Term},
        ...
        ∀ {bound₃ : Term},
          MEqRedJ Γ [] bound₁ bound₃ →
          MEqRedJ Γ [] bound₂ bound₃ →
          ∀ {funBound funBound' innerBound arg arg' inner inner' : Term},
            Term.Scoped (Ctx.depth ({ bound := v, kind := .equ } ::
              { bound := bound₃, kind := .sub } :: Γ)) funBound →
            MEqRed ({ bound := v, kind := .equ } ::
              { bound := bound₃, kind := .sub } :: Γ) [] funBound funBound' →
    ...
    (hAppAppStepFOpFunApp :
      ∀ {u u' v u₂ v₂ : Term},
        ...
        ∀ {bound₃ : Term},
          MEqRedJ Γ [] bound₁ bound₃ →
          MEqRedJ Γ [] bound₂ bound₃ →
          ∀ {funBound funBound' op op' arg : Term},
            Term.Scoped (Ctx.depth ({ bound := v, kind := .equ } ::
              { bound := bound₃, kind := .sub } :: Γ)) funBound →
            MEqRed ({ bound := v, kind := .equ } ::
              { bound := bound₃, kind := .sub } :: Γ) [] funBound funBound' →
        ...
        ∀ {bound₃ : Term},
          ...
          ∀ {funBound funBound' innerBound innerBound' inner inner' : Term},
            ...
            MEqRed ({ bound := funBound, kind := .sub } ::
              { bound := v, kind := .equ } ::
              { bound := bound₃, kind := .sub } :: Γ) [] innerBound innerBound' →
            MSubRed ({ bound := innerBound, kind := .sub } ::
              { bound := funBound, kind := .sub } ::
              { bound := v, kind := .equ } ::
              { bound := bound₃, kind := .sub } :: Γ) [] inner inner' →
            MSubRedStar ({ bound := funBound, kind := .sub } ::
              { bound := v₂, kind := .equ } ::
              { bound := bound₃, kind := .sub } :: Γ) []
              (.abs innerBound inner) (.abs innerBound' inner'))
    (hAppBet :
  ...
  refine
    commute_abs_fun_fun_body_from_operator_join_app_cases_bet_body_stack_handlers_of
      hpvNil hdiamondBound hdiamondBody hSubBound hSubBody hEqBound hEqBody
      hAppAppComm hAppAppSubReplace hAppAppStepEquProPayload
      hAppAppStepEquAppOp hAppAppStepEquBetPro hAppAppStepEquBetAppOp
      hAppAppStepEquBetBody hAppAppStepEquBetFOp ?_ hAppAppStepAppOp
      ?_ hAppBet hFun
  · intro u u' v u₂ v₂ hSubOp hArgScoped hEqOp hEqArg bound₃ hBound₁₃ hBound₂₃
      funBound funBound' inner inner' hBound hOldScoped hBody
    let Γ₃ : Ctx := { bound := bound₃, kind := .sub } :: Γ
    ...
    simpa [Γ₃, Stack.shift] using
      MEqRed.equ_head_replace_from_handlers
        (Γ := Γ₃) (s := []) (old := v) (new := v₂) (u := inner) (v := inner')
              ...
              simpa [Γ₃, Stack.shift] using
                MEqRed.equ_head_replace_from_handlers
                  ...
                  (by
                    ...
                    cases hStack)
                  hBoundStep
            have hBodyNew : MSubRedStar ({ bound := funBound, kind := .sub } ::
              ...
              simpa [Γ₃, Stack.shift] using
                msubRedStar_equ_under_sub_head_sub_tail_nil_replace_from_raw_handlers
                  ...
                  (by
                    intro α α' hEqBody
                    exact MSubRedStar.single (MSubRed.equ
                      (PrevalidExt.replaceAt_equ_same
                        (Γ := { bound := funBound, kind := .sub } ::
                          { bound := v, kind := .equ } ::
                          { bound := bound₃, kind := .sub } :: Γ)
                        (s := []) (cutoff := 1) (old := v) (new := v₂)
                        hpvBodyOld (by simp [Ctx.depth])
                        (by
                          simpa [CtxEntry.ScopedIn, Ctx.depth] using
                            hEqArg₃.scoped_right))
                      (by
                        simpa [Γ₃, Stack.shift] using
                          MEqRed.equ_under_sub_head_sub_tail_nil_replace_from_split_handlers
                            (Γ := Γ) (headBound := funBound) (tailBound := bound₃)
                            (old := v) (new := v₂) (u := α) (v := α')
                            hpvBodyOld hEqArg₃.scoped_right
                            (by
                              intro target hTarget
                              simpa [Γ₃, Stack.shift] using
                                hAppAppStepFOpFunEquProOne hSubOp hArgScoped hEqOp
                                  hEqArg hBound₁₃ hBound₂₃ hFunScoped hBoundStep
                                  hTarget)
                            (by
                              intro i α₀ α₀' hb hα
                              simpa [Γ₃, Stack.shift] using
                                hAppAppStepFOpFunEquProTail hSubOp hArgScoped hEqOp
                                  hEqArg hBound₁₃ hBound₂₃ hFunScoped hBoundStep
                                  hb hα)
                            (by
                              intro op op' arg arg' hOp hArg
                              simpa [Γ₃, Stack.shift] using
                                hAppAppStepFOpFunEquApp hSubOp hArgScoped hEqOp
                                  hEqArg hBound₁₃ hBound₂₃ hFunScoped hBoundStep
                                  hOp hArg)
                            (by
                              intro innerBound innerBound' inner inner' hInnerBound hInner
                              simpa [Γ₃, Stack.shift] using
                                hAppAppStepFOpFunEquFun hSubOp hArgScoped hEqOp
                                  hEqArg hBound₁₃ hBound₂₃ hFunScoped hBoundStep
                                  hInnerBound hInner)
                            (by
                              intro innerBound arg arg' inner inner' hInnerScoped hInner hArg
                              simpa [Γ₃, Stack.shift] using
                                hAppAppStepFOpFunEquBet hSubOp hArgScoped hEqOp
                                  hEqArg hBound₁₃ hBound₂₃ hFunScoped hBoundStep
                                  hInnerScoped hInner hArg)
                            hEqBody)))
                  (by
                    intro op op' arg hOp hArg
                    exact MSubRedStar.single (by
                      simpa [Γ₃, Stack.shift] using
                        hAppAppStepFOpFunApp hSubOp hArgScoped hEqOp hEqArg
...
/-- Canonical `Ms-Fun` handler for innermost `.equ` replacement when the
bound equivalence has been replaced as a raw step but the body replacement is
already diagrammatic. -/
theorem msub_equ_head_fun_handler_of_body_replacement {Γ : Ctx}
    {old new : Term}
    ...
    (hBodyReplace :
      ∀ {bound body body' : Term},
        ...
        MSubStar ({ bound := bound, kind := .sub } ::
          { bound := new, kind := .equ } :: Γ) [] body body')
    {bound bound' body body' : Term}
    (_hBoundScoped : Term.Scoped (Ctx.depth ({ bound := old, kind := .equ } :: Γ)) bound)
    (hBound : MEqRed ({ bound := old, kind := .equ } :: Γ) [] bound bound')
    (hBody : MSubRed ({ bound := bound, kind := .sub } ::
      { bound := old, kind := .equ } :: Γ) [] body body') :
    MSubStar ({ bound := new, kind := .equ } :: Γ) []
  ...
  have hpvNew : PrevalidExt ({ bound := new, kind := .equ } :: Γ) [] :=
    PrevalidExt.equ_head_replace hpvNil hnew
  exact msubStar_abs_fun_body_equ_bound hpvNew (hBoundReplace hBound)
    hBody.scoped_left (hBodyReplace hBody)
...
bound equivalence has been replaced as a raw step but the body replacement is
already diagrammatic. -/
theorem msub_equ_under_head_fun_handler_of_body_replacement {Γ : Ctx}
    {head : CtxEntry} {old new : Term}
    ...
    (hBodyReplace :
      ∀ {bound body body' : Term},
        MSubRed ({ bound := bound, kind := .sub } :: head ::
          { bound := old, kind := .equ } :: Γ) [] body body' →
        MSubStar ({ bound := bound, kind := .sub } :: head ::
          { bound := new, kind := .equ } :: Γ) [] body body')
    {bound bound' body body' : Term}
    (_hBoundScoped : Term.Scoped (Ctx.depth (head :: { bound := old, kind := .equ } :: Γ)) bound)
    (hBound : MEqRed (head :: { bound := old, kind := .equ } :: Γ) [] bound bound')
    (hBody : MSubRed ({ bound := bound, kind := .sub } :: head ::
      { bound := old, kind := .equ } :: Γ) [] body body') :
    MSubStar (head :: { bound := new, kind := .equ } :: Γ) []
  ...
  have hpvNew : PrevalidExt (head :: { bound := new, kind := .equ } :: Γ) [] :=
    PrevalidExt.equ_under_head_replace hpvNil hnew
  exact msubStar_abs_fun_body_equ_bound hpvNew (hBoundReplace hBound)
    hBody.scoped_left (hBodyReplace hBody)
...
/-- Subtype replacement across an innermost changed `.equ` head where the
recursive `Ms-Fun` body replacement is already diagrammatic. -/
theorem msubRed_equ_head_replace_from_body_replacements {Γ : Ctx} {s : Stack}
    {old new u v : Term}
    (hpv : PrevalidExt ({ bound := old, kind := .equ } :: Γ) s)
    (hnew : Term.Scoped Γ.depth new)
    (hEq :
  ...
  msubRed_equ_head_replace_from_handlers hpv hnew hEq
    (msub_equ_head_app_handler_of_operator_replacement hpv hnew hAppOpReplace)
    (msub_equ_head_fun_handler_of_body_replacement
...
/-- Subtype replacement when the changed `.equ` entry sits immediately under
a preserved head and the recursive `Ms-Fun` body replacement is already
diagrammatic. -/
theorem msubRed_equ_under_head_replace_from_body_replacements {Γ : Ctx} {s : Stack}
    {head : CtxEntry} {old new u v : Term}
    (hpv : PrevalidExt (head :: { bound := old, kind := .equ } :: Γ) s)
    (hnew : Term.Scoped Γ.depth new)
    (hEq :
  ...
  msubRed_equ_under_head_replace_from_handlers hpv hnew hEq
    (msub_equ_under_head_app_handler_of_operator_replacement hpv hnew hAppOpReplace)
    (msub_equ_under_head_fun_handler_of_body_replacement
      (PrevalidExt.nil (PrevalidExt.ctx hpv)) hnew
      hFunBoundReplace hFunBodyReplace)
    (msub_equ_under_head_fop_handler_of_body_replacement hpv hnew hFOpBodyReplace)
    h
...
/-- Diagrammatic one-step equivalence replacement across a changed `.equ`
entry at an arbitrary context index, presented by `Ctx.replaceAt`. Stable
leaves are rebuilt directly; lookup and recursive constructor cases are
exposed as handlers. -/
theorem meqRed_replaceAt_equ_from_handlers {Γ : Ctx} {s : Stack}
    ...
    (hBet :
      ∀ {bound arg arg' body body' : Term},
        Term.Scoped
          (Ctx.depth (Ctx.replaceAt cutoff { bound := old, kind := .equ } Γ)) bound →
        MEqRed ({ bound := bound, kind := .sub } ::
          Ctx.replaceAt cutoff { bound := old, kind := .equ } Γ)
          (Stack.shift 0 s) body body' →
        MEqRed (Ctx.replaceAt cutoff { bound := old, kind := .equ } Γ)
    ...
    MSubStar (Ctx.replaceAt cutoff { bound := new, kind := .equ } Γ) s u v := by
  have hpvNew :
  ...
  | var _ hi =>
      exact MSubStar.of_MEqRed hpvNew (MEqRed.var hpvNew (by
        simpa [Ctx.depth_replaceAt] using hi))
  | fun_ hBound hBody =>
      exact hFun hBound hBody
  | tAp _ hu =>
      exact MSubStar.of_MEqRed hpvNew (MEqRed.tAp hpvNew (by
        simpa [Ctx.depth_replaceAt] using hu))
  | fOp hBound hArg hBody =>
      exact hFOp hBound hArg rfl hBody
  | bet ht hBody hArg =>
      exact hBet ht hBody hArg
...
/-- Constructor-wired variant of
`meqRed_equ_under_sub_head_sub_tail_nil_replace_from_handlers`. The
lookup-sensitive `Me-Pro` residuals stay explicit; application, function, and
beta cases are rebuilt from raw recursive replacements. -/
theorem meqRed_equ_under_sub_head_sub_tail_nil_replace_from_replacements
    {Γ : Ctx} {headBound tailBound old new u v : Term}
    (hpv : PrevalidExt ({ bound := headBound, kind := .sub } ::
      { bound := old, kind := .equ } :: { bound := tailBound, kind := .sub } ::
      Γ) [])
  ...
  have hpvNew : PrevalidExt ({ bound := headBound, kind := .sub } ::
    ...
    simpa [CtxEntry.ScopedIn, Ctx.depth] using
      PrevalidExt.replaceAt_equ_same
        ...
        (by simpa [CtxEntry.ScopedIn, Ctx.depth] using hnew)
  exact
    meqRed_equ_under_sub_head_sub_tail_nil_replace_from_handlers
      hpv hnew hProOne hProTail
      (by
        intro op op' arg arg' hOp hArg
      ...
      (by
        ...
        exact MSubStar.of_MEqRed hpvNew
          (MEqRed.bet hBoundScoped (hBetBodyReplace hBody) (hNilReplace hArg)))
      h
...
/-- One-step subtype replacement splitter when the changed `.equ` entry sits
under two preserved heads. This closes the context-stable `Ms-Top` leaf and
exposes lookup-sensitive and recursive constructor cases as handlers. -/
theorem msubRed_equ_under_two_heads_replace_from_handlers {Γ : Ctx} {s : Stack}
    {head₁ head₂ : CtxEntry} {old new u v : Term}
    (hpv : PrevalidExt (head₁ :: head₂ :: { bound := old, kind := .equ } :: Γ) s)
    (hnew : Term.Scoped Γ.depth new)
    (hPro :
      ∀ {i : Nat} {t : Term},
        Ctx.subBinds (head₁ :: head₂ :: { bound := old, kind := .equ } :: Γ) i t →
        MSubStar (head₁ :: head₂ :: { bound := new, kind := .equ } :: Γ) s
          (.bvar i) t)
    (hEq :
      ∀ {u v : Term},
        MEqRed (head₁ :: head₂ :: { bound := old, kind := .equ } :: Γ) s u v →
        MSubStar (head₁ :: head₂ :: { bound := new, kind := .equ } :: Γ) s u v)
    (hApp :
      ∀ {op op' arg : Term},
        MSubRed (head₁ :: head₂ :: { bound := old, kind := .equ } :: Γ)
          (arg :: s) op op' →
        Term.Scoped
          (Ctx.depth (head₁ :: head₂ :: { bound := old, kind := .equ } :: Γ))
          arg →
    ...
    (hFun :
      ∀ {bound bound' body body' : Term},
        ...
        MEqRed (head₁ :: head₂ :: { bound := old, kind := .equ } :: Γ)
          [] bound bound' →
        MSubRed ({ bound := bound, kind := .sub } :: head₁ :: head₂ ::
          { bound := old, kind := .equ } :: Γ) [] body body' →
        MSubStar (head₁ :: head₂ :: { bound := new, kind := .equ } :: Γ)
          [] (.abs bound body) (.abs bound' body'))
    (hFOp :
      ∀ {bound arg body body' : Term} {rest : Stack},
        ...
        Term.Scoped
          ...
          arg →
        s = arg :: rest →
        MSubRed ({ bound := arg, kind := .equ } :: head₁ :: head₂ ::
          { bound := old, kind := .equ } :: Γ) (Stack.shift 0 rest) body body' →
        MSubStar (head₁ :: head₂ :: { bound := new, kind := .equ } :: Γ) s
          (.abs bound body) (.abs bound body'))
    (h : MSubRed (head₁ :: head₂ :: { bound := old, kind := .equ } :: Γ) s u v) :
    MSubStar (head₁ :: head₂ :: { bound := new, kind := .equ } :: Γ) s u v := by
  have hpvNew :
      PrevalidExt (head₁ :: head₂ :: { bound := new, kind := .equ } :: Γ) s :=
    PrevalidExt.equ_under_two_heads_replace hpv hnew
  ...
  | pro _ hb =>
    exact hPro hb
  | top _ hu =>
    exact MSubStar.of_MSubRed (MSubRed.top hpvNew (by
      simpa [Ctx.depth] using hu))
  | equ _ heq =>
    exact hEq heq
  | app hOp hArg =>
...
/-- One-step equivalence replacement splitter when the changed `.equ` entry
sits under three preserved heads. This closes the context-stable leaves and
exposes lookup-sensitive and recursive constructor cases as handlers. -/
theorem meqRed_equ_under_three_heads_replace_from_handlers {Γ : Ctx} {s : Stack}
    {head₁ head₂ head₃ : CtxEntry} {old new u v : Term}
    (hpv :
...
/-- One-step subtype replacement splitter when the changed `.equ` entry sits
under three preserved heads. This closes the context-stable `Ms-Top` leaf and
exposes lookup-sensitive and recursive constructor cases as handlers. -/
theorem msubRed_equ_under_three_heads_replace_from_handlers {Γ : Ctx} {s : Stack}
    {head₁ head₂ head₃ : CtxEntry} {old new u v : Term}
    (hpv :
      PrevalidExt (head₁ :: head₂ :: head₃ :: { bound := old, kind := .equ } :: Γ) s)
    (hnew : Term.Scoped Γ.depth new)
...
/-- Raw subtype-star variant of
`msubRed_equ_under_three_heads_replace_from_handlers`. Stable `Ms-Pro` leaves
and recursive constructors are explicit handlers, while `Ms-Top` is rebuilt
in the replaced context. -/
theorem msubRedStar_equ_under_three_heads_replace_from_handlers {Γ : Ctx}
...
/-- Consume a raw subtype-reduction chain when the changed `.equ` entry sits
under three preserved heads and every old-context step has a raw-subtype-chain
replacement in the new context. -/
theorem msubRedStar_equ_under_three_heads_replace_from_handlers_star {Γ : Ctx}
    {s : Stack} {head₁ head₂ head₃ : CtxEntry} {old new u v : Term}
    (hpv :
      PrevalidExt (head₁ :: head₂ :: head₃ :: { bound := old, kind := .equ } :: Γ) s)
    (hnew : Term.Scoped Γ.depth new)
    ...
    (hFOp :
      ∀ {bound arg body body' : Term} {rest : Stack},
        ...
        MSubRedStar (head₁ :: head₂ :: head₃ :: { bound := new, kind := .equ } :: Γ)
          s (.abs bound body) (.abs bound body'))
    (h : MSubRedStar
      (head₁ :: head₂ :: head₃ :: { bound := old, kind := .equ } :: Γ) s u v) :
    MSubRedStar (head₁ :: head₂ :: head₃ :: { bound := new, kind := .equ } :: Γ)
      s u v := by
  induction h with
  | refl =>
  ...
  | @tail mid v hStar hLast ih =>
    exact MSubRedStar.trans ih
      (msubRedStar_equ_under_three_heads_replace_from_handlers hpv hnew hPro hEq
        hApp hFun hFOp hLast.some)

/-- Raw subtype-star replacement under three preserved heads, where recursive
constructor replacements already return raw subtype chains. -/
theorem msubRedStar_equ_under_three_heads_replace_from_raw_body_replacements
    ...
    MSubRedStar (head₁ :: head₂ :: head₃ :: { bound := new, kind := .equ } :: Γ)
      s u v :=
  msubRedStar_equ_under_three_heads_replace_from_handlers_star hpv hnew
    (fun hb => MSubRedStar.single
      (MSubRed.pro_equ_under_three_heads_replace hpv hnew hb))
    hEq
    (by
...
/-- Raw subtype-star variant of
`msubRed_equ_under_two_heads_replace_from_handlers`. Stable `Ms-Pro` leaves
and recursive constructors are explicit handlers, while `Ms-Top` is rebuilt
in the replaced context. -/
theorem msubRedStar_equ_under_two_heads_replace_from_handlers {Γ : Ctx} {s : Stack}
    {head₁ head₂ : CtxEntry} {old new u v : Term}
    (hpv : PrevalidExt (head₁ :: head₂ :: { bound := old, kind := .equ } :: Γ) s)
    (hnew : Term.Scoped Γ.depth new)
    (hPro :
      ∀ {i : Nat} {t : Term},
        Ctx.subBinds (head₁ :: head₂ :: { bound := old, kind := .equ } :: Γ) i t →
        MSubRedStar (head₁ :: head₂ :: { bound := new, kind := .equ } :: Γ) s
          (.bvar i) t)
    (hEq :
      ∀ {u v : Term},
        MEqRed (head₁ :: head₂ :: { bound := old, kind := .equ } :: Γ) s u v →
        MSubRedStar (head₁ :: head₂ :: { bound := new, kind := .equ } :: Γ) s u v)
    (hApp :
      ∀ {op op' arg : Term},
        MSubRed (head₁ :: head₂ :: { bound := old, kind := .equ } :: Γ)
          (arg :: s) op op' →
    ...
    (hFun :
      ∀ {bound bound' body body' : Term},
        ...
        MSubRed ({ bound := bound, kind := .sub } :: head₁ :: head₂ ::
          { bound := old, kind := .equ } :: Γ) [] body body' →
        MSubRedStar (head₁ :: head₂ :: { bound := new, kind := .equ } :: Γ)
          [] (.abs bound body) (.abs bound' body'))
    (hFOp :
      ∀ {bound arg body body' : Term} {rest : Stack},
        Term.Scoped
        ...
        MSubRedStar (head₁ :: head₂ :: { bound := new, kind := .equ } :: Γ) s
          (.abs bound body) (.abs bound body'))
    (h : MSubRed (head₁ :: head₂ :: { bound := old, kind := .equ } :: Γ) s u v) :
    MSubRedStar (head₁ :: head₂ :: { bound := new, kind := .equ } :: Γ) s u v := by
  have hpvNew :
      PrevalidExt (head₁ :: head₂ :: { bound := new, kind := .equ } :: Γ) s :=
    PrevalidExt.equ_under_two_heads_replace hpv hnew
  cases h with
...
/-- Consume a raw subtype-reduction chain when the changed `.equ` entry sits
under two preserved heads and every old-context step has a raw-subtype-chain
replacement in the new context. -/
theorem msubRedStar_equ_under_two_heads_replace_from_handlers_star {Γ : Ctx}
    {s : Stack} {head₁ head₂ : CtxEntry} {old new u v : Term}
    (hpv : PrevalidExt (head₁ :: head₂ :: { bound := old, kind := .equ } :: Γ) s)
    (hnew : Term.Scoped Γ.depth new)
    (hPro :
      ∀ {i : Nat} {t : Term},
    ...
    (hFOp :
      ∀ {bound arg body body' : Term} {rest : Stack},
        ...
        MSubRedStar (head₁ :: head₂ :: { bound := new, kind := .equ } :: Γ) s
          (.abs bound body) (.abs bound body'))
    (h : MSubRedStar (head₁ :: head₂ :: { bound := old, kind := .equ } :: Γ) s u v) :
    MSubRedStar (head₁ :: head₂ :: { bound := new, kind := .equ } :: Γ) s u v := by
  induction h with
  | refl =>
    exact Relation.ReflTransGen.refl
  | @tail mid v hStar hLast ih =>
    exact MSubRedStar.trans ih
      (msubRedStar_equ_under_two_heads_replace_from_handlers hpv hnew hPro hEq hApp
        hFun hFOp hLast.some)

/-- Raw subtype-star replacement under two preserved heads, where recursive
constructor replacements already return raw subtype chains. -/
theorem msubRedStar_equ_under_two_heads_replace_from_raw_body_replacements
    {Γ : Ctx} {s : Stack} {head₁ head₂ : CtxEntry} {old new u v : Term}
    (hpv : PrevalidExt (head₁ :: head₂ :: { bound := old, kind := .equ } :: Γ) s)
    (hnew : Term.Scoped Γ.depth new)
    (hEq :
      ∀ {u v : Term},
    ...
    (hAppOpReplace :
      ∀ {op op' arg : Term},
        MSubRed (head₁ :: head₂ :: { bound := old, kind := .equ } :: Γ)
          (arg :: s) op op' →
        Term.Scoped
          (Ctx.depth (head₁ :: head₂ :: { bound := old, kind := .equ } :: Γ)) arg →
        MSubRedStar (head₁ :: head₂ :: { bound := new, kind := .equ } :: Γ)
          (arg :: s) op op')
    (hFunBoundReplace :
      ∀ {bound bound' : Term},
        MEqRed (head₁ :: head₂ :: { bound := old, kind := .equ } :: Γ) []
          bound bound' →
        MEqRed (head₁ :: head₂ :: { bound := new, kind := .equ } :: Γ) []
          bound bound')
    (hFunBodyReplace :
      ∀ {bound body body' : Term},
        MSubRed ({ bound := bound, kind := .sub } :: head₁ :: head₂ ::
          { bound := old, kind := .equ } :: Γ) [] body body' →
        MSubRedStar ({ bound := bound, kind := .sub } :: head₁ :: head₂ ::
          { bound := new, kind := .equ } :: Γ) [] body body')
    (hFOpBodyReplace :
      ∀ {arg body body' : Term} {rest : Stack},
        Term.Scoped
        ...
        MSubRed ({ bound := arg, kind := .equ } :: head₁ :: head₂ ::
          { bound := old, kind := .equ } :: Γ) (Stack.shift 0 rest) body body' →
        MSubRedStar ({ bound := arg, kind := .equ } :: head₁ :: head₂ ::
          { bound := new, kind := .equ } :: Γ) (Stack.shift 0 rest) body body')
    (h : MSubRedStar (head₁ :: head₂ :: { bound := old, kind := .equ } :: Γ) s u v) :
    MSubRedStar (head₁ :: head₂ :: { bound := new, kind := .equ } :: Γ) s u v :=
  msubRedStar_equ_under_two_heads_replace_from_handlers_star hpv hnew
    (fun hb => MSubRedStar.single (MSubRed.pro_equ_under_two_heads_replace hpv hnew hb))
    hEq
    (by
      intro op op' arg hOp hArgScoped
      exact msubRedStar_app_fixed_arg
    ...
    (by
      ...
      have hpvNew : PrevalidExt
          (head₁ :: head₂ :: { bound := new, kind := .equ } :: Γ) [] :=
        PrevalidExt.equ_under_two_heads_replace
          (PrevalidExt.nil (PrevalidExt.ctx hpv)) hnew
      exact msubRedStar_abs_fun_body_equ_bound hpvNew (hFunBoundReplace hBound)
        (by simpa [Ctx.depth] using hBody.scoped_left) (hFunBodyReplace hBody))
    (by
...
/-- Diagrammatic one-step subtype replacement across a changed `.equ` entry
at an arbitrary context index, presented by `Ctx.replaceAt`. The stable
`Ms-Pro` and `Ms-Top` leaves are rebuilt directly; equivalence and recursive
constructor cases are exposed as handlers. -/
theorem msubRed_replaceAt_equ_from_handlers {Γ : Ctx} {s : Stack}
    {cutoff : Nat} {old new u v : Term}
    (hpv : PrevalidExt (Ctx.replaceAt cutoff { bound := old, kind := .equ } Γ) s)
    (hcut : cutoff < Γ.depth)
    (hnew : CtxEntry.ScopedIn (List.drop (cutoff + 1) Γ)
  ...
  cases h with
  | pro _ hb =>
      exact MSubStar.of_MSubRed (MSubRed.pro_replaceAt_equ hpv hcut hnew hb)
  | top _ hu =>
      exact MSubStar.of_MSubRed (MSubRed.top hpvNew (by
        simpa [Ctx.depth_replaceAt] using hu))
  | equ _ heq =>
...
/-- Constructor-wired variant of
`msubRed_equ_under_sub_head_sub_tail_nil_replace_from_handlers`. The
`Ms-Equ` branch remains diagrammatic; application and function cases are
rebuilt from recursive replacements, while empty-stack `FOp` is impossible. -/
theorem msubRed_equ_under_sub_head_sub_tail_nil_replace_from_replacements
    {Γ : Ctx} {headBound tailBound old new u v : Term}
    (hpv : PrevalidExt ({ bound := headBound, kind := .sub } ::
      { bound := old, kind := .equ } :: { bound := tailBound, kind := .sub } ::
      Γ) [])
  ...
  have hpvNew : PrevalidExt ({ bound := headBound, kind := .sub } ::
    ...
    simpa [CtxEntry.ScopedIn, Ctx.depth] using
      PrevalidExt.replaceAt_equ_same
        ...
        (by simpa [CtxEntry.ScopedIn, Ctx.depth] using hnew)
  exact
    msubRed_equ_under_sub_head_sub_tail_nil_replace_from_handlers
      hpv hnew hEq
      (by
        intro op op' arg hOp hArgScoped
        exact msubStar_app_fixed_arg hpvNew hArgScoped
          (hAppOpReplace hOp hArgScoped))
      (by
        intro bound bound' body body' hBoundScoped hBound hBody
        exact MSubStar.of_MSubRed
          (MSubRed.fun_ hBoundScoped (hFunBoundReplace hBound)
            (hFunBodyReplace hBody)))
      h

/-- Constructor-wired `{sub}, {equ}, {sub}` subtype replacement where the
recursive `Ms-Fun` body replacement is already diagrammatic. This is the
chain-aware form needed by nested structural-app body residuals. -/
theorem msubRed_equ_under_sub_head_sub_tail_nil_replace_from_body_replacements
    {Γ : Ctx} {headBound tailBound old new u v : Term}
    (hpv : PrevalidExt ({ bound := headBound, kind := .sub } ::
      { bound := old, kind := .equ } :: { bound := tailBound, kind := .sub } ::
      Γ) [])
  ...
  exact
    msubRed_equ_under_sub_head_sub_tail_nil_replace_from_handlers
      ...
      (by
        intro bound bound' body body' hBoundScoped hBound hBody
        exact msubStar_abs_fun_body_equ_bound hpvNew
          (hFunBoundReplace hBound) hBody.scoped_left (hFunBodyReplace hBody))
      h
...
theorem msub_equ_under_two_heads_app_handler_of_operator_replacement {Γ : Ctx}
    ...
    (hOpReplace :
      ∀ {op op' arg : Term},
        ...
        MSubStar (head₁ :: head₂ :: { bound := new, kind := .equ } :: Γ)
          (arg :: s) op op')
    {op op' arg : Term}
    (hOp : MSubRed (head₁ :: head₂ :: { bound := old, kind := .equ } :: Γ)
      (arg :: s) op op')
    (hArgScoped :
      Term.Scoped
        (Ctx.depth (head₁ :: head₂ :: { bound := old, kind := .equ } :: Γ)) arg) :
    MSubStar (head₁ :: head₂ :: { bound := new, kind := .equ } :: Γ) s
      (.app op arg) (.app op' arg) :=
  msubStar_app_fixed_arg (PrevalidExt.equ_under_two_heads_replace hpv hnew)
    (by simpa [Ctx.depth] using hArgScoped) (hOpReplace hOp hArgScoped)

/-- Canonical `Ms-Fun` handler for two-preserved-head `.equ` replacement when
the bound equivalence and body subtype premises have already been replaced as
raw one-step reductions. -/
theorem msub_equ_under_two_heads_fun_handler_of_raw_replacements {Γ : Ctx}
    ...
    (hBoundScoped :
      Term.Scoped
        (Ctx.depth (head₁ :: head₂ :: { bound := old, kind := .equ } :: Γ))
        bound)
    (hBound : MEqRed (head₁ :: head₂ :: { bound := old, kind := .equ } :: Γ) []
      bound bound')
    (hBody : MSubRed ({ bound := bound, kind := .sub } :: head₁ :: head₂ ::
      { bound := old, kind := .equ } :: Γ) [] body body') :
    MSubStar (head₁ :: head₂ :: { bound := new, kind := .equ } :: Γ) []
      (.abs bound body) (.abs bound' body') :=
  MSubStar.of_MSubRed
    (MSubRed.fun_ (by simpa [Ctx.depth] using hBoundScoped)
      (hBoundReplace hBound) (hBodyReplace hBody))

/-- Canonical `Ms-FOp` handler for two-preserved-head `.equ` replacement. -/
theorem msub_equ_under_two_heads_fop_handler_of_body_replacement {Γ : Ctx}
    {s : Stack} {head₁ head₂ : CtxEntry} {old new : Term}
    (hpvTail : PrevalidExt (head₁ :: head₂ :: { bound := old, kind := .equ } :: Γ) s)
    (hnew : Term.Scoped Γ.depth new)
    (hBodyReplace :
      ∀ {arg body body' : Term} {rest : Stack},
        Term.Scoped
        ...
        MSubRed ({ bound := arg, kind := .equ } :: head₁ :: head₂ ::
          { bound := old, kind := .equ } :: Γ) (Stack.shift 0 rest) body body' →
        MSubStar ({ bound := arg, kind := .equ } :: head₁ :: head₂ ::
          { bound := new, kind := .equ } :: Γ) (Stack.shift 0 rest) body body')
    {bound arg body body' : Term} {rest : Stack}
    (hBoundScoped :
      Term.Scoped
    ...
    (hArgScoped :
      Term.Scoped
        (Ctx.depth (head₁ :: head₂ :: { bound := old, kind := .equ } :: Γ)) arg)
    (hStack : s = arg :: rest)
    (hBody : MSubRed ({ bound := arg, kind := .equ } :: head₁ :: head₂ ::
      { bound := old, kind := .equ } :: Γ) (Stack.shift 0 rest) body body') :
    MSubStar (head₁ :: head₂ :: { bound := new, kind := .equ } :: Γ) s
      (.abs bound body) (.abs bound body') := by
  subst hStack
  have hpvNewTail :
      PrevalidExt (head₁ :: head₂ :: { bound := new, kind := .equ } :: Γ) rest :=
    PrevalidExt.equ_under_two_heads_replace (PrevalidExt.tail hpvTail) hnew
  exact msubStar_abs_fOp_body_fixed_bound hpvNewTail
    (by simpa [Ctx.depth] using hBoundScoped)
    (by simpa [Ctx.depth] using hArgScoped)
    (hBodyReplace hArgScoped rfl hBody)

/-- Subtype replacement under two preserved heads, with stable leaves and the
canonical constructor handlers wired in. The `Ms-Equ` branch remains an
explicit diagrammatic replacement premise. -/
theorem msubRed_equ_under_two_heads_replace_from_replacements {Γ : Ctx} {s : Stack}
    {head₁ head₂ : CtxEntry} {old new u v : Term}
    (hpv : PrevalidExt (head₁ :: head₂ :: { bound := old, kind := .equ } :: Γ) s)
    (hnew : Term.Scoped Γ.depth new)
    (hEq :
    ...
    (h : MSubRed (head₁ :: head₂ :: { bound := old, kind := .equ } :: Γ) s u v) :
    MSubStar (head₁ :: head₂ :: { bound := new, kind := .equ } :: Γ) s u v :=
  msubRed_equ_under_two_heads_replace_from_handlers hpv hnew
    (fun hb => MSubStar.of_MSubRed
      (MSubRed.pro_equ_under_two_heads_replace hpv hnew hb))
    hEq
    (msub_equ_under_two_heads_app_handler_of_operator_replacement hpv hnew
      hAppOpReplace)
    (msub_equ_under_two_heads_fun_handler_of_raw_replacements
      hFunBoundReplace hFunBodyReplace)
    (msub_equ_under_two_heads_fop_handler_of_body_replacement hpv hnew
      hFOpBodyReplace)
    h

/-- Subtype replacement under three preserved heads, with stable leaves and
the canonical constructor handlers wired in. The `Ms-Equ` branch remains an
explicit diagrammatic replacement premise. -/
theorem msubRed_equ_under_three_heads_replace_from_replacements {Γ : Ctx} {s : Stack}
    {head₁ head₂ head₃ : CtxEntry} {old new u v : Term}
    (hpv :
      PrevalidExt (head₁ :: head₂ :: head₃ :: { bound := old, kind := .equ } :: Γ) s)
    (hnew : Term.Scoped Γ.depth new)
    ...
    MSubStar (head₁ :: head₂ :: head₃ :: { bound := new, kind := .equ } :: Γ)
      s u v :=
  msubRed_equ_under_three_heads_replace_from_handlers hpv hnew
    (fun hb => MSubStar.of_MSubRed
      (MSubRed.pro_equ_under_three_heads_replace hpv hnew hb))
    hEq
    (by
    ...
    (by
      ...
      have hpvNewTail :
        ...
        PrevalidExt.equ_under_three_heads_replace (PrevalidExt.tail hpv) hnew
      exact msubStar_abs_fOp_body_fixed_bound hpvNewTail
        (by simpa [Ctx.depth] using hBoundScoped)
        (by simpa [Ctx.depth] using hArgScoped)
        (hFOpBodyReplace hArgScoped rfl hBody))
...
/-- Star-level diagrammatic equivalence replacement for the empty-stack
`{sub}, {equ}, {sub}` shape. This composes the one-step splitter over an
equivalence-reduction chain. -/
theorem meqRedStar_equ_under_sub_head_sub_tail_nil_replace_from_handlers
    {Γ : Ctx} {headBound tailBound old new u v : Term}
    (hpv : PrevalidExt ({ bound := headBound, kind := .sub } ::
      { bound := old, kind := .equ } :: { bound := tailBound, kind := .sub } ::
      Γ) [])
    ...
    (hBet :
      ∀ {bound arg arg' body body' : Term},
        ...
        MSubStar ({ bound := headBound, kind := .sub } ::
          { bound := new, kind := .equ } :: { bound := tailBound, kind := .sub } ::
          Γ) [] (.app (.abs bound body) arg) (Term.instantiate 0 arg' body'))
    (h : MEqRedStar ({ bound := headBound, kind := .sub } ::
      { bound := old, kind := .equ } :: { bound := tailBound, kind := .sub } ::
      Γ) [] u v) :
    MSubStar ({ bound := headBound, kind := .sub } ::
      { bound := new, kind := .equ } :: { bound := tailBound, kind := .sub } ::
  ...
  meqRedStar_replace_from_step_replacement
    (meqRed_equ_under_sub_head_sub_tail_nil_replace_from_handlers
      hpv hnew hProOne hProTail hApp hFun hBet)
    h

/-- Star-level diagrammatic subtype replacement for the empty-stack
`{sub}, {equ}, {sub}` shape. This composes the one-step splitter over a
subtype-reduction chain. -/
theorem msubRedStar_equ_under_sub_head_sub_tail_nil_replace_from_handlers
    {Γ : Ctx} {headBound tailBound old new u v : Term}
    (hpv : PrevalidExt ({ bound := headBound, kind := .sub } ::
      { bound := old, kind := .equ } :: { bound := tailBound, kind := .sub } ::
      Γ) [])
  ...
  msubRedStar_replace_from_step_replacement
    (msubRed_equ_under_sub_head_sub_tail_nil_replace_from_handlers
      hpv hnew hEq hApp hFun)
    h

/-- Star-level constructor-wired equivalence replacement for the empty-stack
`{sub}, {equ}, {sub}` shape. This composes the one-step replacement wrapper
over an equivalence-reduction chain. -/
theorem meqRedStar_equ_under_sub_head_sub_tail_nil_replace_from_replacements
    {Γ : Ctx} {headBound tailBound old new u v : Term}
    (hpv : PrevalidExt ({ bound := headBound, kind := .sub } ::
      { bound := old, kind := .equ } :: { bound := tailBound, kind := .sub } ::
      Γ) [])
    ...
    MSubStar ({ bound := headBound, kind := .sub } ::
      ...
      Γ) [] u v :=
  meqRedStar_replace_from_step_replacement
    (meqRed_equ_under_sub_head_sub_tail_nil_replace_from_replacements
      hpv hnew hProOne hProTail hAppOpReplace hNilReplace hFunBodyReplace
      hBetBodyReplace)
    h

/-- Star-level constructor-wired subtype replacement for the empty-stack
`{sub}, {equ}, {sub}` shape. This composes the one-step replacement wrapper
over a subtype-reduction chain. -/
theorem msubRedStar_equ_under_sub_head_sub_tail_nil_replace_from_replacements
    {Γ : Ctx} {headBound tailBound old new u v : Term}
    (hpv : PrevalidExt ({ bound := headBound, kind := .sub } ::
      { bound := old, kind := .equ } :: { bound := tailBound, kind := .sub } ::
      Γ) [])
...
theorem meqRedStar_replaceAt_equ_from_replacements {Γ : Ctx} {s : Stack}
    {cutoff : Nat} {old new u v : Term}
    (hpv : PrevalidExt (Ctx.replaceAt cutoff { bound := old, kind := .equ } Γ) s)
    (hcut : cutoff < Γ.depth)
    (hnew : CtxEntry.ScopedIn (List.drop (cutoff + 1) Γ)
    ...
    (hFOpBodyReplace :
      ∀ {arg body body' : Term} {rest : Stack},
        ...
        MSubStar ({ bound := arg, kind := .equ } ::
          Ctx.replaceAt cutoff { bound := new, kind := .equ } Γ)
          (Stack.shift 0 rest) body body')
    (h : MEqRedStar (Ctx.replaceAt cutoff { bound := old, kind := .equ } Γ) s u v) :
    MSubStar (Ctx.replaceAt cutoff { bound := new, kind := .equ } Γ) s u v :=
  meqRedStar_replace_from_step_replacement
    (meqRed_replaceAt_equ_from_replacements hpv hcut hnew hPro
      hAppOpReplace hNilReplace hFunBodyReplace hBetBodyReplace hFOpBodyReplace)
    h

/-- Star-level subtype replacement across a changed `.equ` entry at an
arbitrary context index, obtained by composing the one-step `Ctx.replaceAt`
wrapper over a subtype-reduction chain. -/
theorem msubRedStar_replaceAt_equ_from_replacements {Γ : Ctx} {s : Stack}
    {cutoff : Nat} {old new u v : Term}
    (hpv : PrevalidExt (Ctx.replaceAt cutoff { bound := old, kind := .equ } Γ) s)
    (hcut : cutoff < Γ.depth)
    (hnew : CtxEntry.ScopedIn (List.drop (cutoff + 1) Γ)
    ...
    (hFOpBodyReplace :
      ∀ {arg body body' : Term} {rest : Stack},
        ...
        MSubStar ({ bound := arg, kind := .equ } ::
          Ctx.replaceAt cutoff { bound := new, kind := .equ } Γ)
          (Stack.shift 0 rest) body body')
    (h : MSubRedStar (Ctx.replaceAt cutoff { bound := old, kind := .equ } Γ) s u v) :
    MSubStar (Ctx.replaceAt cutoff { bound := new, kind := .equ } Γ) s u v :=
  msubRedStar_replace_from_step_replacement
    (msubRed_replaceAt_equ_from_replacements hpv hcut hnew hEq
...
/-- Raw subtype-star replacement across an innermost changed `.equ` head,
where recursive constructor replacements already return raw subtype chains. -/
theorem msubRedStar_equ_head_replace_from_raw_body_replacements {Γ : Ctx} {s : Stack}
    {old new u v : Term}
    (hpv : PrevalidExt ({ bound := old, kind := .equ } :: Γ) s)
    (hnew : Term.Scoped Γ.depth new)
    (hEq :
    ...
    (hAppOpReplace :
      ∀ {op op' arg : Term},
        MSubRed ({ bound := old, kind := .equ } :: Γ) (arg :: s) op op' →
        Term.Scoped (Ctx.depth ({ bound := old, kind := .equ } :: Γ)) arg →
        MSubRedStar ({ bound := new, kind := .equ } :: Γ) (arg :: s) op op')
    (hFunBoundReplace :
      ∀ {bound bound' : Term},
        MEqRed ({ bound := old, kind := .equ } :: Γ) [] bound bound' →
        MEqRed ({ bound := new, kind := .equ } :: Γ) [] bound bound')
    (hFunBodyReplace :
      ∀ {bound body body' : Term},
        MSubRed ({ bound := bound, kind := .sub } ::
          { bound := old, kind := .equ } :: Γ) [] body body' →
        MSubRedStar ({ bound := bound, kind := .sub } ::
          { bound := new, kind := .equ } :: Γ) [] body body')
    (hFOpBodyReplace :
      ∀ {arg body body' : Term} {rest : Stack},
        Term.Scoped (Ctx.depth ({ bound := old, kind := .equ } :: Γ)) arg →
        ...
        MSubRed ({ bound := arg, kind := .equ } ::
          { bound := old, kind := .equ } :: Γ) (Stack.shift 0 rest) body body' →
        MSubRedStar ({ bound := arg, kind := .equ } ::
          { bound := new, kind := .equ } :: Γ) (Stack.shift 0 rest) body body')
    (h : MSubRedStar ({ bound := old, kind := .equ } :: Γ) s u v) :
    MSubRedStar ({ bound := new, kind := .equ } :: Γ) s u v :=
  msubRedStar_equ_head_replace_from_handlers_star hpv hnew hEq
    (by
      intro op op' arg hOp hArgScoped
      exact msubRedStar_app_fixed_arg
        (by simpa [Ctx.depth] using hArgScoped)
    ...
    (by
      intro bound bound' body body' _hBoundScoped hBound hBody
      have hpvNew : PrevalidExt ({ bound := new, kind := .equ } :: Γ) [] :=
        PrevalidExt.equ_head_replace (PrevalidExt.nil (PrevalidExt.ctx hpv)) hnew
      exact msubRedStar_abs_fun_body_equ_bound hpvNew (hFunBoundReplace hBound)
        (by simpa [Ctx.depth] using hBody.scoped_left) (hFunBodyReplace hBody))
    (by
      intro bound arg body body' rest hBoundScoped hArgScoped hStack hBody
...
/-- Star-level subtype replacement when the changed `.equ` entry sits under
two preserved heads. -/
theorem msubRedStar_equ_under_two_heads_replace_from_replacements
    {Γ : Ctx} {s : Stack} {head₁ head₂ : CtxEntry} {old new u v : Term}
    (hpv : PrevalidExt (head₁ :: head₂ :: { bound := old, kind := .equ } :: Γ) s)
    (hnew : Term.Scoped Γ.depth new)
    (hEq :
    ...
    MSubStar (head₁ :: head₂ :: { bound := new, kind := .equ } :: Γ) s u v :=
  msubRedStar_replace_from_step_replacement
    (msubRed_equ_under_two_heads_replace_from_replacements hpv hnew hEq
      hAppOpReplace hFunBoundReplace hFunBodyReplace hFOpBodyReplace)
    h

/-- Star-level subtype replacement when the changed `.equ` entry sits under
three preserved heads. -/
theorem msubRedStar_equ_under_three_heads_replace_from_replacements
    {Γ : Ctx} {s : Stack} {head₁ head₂ head₃ : CtxEntry} {old new u v : Term}
    (hpv :
      PrevalidExt (head₁ :: head₂ :: head₃ :: { bound := old, kind := .equ } :: Γ) s)
    (hnew : Term.Scoped Γ.depth new)
    ...
    MSubStar (head₁ :: head₂ :: head₃ :: { bound := new, kind := .equ } :: Γ)
      s u v :=
  msubRedStar_replace_from_step_replacement
    (msubRed_equ_under_three_heads_replace_from_replacements hpv hnew hEq
      hAppOpReplace hFunBoundReplace hFunBodyReplace hFOpBodyReplace)
    h

/-- Star-level subtype replacement across an innermost changed `.equ` head,
where the recursive `Ms-Fun` body replacement is already diagrammatic. -/
theorem msubRedStar_equ_head_replace_from_body_replacements {Γ : Ctx} {s : Stack}
    {old new u v : Term}
    (hpv : PrevalidExt ({ bound := old, kind := .equ } :: Γ) s)
    (hnew : Term.Scoped Γ.depth new)
    (hEq :
    ...
    MSubStar ({ bound := new, kind := .equ } :: Γ) s u v :=
  msubRedStar_replace_from_step_replacement
    (msubRed_equ_head_replace_from_body_replacements hpv hnew hEq hAppOpReplace
      hFunBoundReplace hFunBodyReplace hFOpBodyReplace)
    h
...
a preserved head and the recursive `Ms-Fun` body replacement is already
diagrammatic. -/
theorem msubRedStar_equ_under_head_replace_from_body_replacements {Γ : Ctx} {s : Stack}
    {head : CtxEntry} {old new u v : Term}
    (hpv : PrevalidExt (head :: { bound := old, kind := .equ } :: Γ) s)
    (hnew : Term.Scoped Γ.depth new)
    (hEq :
...
/-- Function-valued raw subtype-star replacement across an innermost changed
`.equ` head. This packages the raw-chain body replacement wrapper for every
residual stack. -/
theorem msubRedStar_equ_head_replace_function_from_raw_body_replacements {Γ : Ctx}
    {old new u v : Term}
    (hnew : Term.Scoped Γ.depth new)
    (hEq :
      ∀ {s : Stack},
    ...
    ∀ {s : Stack},
      ...
      MSubRedStar ({ bound := new, kind := .equ } :: Γ) s u v :=
  fun hpv =>
    msubRedStar_equ_head_replace_from_raw_body_replacements hpv hnew (hEq hpv)
      (hAppOpReplace hpv) hFunBoundReplace hFunBodyReplace
      (hFOpBodyReplace hpv) (h hpv)

/-- Function-valued raw subtype-star replacement when the changed `.equ`
entry sits under a preserved head. This packages the raw-chain under-head
body replacement wrapper for every residual stack. -/
theorem msubRedStar_equ_under_head_replace_function_from_raw_body_replacements
    ...
    ∀ {s : Stack},
      ...
      MSubRedStar (head :: { bound := new, kind := .equ } :: Γ) s u v :=
  fun hpv =>
    msubRedStar_equ_under_head_replace_from_raw_body_replacements hpv hnew (hEq hpv)
      (hAppOpReplace hpv) hFunBoundReplace hFunBodyReplace
      (hFOpBodyReplace hpv) (h hpv)

/-- Function-valued raw subtype-star replacement when the changed `.equ`
entry sits under two preserved heads. This packages the two-head raw-chain
body replacement wrapper for every residual stack. -/
theorem msubRedStar_equ_under_two_heads_replace_function_from_raw_body_replacements
    {Γ : Ctx} {head₁ head₂ : CtxEntry} {old new u v : Term}
    (hnew : Term.Scoped Γ.depth new)
    (hEq :
      ∀ {s : Stack},
    ...
    (h :
      ∀ {s : Stack},
        PrevalidExt (head₁ :: head₂ :: { bound := old, kind := .equ } :: Γ) s →
        MSubRedStar (head₁ :: head₂ :: { bound := old, kind := .equ } :: Γ) s u v) :
    ∀ {s : Stack},
      PrevalidExt (head₁ :: head₂ :: { bound := old, kind := .equ } :: Γ) s →
      MSubRedStar (head₁ :: head₂ :: { bound := new, kind := .equ } :: Γ) s u v :=
  fun hpv =>
    msubRedStar_equ_under_two_heads_replace_from_raw_body_replacements hpv hnew
...
theorem msubRedStar_equ_under_two_heads_replace_function_from_replacements
    {Γ : Ctx} {head₁ head₂ : CtxEntry} {old new u v : Term}
    (hnew : Term.Scoped Γ.depth new)
    (hEq :
      ∀ {s : Stack},
    ...
    ∀ {s : Stack},
      ...
      MSubStar (head₁ :: head₂ :: { bound := new, kind := .equ } :: Γ) s u v :=
  fun hpv =>
    msubRedStar_equ_under_two_heads_replace_from_replacements hpv hnew
      (hEq hpv) (hAppOpReplace hpv) hFunBoundReplace hFunBodyReplace
      (hFOpBodyReplace hpv) (h hpv)

/-- Function-valued star-level subtype replacement when the changed `.equ`
entry sits under three preserved heads. This packages the diagrammatic
three-head replacement wrapper for every residual stack. -/
theorem msubRedStar_equ_under_three_heads_replace_function_from_replacements
    {Γ : Ctx} {head₁ head₂ head₃ : CtxEntry} {old new u v : Term}
    (hnew : Term.Scoped Γ.depth new)
    (hEq :
      ∀ {s : Stack},
    ...
    ∀ {s : Stack},
      ...
      MSubStar (head₁ :: head₂ :: head₃ :: { bound := new, kind := .equ } :: Γ)
        s u v :=
  fun hpv =>
    msubRedStar_equ_under_three_heads_replace_from_replacements hpv hnew
      (hEq hpv) (hAppOpReplace hpv) hFunBoundReplace hFunBodyReplace
      (hFOpBodyReplace hpv) (h hpv)
...
two-preserved-head `.equ` replacement. -/
theorem meq_equ_under_two_heads_pro_zero_handler_of_replacement {Γ : Ctx} {s : Stack}
    {head₁ head₂ : CtxEntry} {old new : Term}
    (hpv : PrevalidExt (head₁ :: head₂ :: { bound := old, kind := .equ } :: Γ) s)
    (hNewScoped : Term.Scoped Γ.depth new)
    (hReplace :
      ∀ {α α' : Term},
        MEqRed (head₁ :: head₂ :: { bound := old, kind := .equ } :: Γ) s α α' →
        MSubStar (head₁ :: head₂ :: { bound := new, kind := .equ } :: Γ) s α α')
    {α α' : Term}
    (hb : Ctx.equBinds (head₁ :: head₂ :: { bound := old, kind := .equ } :: Γ) 0 α)
    (hα : MEqRed (head₁ :: head₂ :: { bound := old, kind := .equ } :: Γ) s α α') :
    MSubStar (head₁ :: head₂ :: { bound := new, kind := .equ } :: Γ) s
      (.bvar 0) α' :=
  MSubStar.trans
    (MSubStar.of_MEqRed (PrevalidExt.equ_under_two_heads_replace hpv hNewScoped)
      (MEqRed.pro (PrevalidExt.equ_under_two_heads_replace hpv hNewScoped)
        (by simpa [Ctx.equBinds] using hb)
        (MEqRed.refl (PrevalidExt.equ_under_two_heads_replace hpv hNewScoped)
          (by simpa [Ctx.depth] using hα.scoped_left))))
    (hReplace hα)

/-- Canonical handler for preserved-head index `1` `Me-Pro` during
two-preserved-head `.equ` replacement. -/
theorem meq_equ_under_two_heads_pro_one_handler_of_replacement {Γ : Ctx} {s : Stack}
    {head₁ head₂ : CtxEntry} {old new : Term}
    (hpv : PrevalidExt (head₁ :: head₂ :: { bound := old, kind := .equ } :: Γ) s)
    (hNewScoped : Term.Scoped Γ.depth new)
    (hReplace :
      ∀ {α α' : Term},
        ...
        MSubStar (head₁ :: head₂ :: { bound := new, kind := .equ } :: Γ) s α α')
    {α α' : Term}
    (hb : Ctx.equBinds (head₁ :: head₂ :: { bound := old, kind := .equ } :: Γ) 1 α)
    (hα : MEqRed (head₁ :: head₂ :: { bound := old, kind := .equ } :: Γ) s α α') :
    MSubStar (head₁ :: head₂ :: { bound := new, kind := .equ } :: Γ) s
      (.bvar 1) α' :=
  MSubStar.trans
...
/-- Canonical handler for true tail indices `3+` of `Me-Pro` during
two-preserved-head `.equ` replacement. -/
theorem meq_equ_under_two_heads_pro_tail_handler_of_replacement {Γ : Ctx} {s : Stack}
    {head₁ head₂ : CtxEntry} {old new : Term}
    (hpv : PrevalidExt (head₁ :: head₂ :: { bound := old, kind := .equ } :: Γ) s)
    (hNewScoped : Term.Scoped Γ.depth new)
    (hReplace :
      ∀ {α α' : Term},
        ...
        MSubStar (head₁ :: head₂ :: { bound := new, kind := .equ } :: Γ) s α α')
    {i : Nat} {α α' : Term}
    (hb : Ctx.equBinds (head₁ :: head₂ :: { bound := old, kind := .equ } :: Γ)
      (((i + 1) + 1) + 1) α)
    (hα : MEqRed (head₁ :: head₂ :: { bound := old, kind := .equ } :: Γ) s α α') :
    MSubStar (head₁ :: head₂ :: { bound := new, kind := .equ } :: Γ) s
      (.bvar (((i + 1) + 1) + 1)) α' :=
...
/-- Under three preserved heads and a changed `.equ` entry, the old shifted
under-under-under-head bound diagrammatically subtypes the new variable at
index `3`. This is the three-level analogue of
`msub_equ_under_two_heads_old_bound_to_new_bvar2`. -/
theorem msub_equ_under_three_heads_old_bound_to_new_bvar3 {Γ : Ctx} {s : Stack}
    ...
    MSub (head₁ :: head₂ :: head₃ :: { bound := new, kind := .equ } :: Γ) s
      (Term.shift 0 (Term.shift 0 (Term.shift 0 (Term.shift 0 old)))) (.bvar 3) := by
  have hNewShiftScoped :
      Term.Scoped
        (Ctx.depth (head₁ :: head₂ :: head₃ :: { bound := new, kind := .equ } :: Γ))
        (Term.shift 0 (Term.shift 0 (Term.shift 0 (Term.shift 0 new)))) := by
    have hOnce : Term.Scoped (Γ.depth + 1) (Term.shift 0 new) :=
      Term.shift_scoped 0 Γ.depth new (Nat.zero_le _) hNewScoped
    have hTwice : Term.Scoped ((Γ.depth + 1) + 1)
        (Term.shift 0 (Term.shift 0 new)) :=
    ...
    have hThrice : Term.Scoped (((Γ.depth + 1) + 1) + 1)
      ...
      Term.shift_scoped 0 ((Γ.depth + 1) + 1) (Term.shift 0 (Term.shift 0 new))
        (Nat.zero_le _) hTwice
    have hFour : Term.Scoped ((((Γ.depth + 1) + 1) + 1) + 1)
        (Term.shift 0 (Term.shift 0 (Term.shift 0 (Term.shift 0 new)))) :=
      Term.shift_scoped 0 (((Γ.depth + 1) + 1) + 1)
        (Term.shift 0 (Term.shift 0 (Term.shift 0 new))) (Nat.zero_le _) hThrice
    simpa [Ctx.depth, Nat.add_assoc, Nat.add_comm, Nat.add_left_comm] using hFour
  have hNewRefl :
      MEqRed (head₁ :: head₂ :: head₃ :: { bound := new, kind := .equ } :: Γ) s
        (Term.shift 0 (Term.shift 0 (Term.shift 0 (Term.shift 0 new))))
        (Term.shift 0 (Term.shift 0 (Term.shift 0 (Term.shift 0 new)))) :=
  ...
  have hVarNew :
      MEqRed (head₁ :: head₂ :: head₃ :: { bound := new, kind := .equ } :: Γ) s
        (.bvar 3) (Term.shift 0 (Term.shift 0 (Term.shift 0 (Term.shift 0 new)))) :=
    MEqRed.pro hpvNew (by simp [Ctx.equBinds]) hNewRefl
  exact MSub.intro
    (MSubRedStar.single (MSubRed.equ hpvNew hOldNewStack))
    (MEqRedStar.single hVarNew)

/-- Converse three-preserved-head bridge: the new variable at index `3`
diagrammatically subtypes the old quadruply shifted changed-head bound. -/
theorem msub_equ_under_three_heads_new_bvar3_to_old_bound {Γ : Ctx} {s : Stack}
    {head₁ head₂ head₃ : CtxEntry} {old new : Term}
    (hpvNew :
      PrevalidExt (head₁ :: head₂ :: head₃ :: { bound := new, kind := .equ } :: Γ) s)
    (hNewScoped : Term.Scoped Γ.depth new)
    (hOldNewStack :
      MEqRed (head₁ :: head₂ :: head₃ :: { bound := new, kind := .equ } :: Γ) s
        ...
        (Term.shift 0 (Term.shift 0 (Term.shift 0 (Term.shift 0 new))))) :
    MSub (head₁ :: head₂ :: head₃ :: { bound := new, kind := .equ } :: Γ) s
      (.bvar 3) (Term.shift 0 (Term.shift 0 (Term.shift 0 (Term.shift 0 old)))) := by
  have hNewShiftScoped :
      Term.Scoped
        (Ctx.depth (head₁ :: head₂ :: head₃ :: { bound := new, kind := .equ } :: Γ))
        (Term.shift 0 (Term.shift 0 (Term.shift 0 (Term.shift 0 new)))) := by
...
/-- Consume the three-preserved-head `Me-Pro` replacement residual by joining
the new variable at index `3` back to the old quadruply shifted changed-head
bound, then following the recursively replaced residual chain. -/
theorem msubStar_equ_under_three_heads_new_bvar3_to_replaced_residual {Γ : Ctx}
    {s : Stack} {head₁ head₂ head₃ : CtxEntry} {old new target : Term}
    (hpvNew :
      PrevalidExt (head₁ :: head₂ :: head₃ :: { bound := new, kind := .equ } :: Γ) s)
    (hNewScoped : Term.Scoped Γ.depth new)
    (hOldNewStack :
    ...
    (hResidual :
      MSubStar (head₁ :: head₂ :: head₃ :: { bound := new, kind := .equ } :: Γ) s
        (Term.shift 0 (Term.shift 0 (Term.shift 0 (Term.shift 0 old)))) target) :
    MSubStar (head₁ :: head₂ :: head₃ :: { bound := new, kind := .equ } :: Γ) s
      (.bvar 3) target :=
  MSubStar.trans
    (MSub.to_star
      (msub_equ_under_three_heads_new_bvar3_to_old_bound hpvNew hNewScoped
        hOldNewStack))
    hResidual

/-- Canonical handler for the changed-entry `Me-Pro` residual at index `3`
under three preserved heads: expose the old quadruply shifted bound, replace
that residual recursively, then compose it with the converse `bvar 3` bridge.
-/
theorem meq_equ_under_three_heads_pro_three_handler_of_replacement {Γ : Ctx}
    ...
    (hReplace :
      ∀ {target : Term},
        MEqRed (head₁ :: head₂ :: head₃ :: { bound := old, kind := .equ } :: Γ) s
          (Term.shift 0 (Term.shift 0 (Term.shift 0 (Term.shift 0 old)))) target →
        MSubStar (head₁ :: head₂ :: head₃ :: { bound := new, kind := .equ } :: Γ) s
          (Term.shift 0 (Term.shift 0 (Term.shift 0 (Term.shift 0 old)))) target)
    {α α' : Term}
    ...
    MSubStar (head₁ :: head₂ :: head₃ :: { bound := new, kind := .equ } :: Γ) s
      (.bvar 3) α' := by
  have hαeq : α = Term.shift 0 (Term.shift 0 (Term.shift 0 (Term.shift 0 old))) := by
    have hraw : Term.shift 0 (Term.shift 0 (Term.shift 0 (Term.shift 0 old))) = α := by
      simpa [Ctx.equBinds] using hb
    exact hraw.symm
  subst hαeq
  exact msubStar_equ_under_three_heads_new_bvar3_to_replaced_residual
    hpvNew hNewScoped hOldNewStack (hReplace hα)
...
theorem meqRed_equ_under_two_heads_replace_with_pro_from_replacements {Γ : Ctx}
    {s : Stack} {head₁ head₂ : CtxEntry} {old new u v : Term}
    (hpvOld : PrevalidExt (head₁ :: head₂ :: { bound := old, kind := .equ } :: Γ) s)
    (hpvNew : PrevalidExt (head₁ :: head₂ :: { bound := new, kind := .equ } :: Γ) s)
    (hnew : Term.Scoped Γ.depth new)
    (hOldNewStack :
      MEqRed (head₁ :: head₂ :: { bound := new, kind := .equ } :: Γ) s
    ...
    (hNilReplace :
      ∀ {arg arg' : Term},
        MEqRed (head₁ :: head₂ :: { bound := old, kind := .equ } :: Γ) [] arg arg' →
        MEqRed (head₁ :: head₂ :: { bound := new, kind := .equ } :: Γ) [] arg arg')
    (hFunBodyReplace :
      ∀ {bound body body' : Term},
        MEqRed ({ bound := bound, kind := .sub } :: head₁ :: head₂ ::
          { bound := old, kind := .equ } :: Γ) [] body body' →
    ...
    (h : MEqRed (head₁ :: head₂ :: { bound := old, kind := .equ } :: Γ) s u v) :
    MSubStar (head₁ :: head₂ :: { bound := new, kind := .equ } :: Γ) s u v :=
  meqRed_equ_under_two_heads_replace_from_handlers hpvOld hnew
    (by
      intro i α α' hb hα
      cases i with
      | zero =>
          exact meq_equ_under_two_heads_pro_zero_handler_of_replacement hpvOld hnew
            (fun h => hSelfReplace h) hb hα
      | succ i =>
          cases i with
          | zero =>
              exact meq_equ_under_two_heads_pro_one_handler_of_replacement hpvOld hnew
                (fun h => hSelfReplace h) hb hα
          | succ i =>
              cases i with
              | zero =>
                  exact meq_equ_under_two_heads_pro_two_handler_of_replacement hpvNew hnew
                    hOldNewStack (fun h => hSelfReplace h) hb hα
              | succ i =>
                  exact meq_equ_under_two_heads_pro_tail_handler_of_replacement hpvOld
                    hnew (fun h => hSelfReplace h) hb hα)
    (by
      intro op op' arg arg' hOp hArg
      exact MSubStar.of_MEqRed (PrevalidExt.equ_under_two_heads_replace hpvOld hnew)
        (MEqRed.app (hAppOpReplace hOp) (hNilReplace hArg)))
    (by
      intro bound bound' body body' hBound hBody
      exact MSubStar.of_MEqRed
    ...
    (by
      ...
      have hpvNewTail :
          PrevalidExt (head₁ :: head₂ :: { bound := new, kind := .equ } :: Γ) rest :=
        PrevalidExt.equ_under_two_heads_replace (PrevalidExt.tail hpvOld) hnew
      exact msubStar_abs_fOp_equ_bound_body hpvNewTail (hNilReplace hBound)
        (by simpa [Ctx.depth] using hArgScoped)
        (by simpa [Ctx.depth] using hBody.scoped_left)
        (hFOpBodyReplace hArgScoped rfl hBody))
...
/-- Chain-level three-preserved-head subtype replacement whose `Ms-Equ`
branch is wired through the canonical three-head equivalence replacement with
all `Me-Pro` cases handled. -/
theorem msubRedStar_equ_under_three_heads_replace_with_pro_from_replacements
    {Γ : Ctx} {s : Stack} {head₁ head₂ head₃ : CtxEntry} {old new u v : Term}
    (hpvOld :
      PrevalidExt (head₁ :: head₂ :: head₃ :: { bound := old, kind := .equ } :: Γ) s)
    (hpvNew :
    ...
    MSubStar (head₁ :: head₂ :: head₃ :: { bound := new, kind := .equ } :: Γ)
      s u v :=
  msubRedStar_equ_under_three_heads_replace_from_replacements hpvOld hnew
    (meqRed_equ_under_three_heads_replace_with_pro_from_replacements hpvOld
      hpvNew hnew hOldNewStack hEqSelfReplace hEqAppOpReplace hNilReplace
      hEqFunBodyReplace hEqBetBodyReplace hEqFOpBodyReplace)
    hAppOpReplace hNilReplace hFunBodyReplace hFOpBodyReplace h

/-- Function-valued form of
`msubRedStar_equ_under_three_heads_replace_with_pro_from_replacements`,
packaging the subtype wrapper over every residual stack while deriving
new-context prevalidity locally. -/
theorem msubRedStar_equ_under_three_heads_replace_with_pro_function_from_replacements
    {Γ : Ctx} {head₁ head₂ head₃ : CtxEntry} {old new u v : Term}
    (hnew : Term.Scoped Γ.depth new)
    (hOldNewStack :
      ∀ {s : Stack},
    ...
    ∀ {s : Stack},
      ...
      MSubStar (head₁ :: head₂ :: head₃ :: { bound := new, kind := .equ } :: Γ)
        s u v :=
  fun hpvOld =>
    msubRedStar_equ_under_three_heads_replace_with_pro_from_replacements hpvOld
      (PrevalidExt.equ_under_three_heads_replace hpvOld hnew) hnew
...
theorem meqRedStar_equ_under_two_heads_replace_with_pro_from_replacements
    ...
    (hFOpBodyReplace :
      ∀ {arg body body' : Term} {rest : Stack},
        ...
        MSubStar ({ bound := arg, kind := .equ } :: head₁ :: head₂ ::
          { bound := new, kind := .equ } :: Γ) (Stack.shift 0 rest) body body')
    (h : MEqRedStar (head₁ :: head₂ :: { bound := old, kind := .equ } :: Γ) s u v) :
    MSubStar (head₁ :: head₂ :: { bound := new, kind := .equ } :: Γ) s u v :=
  meqRedStar_replace_from_step_replacement
    (meqRed_equ_under_two_heads_replace_with_pro_from_replacements hpvOld hpvNew
      hnew hOldNewStack hSelfReplace hAppOpReplace hNilReplace hFunBodyReplace
      hBetBodyReplace hFOpBodyReplace)
    h

/-- Function-valued chain-level two-preserved-head equivalence replacement
with all canonical `Me-Pro` cases wired. This packages
`meqRedStar_equ_under_two_heads_replace_with_pro_from_replacements` across
every residual stack, deriving new-context prevalidity from the old one. -/
theorem meqRedStar_equ_under_two_heads_replace_with_pro_function_from_replacements
    {Γ : Ctx} {head₁ head₂ : CtxEntry} {old new u v : Term}
    (hnew : Term.Scoped Γ.depth new)
    (hOldNewStack :
      ∀ {s : Stack},
    ...
    (h :
      ∀ {s : Stack},
        PrevalidExt (head₁ :: head₂ :: { bound := old, kind := .equ } :: Γ) s →
        MEqRedStar (head₁ :: head₂ :: { bound := old, kind := .equ } :: Γ) s u v) :
    ∀ {s : Stack},
      PrevalidExt (head₁ :: head₂ :: { bound := old, kind := .equ } :: Γ) s →
      MSubStar (head₁ :: head₂ :: { bound := new, kind := .equ } :: Γ) s u v :=
  fun hpvOld =>
    meqRedStar_equ_under_two_heads_replace_with_pro_from_replacements hpvOld
      (PrevalidExt.equ_under_two_heads_replace hpvOld hnew) hnew
      (hOldNewStack hpvOld) (hSelfReplace hpvOld) (hAppOpReplace hpvOld)
      hNilReplace hFunBodyReplace (hBetBodyReplace hpvOld)
      (hFOpBodyReplace hpvOld) (h hpvOld)

/-- Chain-level two-preserved-head subtype replacement whose `Ms-Equ` branch
is wired through the canonical two-head equivalence replacement with all
`Me-Pro` cases handled. -/
theorem msubRedStar_equ_under_two_heads_replace_with_pro_from_replacements
    {Γ : Ctx} {s : Stack} {head₁ head₂ : CtxEntry} {old new u v : Term}
    (hpvOld : PrevalidExt (head₁ :: head₂ :: { bound := old, kind := .equ } :: Γ) s)
    (hpvNew : PrevalidExt (head₁ :: head₂ :: { bound := new, kind := .equ } :: Γ) s)
    (hnew : Term.Scoped Γ.depth new)
    ...
    (h : MSubRedStar (head₁ :: head₂ :: { bound := old, kind := .equ } :: Γ) s u v) :
    MSubStar (head₁ :: head₂ :: { bound := new, kind := .equ } :: Γ) s u v :=
  msubRedStar_equ_under_two_heads_replace_from_replacements hpvOld hnew
    (meqRed_equ_under_two_heads_replace_with_pro_from_replacements hpvOld hpvNew
...
/-- Function-valued form of
`msubRedStar_equ_under_two_heads_replace_with_pro_from_replacements`,
packaging the subtype wrapper over every residual stack while deriving
new-context prevalidity locally. -/
theorem msubRedStar_equ_under_two_heads_replace_with_pro_function_from_replacements
    {Γ : Ctx} {head₁ head₂ : CtxEntry} {old new u v : Term}
    (hnew : Term.Scoped Γ.depth new)
    (hOldNewStack :
      ∀ {s : Stack},
    ...
    ∀ {s : Stack},
      ...
      MSubStar (head₁ :: head₂ :: { bound := new, kind := .equ } :: Γ) s u v :=
  fun hpvOld =>
    msubRedStar_equ_under_two_heads_replace_with_pro_from_replacements hpvOld
      (PrevalidExt.equ_under_two_heads_replace hpvOld hnew) hnew
      (hOldNewStack hpvOld) (hEqSelfReplace hpvOld)
      (hEqAppOpReplace hpvOld) hNilReplace hEqFunBodyReplace
      (hEqBetBodyReplace hpvOld) (hEqFOpBodyReplace hpvOld)
...
given a lift for each single subtype step. -/
theorem msubRedStar_equ_head_stack_lift_from_step_lift {Γ : Ctx} {s : Stack}
    {head u v : Term}
    (hStep :
      ∀ {a b : Term},
        MSubRed Γ [] a b →
        MSubRed ({ bound := head, kind := .equ } :: Γ) (Stack.shift 0 s)
          (Term.shift 0 a) (Term.shift 0 b))
    (h : MSubRedStar Γ [] u v) :
  ...
  | refl =>
    exact Relation.ReflTransGen.refl
  | @tail mid v hStar hLast ih =>
    exact Relation.ReflTransGen.trans ih (MSubRedStar.single (hStep hLast.some))

/-- Function-valued variant of
`msubRedStar_equ_head_stack_lift_from_step_lift`, for callers that can lift
each single subtype step under every residual tail stack. -/
theorem msubRedStar_equ_head_stack_lift_function_from_step_lift {Γ : Ctx}
    {head u v : Term}
    (hStep :
      ∀ {s : Stack},
        PrevalidExt Γ s →
...
an `FOp` abstraction lift. -/
theorem msub_equ_head_stack_lift_from_replacements {Γ : Ctx} {s : Stack}
    {head u v : Term}
    (hpvTail : PrevalidExt Γ s)
    (hHeadScoped : Term.Scoped Γ.depth head)
    (hEqReplace :
    ...
    (hAppOpReplace :
      ∀ {op op' arg : Term},
        MSubRed Γ (arg :: []) op op' →
        Term.Scoped Γ.depth arg →
        MSubStar ({ bound := head, kind := .equ } :: Γ)
          (Term.shift 0 arg :: Stack.shift 0 s)
          (Term.shift 0 op) (Term.shift 0 op'))
    (hNilEqReplace :
      ∀ {a b : Term},
    ...
    (hFunNilBodyReplace :
      ∀ {bound body body' : Term},
        MSubRed ({ bound := bound, kind := .sub } :: Γ) [] body body' →
        MSubStar ({ bound := Term.shift 0 bound, kind := .sub } ::
          { bound := head, kind := .equ } :: Γ) []
          (Term.shift 1 body) (Term.shift 1 body'))
    (hFunConsBodyReplace :
      ∀ {α bound body body' : Term} {rest : Stack},
        Term.Scoped Γ.depth α →
        MSubRed ({ bound := bound, kind := .sub } :: Γ) [] body body' →
        MSubStar ({ bound := Term.shift 0 α, kind := .equ } ::
          { bound := head, kind := .equ } :: Γ)
          (Stack.shift 0 (Stack.shift 0 rest)) (Term.shift 1 body) (Term.shift 1 body'))
    (h : MSubRed Γ [] u v) :
    MSubStar ({ bound := head, kind := .equ } :: Γ) (Stack.shift 0 s)
  ...
  | fun_ hBoundScoped hBound hBody =>
    cases s with
    | nil =>
      exact msubStar_abs_fun_body_equ_bound
        (PrevalidExt.nil (Prevalid.equ (PrevalidExt.ctx hpvTail) hHeadScoped))
        (hNilEqReplace hBound)
        (by
          simpa [Ctx.depth] using
        ...
        (hFunNilBodyReplace hBody)
    | cons α rest =>
      exact msubStar_abs_fOp_equ_bound_body hpvNew.tail (hNilEqReplace hBound)
        (Term.shift_scoped 0 Γ.depth _ (Nat.zero_le _) hpvTail.head_scoped)
        (by
          simpa [Ctx.depth] using
            Term.shift_scoped 1 (Γ.depth + 1) _ (by omega) hBody.scoped_left)
        (hFunConsBodyReplace hpvTail.head_scoped hBody)

/-- Chain-level subtype changed-head stack lift with canonical constructor
handlers wired in. -/
theorem msubStar_equ_head_stack_lift_from_replacements {Γ : Ctx} {s : Stack}
    {head u v : Term}
    (hpvTail : PrevalidExt Γ s)
    (hHeadScoped : Term.Scoped Γ.depth head)
    (hEqReplace :
    ...
    MSubStar ({ bound := head, kind := .equ } :: Γ) (Stack.shift 0 s)
      (Term.shift 0 u) (Term.shift 0 v) :=
  msubStar_equ_head_stack_lift_from_step_msub_lift
    (msub_equ_head_stack_lift_from_replacements hpvTail hHeadScoped hEqReplace
      hAppOpReplace hNilEqReplace hFunNilBodyReplace hFunConsBodyReplace)
    h

/-- Function-valued chain-level subtype changed-head stack lift with canonical
constructor handlers wired in for every residual tail stack. -/
theorem msubStar_equ_head_stack_lift_function_from_replacements {Γ : Ctx}
    {head u v : Term}
    (hHeadScoped : Term.Scoped Γ.depth head)
    (hEqReplace :
      ∀ {s : Stack},
    ...
    ∀ {s : Stack},
      ...
      MSubStar ({ bound := head, kind := .equ } :: Γ) (Stack.shift 0 s)
        (Term.shift 0 u) (Term.shift 0 v) :=
  fun hpvTail =>
    msubStar_equ_head_stack_lift_from_replacements hpvTail hHeadScoped
      (hEqReplace hpvTail) (hAppOpReplace hpvTail) hNilEqReplace
      hFunNilBodyReplace hFunConsBodyReplace h
