...
Phase 5d analog of `cofinAvoidsProSelf` (§3.3) for the AvoidsProUniv
architecture. At every `fOp` arm, asserts that for ALL cofinite witnesses
`y_i ∉ L`, the body derivation `hbody y_i _` universally avoids
`y_i` itself (i.e. `AvoidsProUniv (hbody y_i _) y_i`).
