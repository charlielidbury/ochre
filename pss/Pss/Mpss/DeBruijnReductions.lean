...
/-- Split a one-step equivalence replacement across a changed `.equ` entry at
an arbitrary context index, preserving the raw equivalence conclusion. Stable
leaves are rebuilt directly; lookup and recursive constructor cases are
exposed as handlers. This is the `Ctx.replaceAt`-indexed counterpart to the
fixed head/under-head replacement splitters. -/
noncomputable def MEqRed.replaceAt_equ_from_handlers {Γ : Ctx} {s : Stack}
    ...
    (hPro :
      ∀ {i : Nat} {α α' : Term},
        ...
        MEqRed (Ctx.replaceAt cutoff { bound := new, kind := .equ } Γ) s
          (.bvar i) α')
    (hApp :
      ∀ {op op' arg arg' : Term},
...
/-- Split a one-step equivalence replacement across a changed `.equ` entry
under two preserved heads, preserving the raw equivalence conclusion. Stable
leaves are rebuilt directly; lookup and recursive constructor cases are
exposed as handlers. -/
noncomputable def MEqRed.equ_under_two_heads_replace_from_handlers {Γ : Ctx}
    ...
    MEqRed (head₁ :: head₂ :: { bound := new, kind := .equ } :: Γ) s u v := by
  have hpvNew :
...
noncomputable def MEqRed.equ_under_two_sub_heads_replace_from_handlers {Γ : Ctx}
    {s : Stack} {headBound₁ headBound₂ old new u v : Term}
  ...
  MEqRed.equ_under_two_heads_replace_from_handlers hpv hnew
    (by
      intro i α α' hb hα
      cases i with
      | zero =>
...
/-- Empty-stack specialization when a changed `.equ` entry sits under two
preserved `.sub` heads and above a preserved `.sub` tail head. The changed
entry is index `2`; index `3` is the tail `.sub` head and is impossible for
`Me-Pro`, so genuine tail equivalence lookups start at `4+`. -/
noncomputable def MEqRed.equ_under_two_sub_heads_sub_tail_nil_replace_from_split_handlers
    {Γ : Ctx} {headBound₁ headBound₂ tailBound old new u v : Term}
    (hpv : PrevalidExt ({ bound := headBound₁, kind := .sub } ::
      { bound := headBound₂, kind := .sub } ::
      { bound := old, kind := .equ } ::
    ...
    (hProTwo :
      ∀ {α α' : Term},
        Ctx.equBinds ({ bound := headBound₁, kind := .sub } ::
          { bound := headBound₂, kind := .sub } ::
          { bound := old, kind := .equ } ::
          { bound := tailBound, kind := .sub } :: Γ) 2 α →
        MEqRed ({ bound := headBound₁, kind := .sub } ::
          { bound := headBound₂, kind := .sub } ::
          { bound := old, kind := .equ } ::
        ...
        MEqRed ({ bound := headBound₁, kind := .sub } ::
          { bound := headBound₂, kind := .sub } ::
          { bound := new, kind := .equ } ::
          { bound := tailBound, kind := .sub } :: Γ) [] (.bvar 2) α')
    (hProTail :
      ∀ {i : Nat} {α α' : Term},
        Ctx.equBinds ({ bound := headBound₁, kind := .sub } ::
          { bound := headBound₂, kind := .sub } ::
          { bound := old, kind := .equ } ::
          { bound := tailBound, kind := .sub } :: Γ) ((((i + 1) + 1) + 1) + 1) α →
        MEqRed ({ bound := headBound₁, kind := .sub } ::
          { bound := headBound₂, kind := .sub } ::
          { bound := old, kind := .equ } ::
          { bound := tailBound, kind := .sub } :: Γ) [] α α' →
    ...
    MEqRed ({ bound := headBound₁, kind := .sub } ::
      ...
      { bound := new, kind := .equ } ::
      { bound := tailBound, kind := .sub } :: Γ) [] u v :=
  MEqRed.equ_under_two_sub_heads_nil_replace_from_split_handlers
    (Γ := { bound := tailBound, kind := .sub } :: Γ)
    (headBound₁ := headBound₁) (headBound₂ := headBound₂)
    (old := old) (new := new) hpv hnew hProTwo
    (by
      intro i α α' hb hα
      cases i with
      | zero =>
...
/-- Empty-stack specialization when a changed `.equ` entry sits under a
preserved `.sub` head and above a preserved `.sub` tail head. The changed
entry is index `1`; index `2` is the tail `.sub` head and is impossible for
`Me-Pro`, so genuine tail equivalence lookups start at `3+`. -/
noncomputable def MEqRed.equ_under_sub_head_sub_tail_nil_replace_from_split_handlers
    ...
    (hProOne :
      ∀ {target : Term},
        ...
        MEqRed ({ bound := headBound, kind := .sub } ::
          { bound := new, kind := .equ } ::
          { bound := tailBound, kind := .sub } :: Γ) [] (.bvar 1) target)
    (hProTail :
      ∀ {i : Nat} {α α' : Term},
        Ctx.equBinds ({ bound := headBound, kind := .sub } ::
          { bound := old, kind := .equ } ::
          { bound := tailBound, kind := .sub } :: Γ) (((i + 1) + 1) + 1) α →
        MEqRed ({ bound := headBound, kind := .sub } ::
          { bound := old, kind := .equ } ::
          { bound := tailBound, kind := .sub } :: Γ) [] α α' →
        MEqRed ({ bound := headBound, kind := .sub } ::
    ...
    MEqRed ({ bound := headBound, kind := .sub } ::
      { bound := new, kind := .equ } ::
      { bound := tailBound, kind := .sub } :: Γ) [] u v :=
  MEqRed.equ_under_sub_head_nil_replace_from_split_handlers
    (Γ := { bound := tailBound, kind := .sub } :: Γ)
    (headBound := headBound) (old := old) (new := new) hpv hnew hProOne
    (by
      intro i α α' hb hα
      cases i with
      | zero =>
...
/-- Split a one-step subtype replacement across a changed `.equ` entry under
two preserved heads, preserving the raw subtype conclusion. The stable `Top`
leaf is rebuilt directly; lookup and recursive constructor cases are exposed
as handlers. -/
noncomputable def MSubRed.equ_under_two_heads_replace_from_handlers {Γ : Ctx}
    {s : Stack} {head₁ head₂ : CtxEntry} {old new u v : Term}
    (hpv : PrevalidExt (head₁ :: head₂ :: { bound := old, kind := .equ } :: Γ) s)
    (hnew : Term.Scoped Γ.depth new)
    (hPro :
    ...
    (hEq :
      ∀ {u v : Term},
        ...
        MEqRed (head₁ :: head₂ :: { bound := new, kind := .equ } :: Γ) s u v)
    (hApp :
      ∀ {op op' arg : Term},
    ...
    (hFun :
      ∀ {bound bound' body body' : Term},
        ...
        MSubRed ({ bound := bound, kind := .sub } :: head₁ :: head₂ ::
          { bound := old, kind := .equ } :: Γ) [] body body' →
        MSubRed (head₁ :: head₂ :: { bound := new, kind := .equ } :: Γ)
          [] (.abs bound body) (.abs bound' body'))
    (hFOp :
      ∀ {bound arg body body' : Term} {rest : Stack},
        Term.Scoped
    ...
    MSubRed (head₁ :: head₂ :: { bound := new, kind := .equ } :: Γ) s u v := by
  have hpvNew :
      PrevalidExt (head₁ :: head₂ :: { bound := new, kind := .equ } :: Γ) s :=
    PrevalidExt.equ_under_two_heads_replace hpv hnew
  cases h with
...
/-- Split a one-step subtype replacement across a changed `.equ` entry at an
arbitrary context index, preserving the raw subtype conclusion. Stable `Pro`
and `Top` leaves are rebuilt directly; equivalence and recursive constructor
cases are exposed as handlers. -/
noncomputable def MSubRed.replaceAt_equ_from_handlers {Γ : Ctx} {s : Stack}
    {cutoff : Nat} {old new u v : Term}
    (hpv : PrevalidExt (Ctx.replaceAt cutoff { bound := old, kind := .equ } Γ) s)
    (hcut : cutoff < Γ.depth)
    (hnew : CtxEntry.ScopedIn (List.drop (cutoff + 1) Γ)
    ...
    (hFun :
      ∀ {bound bound' body body' : Term},
        ...
        MSubRed ({ bound := bound, kind := .sub } ::
          Ctx.replaceAt cutoff { bound := old, kind := .equ } Γ)
          [] body body' →
        MSubRed (Ctx.replaceAt cutoff { bound := new, kind := .equ } Γ)
          [] (.abs bound body) (.abs bound' body'))
    (hFOp :
      ∀ {bound arg body body' : Term} {rest : Stack},
        Term.Scoped
    ...
    MSubRed (Ctx.replaceAt cutoff { bound := new, kind := .equ } Γ) s u v := by
  have hpvNew :
      PrevalidExt (Ctx.replaceAt cutoff { bound := new, kind := .equ } Γ) s :=
    PrevalidExt.replaceAt_equ_same hpv hcut hnew
  cases h with
  | pro _ hb =>
      exact MSubRed.pro_replaceAt_equ hpv hcut hnew hb
  | top _ hu =>
      exact MSubRed.top hpvNew (by simpa [Ctx.depth_replaceAt] using hu)
  | equ _ heq =>
      exact MSubRed.equ hpvNew (hEq heq)
  | app hOp hArg =>
      exact hApp hOp hArg
...
noncomputable def MSubRed.equ_under_two_sub_heads_nil_replace_from_handlers
    {Γ : Ctx} {headBound₁ headBound₂ old new u v : Term}
    (hpv : PrevalidExt ({ bound := headBound₁, kind := .sub } ::
      { bound := headBound₂, kind := .sub } ::
      { bound := old, kind := .equ } :: Γ) [])
...
/-- Empty-stack two-`.sub` subtype replacement specialized to the case where
the changed `.equ` entry is followed by a preserved `.sub` tail head. The
`Ms-Equ` branch uses the matching index-2/true-tail equivalence splitter. -/
noncomputable def MSubRed.equ_under_two_sub_heads_sub_tail_nil_replace_from_split_handlers
    {Γ : Ctx} {headBound₁ headBound₂ tailBound old new u v : Term}
    (hpv : PrevalidExt ({ bound := headBound₁, kind := .sub } ::
      { bound := headBound₂, kind := .sub } ::
      { bound := old, kind := .equ } ::
    ...
    MSubRed ({ bound := headBound₁, kind := .sub } ::
      ...
      { bound := new, kind := .equ } ::
      { bound := tailBound, kind := .sub } :: Γ) [] u v :=
  MSubRed.equ_under_two_sub_heads_nil_replace_from_split_handlers
    (Γ := { bound := tailBound, kind := .sub } :: Γ)
    (headBound₁ := headBound₁) (headBound₂ := headBound₂)
    (old := old) (new := new) hpv hnew hEqProTwo
    (by
      intro i α α' hb hα
      cases i with
      | zero =>
...
/-- Empty-stack one-`.sub` subtype replacement specialized to the case where
the changed `.equ` entry is followed by a preserved `.sub` tail head. The
`Ms-Equ` branch uses the matching index-1/true-tail equivalence splitter. -/
noncomputable def MSubRed.equ_under_sub_head_sub_tail_nil_replace_from_split_handlers
    {Γ : Ctx} {headBound tailBound old new u v : Term}
    (hpv : PrevalidExt ({ bound := headBound, kind := .sub } ::
      { bound := old, kind := .equ } ::
      { bound := tailBound, kind := .sub } :: Γ) [])
    ...
    MSubRed ({ bound := headBound, kind := .sub } ::
      { bound := new, kind := .equ } ::
      { bound := tailBound, kind := .sub } :: Γ) [] u v :=
  MSubRed.equ_under_sub_head_nil_replace_from_split_handlers
    (Γ := { bound := tailBound, kind := .sub } :: Γ)
    (headBound := headBound) (old := old) (new := new) hpv hnew
    hEqProOne
    (by
      intro i α α' hb hα
      cases i with
