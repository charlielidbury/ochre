...
\begin{Definition}{Environment Helpers}{}
  ...
  $\Omega$ & $\Gamma \vdash \kw{comptime}$ & $\Delta \vdash \kw{concrete}$ & $\Omega \vdash \kw{drop}$ \\
  \hline
...
\begin{Definition}{Subtype}{}
  ...
  \begin{tabular}{c|ccc}
    ...
    \inferrule{
      ...
      \Omega \vdash \kw{comptime} \dashv \Gamma \\\\
      \Gamma \vdash T \maxarrow{\erasedwritearrow} t \dashv \Gamma' \\\\
      % \Gamma' \vdash T \erasedwritearrow t \dashv \Gamma'' \\\\
      \Gamma' \vdash S \subtype S' \erasedreadarrow v
    }{
...
\begin{Definition}{Function Interpretations}{}
  ...
  \begin{tabular}{p{1.2cm}|cc}
    ...
    $\raisebox{-4ex}{$\dfrac{
      \begin{array}{ll}
        \Omega \vdash M \maxarrow{\writearrowabs} m \dashv \Omega' & \ocomment{input type} \\
...
The return type must be a sound approximation of the body if the aforementioned difference in behavior between concrete and abstract interpretation is to be sound. This is done via bounded statement interpretation, $\Omega \vdash S \subtype S_t \movearrowabs u$. If bounded statement interpretation succeeds, then $S$ is a subtype $S_t$, and it will continue to be in all narrowings of $\Omega$. This property is discussed further in Theorem \ref{theorem:statementbounds} and \ref{theorem:statementsubtypingpreservation}.
...
Assignment has to be a statement-level construct because it can narrow the environment. Expressions cannot narrow the environment because that would break Theorem \ref{theorem:expressionreadwidens}. This is why we must assert the assignment has only tightened our upper bound, because that would defeat the point of an upper bound and break Theorem \ref{theorem:statementbounds}.
...
Abstract match statement interpretation first evaluates the scrutinee, then it evaluates all of the branches. The result of interpreting a match statement is the union of the types of each of the branches, which is precise (Theorem \ref{theorem:precisetypeunion}).
...
\chapter{Analysis}
This chapter analyses whether the abstract interpretation, as defined in Chapter \ref{chapter:definition}, is capable of accepting correct programs and rejecting incorrect ones.

Section \ref{section:examplechecks} answers this by using the abstract interpretation manually on interesting programs.

Section \ref{section:properties} answers this by reasoning more generally about the properties held by the abstract interpretations.
...
Define meta-level shortcuts for each of the lines:
...
The task of type-checking is to find a derivation for the following:
...
% pair = ('world, 'hello)
\ocomment{\mono{pair = ('world, 'hello)}} \\
\Omega_0 \vdash \mono{('world, 'hello)} \movearrowabs (\atom{world}, \mono{\_} \rightarrow \atom{hello}) \\
\otab \Omega_0 \vdash \mono{'world} \movearrowabs \atom{world} \\
\otab \Omega_0 \vdash \mono{'hello} \movearrowabs \atom{hello} \\
\Omega_0 \vdash \mono{pair} \writearrowabs (\atom{world}, \mono{\_} \rightarrow \atom{hello}) \dashv \Omega_1 &
\langle\text{Rearrange-Before}\rangle \\
\otab \Omega_0 \rearrangearrow \Omega_0' &
\langle\text{Allocate}\rangle \\
\otab\otab \Omega_0' = \Omega_0,\absmapm{pair}{\top} &
...
\otab\otab \Omega_1 = \Omega_0'\left[\dfrac{\absmapm{pair}{(\atom{world}, \mono{\_} \rightarrow \atom{hello})}}{\absmapm{pair}{\top}}\right]
&
...
% temp = pair.0
\ocomment{\mono{temp = pair.0}} \\
% read right
\Omega_1 \vdash \mono{pair.0} \movearrowabs \atom{world} \dashv \Omega_1'' \\
\otab \Omega_1 \vdash \mono{pair} \movearrowabs (\atom{world}, \mono{\_} \rightarrow \atom{hello}) \dashv \Omega_1' \\
\otab\otab \Omega_1' = \Omega_1\left[\dfrac{\absmapm{pair}{\top}}{\absmapm{pair}{(\atom{world}, \mono{\_} \rightarrow \atom{hello})}}\right] &
\Omega_1' = \emptyset, \absmapm{pair}{\top} \\
\otab \Omega_1' \vdash \oleft{(\atom{world}, \mono{\_} \rightarrow \atom{hello})} = \atom{world} \\
\otab \Omega_1' \vdash \oright{(\atom{world}, \mono{\_} \rightarrow \atom{hello})} = \atom{hello} \\
\otab \Omega_1' \vdash \mono{pair} \writearrowabs (\top, \mono{\_} \rightarrow \atom{hello}) \dashv \Omega_1'' \\
\otab\otab \Omega_1'' = \Omega_1'\left[\dfrac{\absmapm{pair}{(\top, \mono{\_} \rightarrow \atom{hello})}}{\absmapm{pair}{\top}}\right] &
\Omega_1'' = \emptyset,\absmapm{pair}{(\top, \mono{\_} \rightarrow \atom{hello})} \\
% write to left
\Omega_1'' \vdash \mono{temp} \writearrowabs \atom{world} \dashv \Omega_2 &
\langle\text{Rearrange-Before}\rangle \\
\otab \Omega_1'' \rearrangearrow \Omega_1''' &
\langle\text{Allocate}\rangle \\
\otab\otab \Omega_1''' = \Omega_1'', \absmapm{temp}{\top} &
\Omega''' = \emptyset,\absmapm{pair}{(\top, \mono{\_} \rightarrow \atom{hello})}, \absmapm{temp}{\top} \\
\otab \Omega_2 = \Omega_1'''\left[\dfrac{\absmapm{temp}{\atom{world}}}{\absmapm{temp}{\top}}\right] &
...
% pair.0 = pair.1
\ocomment{\mono{pair.0 = pair.1}} \\
% read right
\Omega_2 \dashv \mono{pair.1} \movearrowabs \atom{hello} \dashv \Omega_2' \\
\otab \ocomment{Similar to $\Omega_1 \vdash \mono{pair.0} \movearrowabs \atom{world} \dashv \Omega_1''$} &
% \otab \Omega_2 \vdash \mono{pair} \movearrowabs (\top, \mono{\_} \rightarrow \atom{hello}) \dashv \Omega_2' \\
% \otab\otab \Omega_2' = \Omega_2\left[\dfrac{\absmapm{pair}{\top}}{\absmapm{pair}{(\top, \mono{\_} \rightarrow \atom{hello})}}\right] &
\Omega_2' = \emptyset, \absmapm{pair}{(\top, \mono{\_} \rightarrow \top)}, \absmapm{temp}{\atom{world}} \\
% \otab \Omega_2' \vdash \oleft{(\top, \mono{\_} \rightarrow \atom{hello})} = \top \\
% \otab \Omega_2' \vdash \oright{(\top, \mono{\_} \rightarrow \atom{hello})} = \atom{hello} \\
% \otab \Omega_2' \vdash \mono{pair} \writearrowabs (\top, \mono{\_} \rightarrow \top) \dashv \Omega_2'' \\
% \otab\otab \Omega_2'' = \Omega_2'\left[\dfrac{\absmapm{pair}{(\top, \mono{\_} \rightarrow \top)}}{\absmapm{pair}{\top}}\right] &
%   \Omega_2'' = \emptyset,\absmapm{pair}{(\top, \mono{\_} \rightarrow \top)}, \absmapm{temp}{\atom{world}} \\
% % write to left  
\Omega_2' \dashv \mono{pair.0} \writearrowabs \atom{hello} \dashv \Omega_3 \\
\otab \Omega_2' \vdash \mono{pair} \movearrowabs (\top, \mono{\_} \rightarrow \top) \dashv \Omega_2'' \\
\otab\otab \Omega_2'' = \Omega_2'\left[\dfrac{\absmapm{pair}{\top}}{\absmapm{pair}{(\top, \mono{\_} \rightarrow \top)}}\right] &
\Omega_2'' = \emptyset, \absmapm{pair}{\top}, \absmapm{temp}{\atom{world}} \\
\otab \Omega_2'' \vdash \oleft{(\top, \mono{\_} \rightarrow \top)} = \top \\
\otab \Omega_2'' \vdash \oright{(\top, \mono{\_} \rightarrow \top)} = \top \\
\otab \Omega_2'' \vdash \mono{pair} \writearrowabs (\atom{hello}, \mono{\_} \rightarrow \top) \dashv \Omega_2''' \\
\otab\otab \Omega_3 = \Omega_2''\left[\dfrac{\absmapm{pair}{(\atom{hello}, \mono{\_} \rightarrow \top)}}{\absmapm{pair}{\top}}\right] &
...
% pair.1 = temp
\ocomment{\mono{pair.1 = temp}} \\
\Omega_3 \vdash \mono{temp} \movearrowabs \atom{world} \dashv \Omega_3' \\
\otab \Omega_3' = \Omega_3\left[\dfrac{\absmapm{temp}{\top}}{\absmapm{temp}{\atom{world}}}\right] &
\Omega_3' = \emptyset, \absmapm{pair}{(\atom{hello}, \mono{\_} \rightarrow \top)}, \absmapm{temp}{\top} \\
\Omega_3' \vdash \mono{pair.1} \writearrowabs \atom{world} \dashv \Omega_4 \\
\otab \ocomment{Similar to $\Omega_2' \dashv \mono{pair.0} \writearrowabs \atom{hello} \dashv \Omega_3$} &
\Omega_4 = \emptyset,\absmapm{pair}{(\atom{hello}, \mono{\_} \rightarrow \atom{world})}, \absmapm{temp}{\top} \\
\Omega_4 \vdash L_7 \subtype \mono{*} \movearrowabs (\atom{hello}, \mono{\_} \rightarrow \atom{world}) \\
\cline{0-0}
...
% pair
\ocomment{\mono{pair}} \\
\Omega_4 \vdash \mono{pair} \movearrowabs (\atom{hello}, \mono{\_} \rightarrow \atom{world}) \dashv \Omega_4' \\
\otab \Omega_4' = \Omega_4\left[\dfrac{\absmapm{pair}{\top}}{\absmapm{pair}{(\atom{hello}, \mono{\_} \rightarrow \atom{world})}}\right] &
\Omega_4' = \emptyset, \absmapm{pair}{\top}, \absmapm{temp}{\top} \\
\Omega_4' \vdash \drop{} \\
\otab \emptyset, \absmapm{pair}{\top}, \absmapm{temp}{\top} \vdash \kw{drop} \\
\otab\otab \emptyset, \absmapm{pair}{\top} \vdash \drop{\top} \\
\otab\otab \emptyset, \absmapm{pair}{\top} \vdash \drop{} \\
\otab\otab\otab \emptyset \vdash \drop{\top} \\
\otab\otab\otab \emptyset \vdash \drop{} \\
...
\subsubsection{Accepting Correct Mutation}
...
Helper derivation: function type check

$M = \mono{(p: \&mut Same)}$

% $S_t = \mono{'unit}$

$S = \mono{*p.0 = 'a; *p.1 = 'a; 'unit}$
...
\smaller
\[\begin{array}{ll}
  \mathcal{D_{\text{overwrite}}} = \\
  \\
  \Omega_1 \vdash \mono{$M$ -> 'unit \{$S$\}} \movearrowabs M \rightarrow \mono{'unit} \\
  \cline{0-0}

  \Omega_1 \vdash \mono{(p:\&mut Same)} \maxarrow{\writearrowabs} \borrowm{l}{p_0} \dashv \Omega_{10} \\
  \otab \Omega_1 \vdash \mono{\&mut Same} \erasedreadarrow \borrowm{l}{p_0} \dashv \Omega_1' \\
  \otab\otab \Omega_1 \vdash \mono{Same} \erasedreadarrow p_0 \\
  \otab\otab\otab \absmapm{Same}{p_0} \in \Omega_1 &
    p_0 = (\{\atom{a}, \atom{b}\}, L \rightarrow L) \\
  \otab\otab \Omega_1' = \Omega_1, \absmap{l}{p_0} &
    \Omega_1' = \Omega_1, \absmap{l}{p_0} \\
  \otab \Omega_1' \vdash \mono{p} \writearrowabs \borrowm{l}{p_0} \dashv \Omega_{10} &
    \langle\text{Rearrange-Before}\rangle \\
  \otab\otab \Omega_1' \rearrangearrow \Omega_1'' &
    \langle\text{Allocate}\rangle \\
  \otab\otab\otab \Omega_1'' = \Omega_1', \absmapm{p}{\top} &
    \Omega_1'' = \Omega_1', \absmapm{p}{\top} \\
  \otab\otab \Omega_1'' \vdash \mono{p} \writearrowabs \borrowm{l}{p_0} \dashv \Omega_{10} \\
  \otab\otab\otab \Omega_{10} = \Omega_1'\left[\dfrac{\absmapm{p}{\borrowm{l}{p_0}}}{\absmap{p}{\top}}\right] &
    \Omega_{10} = \Omega_1', \absmapm{p}{\borrowm{l}{p_0}} \\
  
  \Omega_{10} \vdash \mono{*p.0 = 'a; *p.1 = 'a; 'unit} \subtype \mono{'unit} \movearrowabs \atom{unit} \\
  \cline{0-0}

  \Omega_{10} \vdash \mono{'a} \movearrowabs \atom{a} \\
  \Omega_{10} \vdash \mono{*p.0} \writearrowabs \atom{a} \dashv \Omega_{11} &
    \langle\text{Rearrange-Before}\rangle \\
  \otab \Omega_{10} \rearrangearrow \Omega_{10}' \, \ocomment{must set $*p.0$ to $\top$ to write} &
    \langle\text{Type-Widen}\rangle \\
  \otab\otab \Omega_{10}' = \Omega_{10}\left[\dfrac{\absmapm{p}{\borrowm{l}{p_1}}}{\absmapm{p}{\borrowm{l}{p_0}}}\right] &
    \Omega_{10}' = \Omega_1', \absmap{p}{\borrowm{l}{p_1}} \\
  \otab\otab \Omega_{10} \vdash \borrowm{l}{p_0} \subtype \borrowm{l}{p_1}  &
    p_1 = (\top, \mono{\_} \rightarrow \{\atom{a}, \atom{b}\}) \\
  \otab\otab\otab \Omega_{10} \vdash p_0 \subtype p_1 \\
  \otab\otab\otab\otab \Omega_{10} \vdash \{\atom{a}, \atom{b}\} \subtype \top \\
  \otab\otab\otab\otab \Omega_{10} \vdash \comptime{} \dashv \Gamma_1 &
    \Gamma_1 = \Omega_1,\absmapm{L}{\top} \\
  \otab\otab\otab\otab \Gamma_1 \vdash \mono{\_} \erasedwritearrow \top \\
  \otab\otab\otab\otab \Gamma_1 \vdash \mono{L} \erasedwritearrow \{\atom{a}, \atom{b}\} \dashv \Gamma_1' \\
  \otab\otab\otab\otab\otab \Gamma_1' = \Gamma_1\left[\dfrac{\absmapm{L}{\{\atom{a}, \atom{b}\}}}{\absmapm{L}{\top}}\right] \\
  \otab\otab\otab\otab \Gamma_1' \vdash \mono{L} \subtype \{\atom{a}, \atom{b}\} \erasedreadarrow \{\atom{a}, \atom{b}\} \\
  \otab\otab\otab\otab\otab \Gamma_1' \vdash L \erasedreadarrow \{\atom{a}, \atom{b}\} \\
  \otab\otab\otab\otab\otab \Gamma_1' \vdash \{\atom{a}, \atom{b}\} \erasedreadarrow \{\atom{a}, \atom{b}\} \\
  \otab\otab\otab\otab\otab \Gamma_1' \vdash \{\atom{a}, \atom{b}\} \subtype \{\atom{a}, \atom{b}\} \\
  \otab\otab\otab\otab\otab\otab \{\atom{a}, \atom{b}\} \subseteq \{\atom{a}, \atom{b}\} \\
  \otab \Omega_{10}' \vdash \mono{*p.0} \writearrowabs \atom{a} \dashv \Omega_{11} \, \ocomment{write to \mono{*p.0}} \\
  \otab\otab \Omega_{10}' \vdash \mono{*p} \movearrowabs p_1 \dashv \Omega_{10}''' \\
  \otab\otab\otab \Omega_{10}' \vdash \mono{p} \movearrowabs \borrowm{l}{p_1} \vdash \Omega_{10}'' \\
  \otab\otab\otab\otab \Omega_{10}'' = \Omega_{10}'\left[\dfrac{\absmapm{p}{\top}}{\absmapm{p}{p_1}}\right] &
    \Omega_{10}'' = \Omega_1',\absmapm{p}{\top} \\
  \otab\otab\otab \Omega_{10}'' \vdash \mono{p} \writearrowabs \borrowm{l}{\top} \vdash \Omega_{10}''' \\
  \otab\otab\otab\otab \Omega_{10}''' = \Omega_{10}''\left[\dfrac{\absmapm{p}{\borrowm{l}{\top}}}{\absmapm{p}{\top}}\right] &
    \Omega_{10}''' = \Omega_1',\absmapm{p}{\borrowm{l}{\top}} \\
  \otab\otab \Omega_{10}''' \vdash \oleft{p_1} = \top \\
  \otab\otab \Omega_{10}''' \vdash \oright{p_1} = \{\atom{a}, \atom{b}\} \\
  \otab\otab \Omega_{10}''' \vdash \mono{*p} \writearrowabs p_2 \dashv \Omega_{11} &
    p_2 = (\atom{a}, \mono{\_} \rightarrow \{\atom{a}, \atom{b}\}) \\
  \otab\otab\otab \Omega_{10}''' \vdash \mono{p} \movearrowabs \borrowm{l}{\top} \dashv \Omega_{10}'''' \\
  \otab\otab\otab\otab \Omega_{10}'''' = \Omega_{10}'''\left[\dfrac{\absmapm{p}{\top}}{\absmap{p}{\borrowm{l}{\top}}}\right] &
    \Omega_{10}'''' = \Omega_1',\absmapm{p}{\top} \\
  \otab\otab\otab \Omega_{10}'''' \vdash \mono{p} \writearrowabs \borrowm{l}{p_2} \dashv \Omega_{11} \\
  \otab\otab\otab\otab \Omega_{11} = \Omega_{10}''''\left[\dfrac{\absmapm{p}{\borrowm{l}{p_2}}}{\absmap{p}{\top}}\right] &
...
\[\begin{array}{ll}
  \Omega_{11} \vdash \mono{*p.1 = 'a; 'unit} \subtype \mono{'unit} \movearrowabs \atom{unit} \\
  \cline{0-0}
  \Omega_{11} \vdash \mono{'a} \movearrowabs \atom{a} \\
  \Omega_{11} \vdash \mono{*p.1} \writearrowabs \atom{a} \dashv \Omega_{12} &
    \Omega_{12} = \Omega_1',\absmapm{p}{\borrowm{l}{p_3}} \\
  \otab \ocomment{Similar to $\Omega_{10}' \vdash \mono{*p.0} \writearrowabs \atom{a} \dashv \Omega_{11}$} &
    p_3 = (\atom{a}, \mono{\_} \rightarrow \atom{a}) \\
  \Omega_{12} \vdash \mono{'unit} \subtype \mono{'unit} \movearrowabs \atom{unit} \\
  \cline{0-0}

  \Omega_{12} \vdash \mono{'unit} \movearrowabs \atom{unit} \\
  \ocomment{$\Omega_{12} \vdash \drop{} =$} \\
  \Omega_1,\absmap{l}{p_0},\absmap{p}{\borrowm{l}{p_3}} \vdash \drop{} &
    \ocomment{final cleanup} \\
  \otab \Omega_1,\absmap{l}{p_0} \vdash \drop{(\borrowm{l}{p_3})} \dashv \Omega_1 \\
  \otab\otab \Omega_1 = \Omega_1,\absmap{l}{p_0} \backslash \{ \absmap{l}{p_0} \} \\
  \otab\otab \Omega_1 \vdash p_3 \subtype p_0 \\
  \otab\otab\otab \Omega_1' \vdash \atom{a} \subtype \{\atom{a}, \atom{b}\} \\
  \otab\otab\otab \Omega_1' \vdash \comptime{} \dashv \Gamma_2 &
    \Gamma_2 = \Omega_1,\absmapm{L}{\top} \\
  \otab\otab\otab \Gamma_2 \vdash \mono{\_} \erasedwritearrow \top \\
  \otab\otab\otab \Gamma_2 \vdash \mono{L} \erasedwritearrow \atom{a} \dashv \Gamma_2' \\
  \otab\otab\otab\otab \Gamma_2' = \Gamma_2\left[\dfrac{\absmapm{L}{\atom{a}}}{\absmapm{L}{\top}}\right] \\
  \otab\otab\otab \Gamma_2' \vdash \mono{L} \subtype \atom{a} \erasedreadarrow \atom{a} \\
  \otab\otab\otab\otab \Gamma_2' \vdash L \erasedreadarrow \atom{a} \\
  \otab\otab\otab\otab \Gamma_2' \vdash \atom{a} \erasedreadarrow \atom{a} \\
  \otab\otab\otab\otab \Gamma_2' \vdash \atom{a} \subtype \atom{a} \\
  \otab\otab\otab\otab\otab \atom{a} \subseteq \atom{a} \\
  \Omega_{12} \vdash \mono{'unit} \subtype \mono{*} \erasedreadarrow \atom{unit} \\
  
  \Omega_{12} \vdash \atom{unit} \subtype \atom{unit}
\end{array}\]
\end{adjustwidth}


\subsubsection{Rejecting Incorrect Mutation}
The function body may temporarily change the type of any references to any type, but by the end of the function body, they must all be of the correct type. The below code mutates the pair it is given reference to incorrectly: while $\atom{b}$ is a valid value for the right-hand side of the pair, it can only be $\atom{b}$ when the left is also $\atom{b}$, which in this case, it is not.
...
The derivation for this incorrect program is almost identical to the derivation for its correct counterpart, apart from $p_3 = (\atom{a}, \mono{\_} \rightarrow \atom{b})$ instead of $p_3 = (\atom{a}, \mono{\_} \rightarrow \atom{a})$. This causes the final cleanup to fail like so:
...
\begin{Definition}{Colon Operator}{}
  \inferrule{
    \concrete{v} \\\\
    \Omega \vdash v \subtype t
  }{
...
In theorems, unless otherwise stated, free variables are universally quantified.
...
\begin{Definition}{Environment Subtyping}{}
  \begin{mathpar}
  \inferrule{
    \ocomment{todo}
  }{
...
\begin{Theorem}{Expression read-interpretation is sound}{} 
  for all $\diamond$ in $\{ \readarrow, \movearrow \}$:
  \[\oimplies{
      \inferrule{
...
\begin{Theorem}{Expression write-interpretation is sound}{}
  \[\oimplies{
    \inferrule{
...
\begin{Theorem}{Statement interpretation is monotonic}{}
  ...
  \[\oimplies{
    \inferrule{
    \Omega \vdash S \subtype S' \diamond v \\\\
    \Omega' \subtype \Omega
  }{}
  }{
    \text{there exists $v'$ s.t.   }
    \inferrule{
      \Omega' \vdash S \subtype S' \diamond v' \\\\
      \Omega' \vdash v' \subtype v
    }{}
...
\begin{Theorem}{Expression read-interpretation is monotonic}{}
...
\begin{Theorem}{Expression write-interpretation is monotonic}{}
...
\begin{Theorem}{Statement bounds are respected}{}
...
\begin{Definition}{Expression Subtyping}{}
  \[\inferrule{
...
\begin{Theorem}{Expression subtyping is preserved across environment subtyping}{}
  \[\oimplies{
    \inferrule{
      \Omega \vdash M \subtype N \\\\
      \Omega' \subtype \Omega
...
\begin{Theorem}{Expression-Like Statement subtyping \textbf{is not} preserved across environment subtyping}{} \[
  \inferrule{
...
The fact this does not hold is the motivation behind bounded statement interpretation because, with bounded statement interpretation, we can define statement subtyping and an equivalent subtyping preservation theorem. This is the primary reason why statements and expressions are separated.

\begin{Definition}{Statement Subtyping}{}
  \[\inferrule{
...
\begin{Theorem}{Statement subtyping \textbf{is} preserved across environment subtyping}{} \[
  \inferrule{
    \Omega \vdash S_a \subtype S_b \\\\
    \Omega' \subtype \Omega
  }{}
...
\subsubsection{Non-Soundness}
\begin{Theorem}{Expressions Read-Interpretation Widens Environment}{}
...
\begin{Theorem}{Expressions Write-Interpretation Narrows Environment}{}
...
\begin{Theorem}{Type Union is Precise}{}
  \[
...
\begin{Theorem}{Environment Union is \textbf{not} Precise}{}
  \ocomment{todo}
\end{Theorem}
...
\textbf{Inprecise Environment Union} - Environment type union cannot be precise because dependencies between variables are not supported (see Theorem \ref{theorem:inpreciseenvunion} and surrounding discussion). Initially, dependencies between variables were avoided to simplify, but it turns out that not having a precise environment union introduces the requirement for statements, which complicates things much further. I hope in the future I can change this and simplify the presented system.
