...
\begin{figure}
  ...
  \begin{mathpar}
    \inferrule[Var-Move]{
      \Omega' = \Omega[\text{\texttt{x}} \mapsto \bot/\text{\texttt{x}} \mapsto m]
    }{
      \Omega \vdash \text{\texttt{x}}\,\dot{\Rightarrow}\,m \dashv \Omega'
    }
    
    \inferrule[Pair-Left-Move]{
    ...
    \inferrule[Pair-Right-Move]{
      \Omega \vdash \mono{M} \movearrow (m, n, \mono{T} \rightarrow \mono{U}) \dashv \Omega'\\\textit{get pair}\\\\
      \Omega' \vdash \mono{T} \erasedwritearrow m \dashv \Omega''\\\textit{calculate right restriction}\\\\
      \Omega' \vdash \mono{U} \erasedreadarrow u \dashv \Omega''\\\textit{calculate right restriction}\\\\
      \Omega'' \vdash \mono{M} \writearrow (m, \bot, \mono{T} \rightarrow \mono{\_}) \dashv \Omega'''\\\textit{return pair}
    }{
      \Omega \vdash \mono{M.1} \movearrow n \cap u \dashv \Omega'''
    }

    \inferrule[Mut-Ref-Move]{
    ...
    \inferrule[Pair-Construction]{
      \Omega \vdash \mono{M} \movearrow m \dashv \Omega'\\\\
      \Omega' \vdash \mono{N} \movearrow n \dashv \Omega''
    }{
      \Omega \vdash \mono{M, N} \movearrow (m, n, \mono{\_} \rightarrow \mono{\_}) \dashv \Omega''
    }
    ...
    \inferrule[Atom-Construction]{
    }{
      \Omega \vdash \mono{'a} \movearrow 'a
    }
    ...
    \inferrule[Mutable-Borrow]{
      \Omega \vdash \mono{M} \movearrow m \dashv \Omega'\\\\
      \Omega' \vdash \mono{M} \writearrow \kw{borrow}^m\,l \dashv \Omega''
    }{
      \Omega \vdash \mono{\&mut M} \movearrow \kw{borrow}^m\,l\,m \dashv \Omega''
    }

    \inferrule[Type-Constraint-Move]{

    }{
      \Omega \vdash \mono{M : T} \movearrow m \dashv \Omega
    }
