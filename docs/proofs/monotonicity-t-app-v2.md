# Monotonicity — T-App Case (v2, with abstract domain erasure)
...
By HN-Mono (lemma-hn-mono-v2.md) applied to `F' ⊑ F` and `F ⇓ (x: A) → B`:
...
- R₀ = (M₁ : E): Ascription sub-gap (same as Erase-Sub). ✓ modulo gap.
...
All steps resolved. The proof is complete modulo the ascription sub-gap
(which appears in the Monotone Covariant Substitution Lemma for
ascription terms in R₀). ∎
