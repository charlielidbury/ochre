...
2. **Self-elim** (σ = mu, τ ≠ mu, 4 cases EXPANDED by agent ochre-20260405-181325):
   Each case now has 10 VCompat sub-cases with localized sorrys.
   **PROVED (any seen):** refl, structural mu (Strategy A: mu-left +
   absEval_preserves + ih_fuel at m+1 with original v). Impossible cases proved.
   **PROVED (seen=[]):** mu-left, inferType, asc-left (Strategy B: ih_n).
   **For VCompat.adequacy (main caller, seen=[]):** 8/10 sub-cases proved.
   **REMAINING:** mu-right/normalized mu-right (step-loss), seen≠[] callback
   mismatch, annotation path failures. The step-loss sorrys are blocked by
   the same fundamental issue as the app lam-lam case.
