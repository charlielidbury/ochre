//@ [!lean] skip
//@ [lean] subdir=HashmapSplice
//@ [lean] aeneas-args=-split-files -loops-to-rec

//! A hashmap implementation.
//!
//! ## This file is the SPLICE variant
//!
//! Byte-for-byte `../current/hashmap.rs` except for the body of
//! [`HashMap::move_elements_from_list`] (and this header). `diff -u
//! ../current/hashmap.rs hashmap.rs` is the whole change.
//!
//! Upstream's resize moves entries by *re-inserting* them: it destructures each
//! source node by value and calls `insert_no_resize`, which ends at
//! `*ls = AList::Cons(key, value, Box::new(AList::Nil))`. That is one fresh
//! heap allocation per moved entry, and one old box dropped per moved entry —
//! so a resize that moves N entries performs N allocations on top of the new
//! slots vector.
//!
//! The splice moves entries by *relinking* them: each source node keeps its own
//! tail box, and that box is reused as the link to whatever the destination
//! bucket previously held. A resize's only allocation is then the new slots
//! vector built by `new_with_capacity` — zero allocations while moving. See
//! `../SPLICE-NOTES.md` for the measured counts.
//!
//! Current limitations:
...
impl<T> HashMap<T> {
    ...
    /// Auxiliary function.
    ///
    /// Moves the nodes of `ls` into `ntable` by relinking them, allocating
    /// nothing. Each node keeps the `Box` it already owns; that box is reused
    /// as the node's new tail, holding whatever the destination bucket used to
    /// hold. Compare upstream, which destructured the node by value and called
    /// `insert_no_resize`, whose `AList::Nil` arm allocates a fresh
    /// `Box::new(AList::Nil)` for every entry moved.
    ///
    /// Two things this relies on, both consequences of resizing a *map*:
    ///
    /// - No duplicate-key handling is needed. `insert_no_resize` walks the
    ///   destination bucket so that an existing key has its value overwritten
    ///   rather than a second node appended. During a move every key in the
    ///   source table is distinct and the destination table starts empty and is
    ///   only ever fed source entries, so that walk always runs to `Nil` and
    ///   `insert_in_list` always reports `true`. Prepending unconditionally is
    ///   therefore the same map. (It is also what drops the walk: moving an
    ///   entry is now O(1) rather than O(destination bucket length).)
    /// - Bucket order changes. `insert_in_list` appends at the tail; this
    ///   prepends at the head, so entries that shared a slot before the resize
    ///   and still share one after it come out in reverse relative order. Keys
    ///   are distinct and every lookup scans the whole bucket, so `get`,
    ///   `get_mut`, `contains_key`, `remove` and `len` are unaffected. The
    ///   DLLBC hashmap this is being compared against makes the same trade.
    ///
    /// `num_entries` is incremented here exactly as `insert_no_resize` did.
    /// Note it is dead bookkeeping either way: `try_resize` moves only `slots`
    /// and `max_load` out of `ntable`, never `num_entries` — the count in the
    /// live table is already correct because a resize does not change it.
        ...
        loop {
            match ls {
                ...
                AList::Cons(k, v, mut tl) => {
                    // Find the destination bucket, as `insert_no_resize` would
                    let hash = hash_key(&k);
                    let hash_mod = hash % ntable.slots.len();
                    let dst = &mut ntable.slots[hash_mod];
                    // Rotate: take the destination's head out, park it in the
                    // node's own tail box (which hands back the rest of the
                    // source list), then make the node the destination's head.
                    // Three enum-sized writes, no allocation, no drop.
