...
theorem R_mono {n m d v e} (hle : m ≤ n) (h : R n d v e) :
  ...
  | _+1, _+1, .lam dV cl, hle, h =>
      ...
      exact ⟨ρe', dome, R_mono hle hRd, RList_mono hle hRL, hclb,
             hlvl_cl_env, hfq_cl_env, heqL⟩
  | _+1, _+1, .iota aV cl, hle, h =>
      unfold R at h ⊢
      obtain ⟨ρe', anne, hRa, hRL, hclb, hlvl_cl_env, hfq_cl_env, heqI⟩ := h
      exact ⟨ρe', anne, R_mono hle hRa, RList_mono hle hRL, hclb,
             hlvl_cl_env, hfq_cl_env, heqI⟩
  | _+1, _+1, .«fix» aV cl, hle, h =>
      unfold R at h ⊢
      obtain ⟨ρe', anne, hRa, hRL, hclb, hlvl_cl_env, hfq_cl_env, heqF⟩ := h
      exact ⟨ρe', anne, R_mono hle hRa, RList_mono hle hRL, hclb,
             hlvl_cl_env, hfq_cl_env, heqF⟩
...
theorem R_resp_Equiv {n d v e e'}
  ...
  | _+1, h =>
    ...
    | lam dV cl =>
        obtain ⟨ρe', dome, hRd, hRL, hclb, hlvl_cl_env, hfq_cl_env, heqL⟩ := h
        exact ⟨ρe', dome, hRd, hRL, hclb, hlvl_cl_env, hfq_cl_env,
          Equiv_c.trans (Equiv_c.of_Equiv (Equiv.symm heq)) heqL⟩
    | iota aV cl =>
        obtain ⟨ρe', anne, hRa, hRL, hclb, hlvl_cl_env, hfq_cl_env, heqI⟩ := h
        exact ⟨ρe', anne, hRa, hRL, hclb, hlvl_cl_env, hfq_cl_env,
...
theorem R_depth_lift {n d v e}
  ...
  | k+1, .lam dV cl, hlvl, hfq, hq, h =>
      ...
      have htail_eq : (.bvar 0 :: ρe'.map (·.shift 1 0)).map (·.shift 1 1)
        ...
        simpa using this
      have hlvl_cl_d1 : Closure.envLevelsBelow (d+1) cl.env :=
        Closure.envLevelsBelow_mono (Nat.le_succ d) _ hlvl_cl
      have hfq_cl_d1 : Closure.envFullyQuotable (d+1) cl.env :=
        Closure.envFullyQuotable_mono (Nat.le_succ d) _ hfq_cl
      unfold R
      refine ⟨ρe'.map (·.shift 1 0), dome.shift 1 0,
              hRd', hRL', hclb', hlvl_cl_d1, hfq_cl_d1, ?_⟩
      have target_rewrite :
          (Expr.lam dome
            (cl.body.substEnv (.bvar 0 :: ρe'.map (·.shift 1 0)))).shift 1 0
  ...
  | k+1, .iota aV cl, hlvl, hfq, hq, h =>
      ...
      refine ⟨ρe'.map (·.shift 1 0), anne.shift 1 0,
              hRa', hRL', hclb', hlvl_cl_d1, hfq_cl_d1, ?_⟩
      have target_rewrite :
          (Expr.iota anne
            (cl.body.substEnv (.bvar 0 :: ρe'.map (·.shift 1 0)))).shift 1 0
  ...
  | succ m =>
      ...
      | lam dV cl =>
          ...
          have hEb : Equiv_c (d+1) body'
              (cl.body.substEnv (.bvar 0 :: ρe'.map (·.shift 1 0))) :=
            quoteClosure_realises hbody'_ω hRL hclb hlvl_cl_env hfq_cl_env
    ...
    | fuel+1, hvapp =>
      ...
      | .iota annV ⟨lbody, lenv⟩, hvapp =>
          ...
          · -- Unfold branch.
            ...
            unfold R at hRf'
            obtain ⟨ρe', anne, _hRa_head, hRL, hclb, _hlvl_cl_env, _hfq_cl_env, heqI⟩ := hRf'
            have henv' := REnv_of_RList hRL
      ...
      | .«fix» annV ⟨lbody, lenv⟩, hvapp =>
          ...
          · simp only [hcond, Bool.false_eq_true, ↓reduceIte,
            ...
            have hRf'' := hRf'
            unfold R at hRf'
            obtain ⟨ρe', anne, _hRa_head, hRL, hclb, _hlvl_cl_env, _hfq_cl_env, heqF⟩ := hRf'
            have henv' := REnv_of_RList hRL
            have henv_ext := REnv_cons henv' hRf''
            have hclb' : lbody.closedAt (fe :: ρe').length = true := by
              simpa using hclb
...
theorem tyInfer_sound_open
  ...
  | fuel + 1, hfuel, h =>
    ...
    · -- `tyInfer` checks `a` against `domV`, then recurses
      ...
      -- `tyCheck_sound_open`.
      --
      -- Category B attempt (2026-04-23, revisited
      -- 2026-04-22): the IH from `push_let` delivers
      -- Subtype' at context `(domTye :: Γe)` with
      -- `ρe' = (a.substEnv ρe).shift 1 0 :: ρe.map shift`
      -- and quote at depth `Γ.size+1`. Bridging back to the
      -- outer `Γe` / `Γ.size` needs the UNSHIFT lemma at
      -- cutoff 0: `Subtype' (X :: Γ) (a.shift 1 0)
      -- (b.shift 1 0) → Subtype' Γ a b` (under endpoint
      -- closedness). See `Subtype'.unshift_head`
      -- documentation in `Subtyping.lean` (~line 1045) for
      -- the proof plan — roughly a 300–500 LOC substitution
      -- lemma over every `Subtype'` constructor, with
      -- seen-set depth rewiring for productive rules and
      -- `.hyp`. Out of scope for a single session; left
      -- sorried.
      sorry
    ...
    · -- `tyInfer` floats the let out and recurses on
      ...
      --
      -- Same UNSHIFT obstruction as the β fast-path above.
      -- `letE_L + app_cong` assembles the Subtype' skeleton,
      -- but cannot bridge the depth / context mismatch
      -- without `Subtype'.unshift_head` (see Subtyping.lean
      -- docstring for proof plan). Left sorried.
      sorry
    -- .app f a — generic.
    ...
    · rename_i _e' val body _h'
      ...
      --
      -- Category B attempt (2026-04-23, revisited 2026-04-22):
      -- two directions probed:
      ...
      --       still needs the UNSHIFT lemma
      --       `Subtype'.unshift_head` (see Subtyping.lean
      --       docstring for the 300–500 LOC proof plan).
      ...
      -- inversion / OpenCtx indexing) outside a single-session
