...
-- NOTE: EnvClosed, envClosed_extend, absEval_freeVars_covered (which was FALSE
-- for the app-lam case — domain annotations leak stale variable references) have
-- been removed. See git history (session ochre-lean-20260401-160031) for details.
-- The correct replacement is absEval_evalFreeVars_general (below), which uses
-- evalFreeVars (excluding domains) and an arbitrary predicate P.
