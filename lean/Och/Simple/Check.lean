  ...
  | fuel + 1 =>
    ...
    | .mu A body =>
      ...
      | _ =>
        ...
        synth fuel Γ (body.subst 0 (.mu A body))
    | .top => some .top
...
def check (fuel : Nat) (Γ : Ctx) (a b : Expr) : Option Bool :=
  match fuel with
  | 0 => none
  | fuel + 1 =>
    -- Fast paths
    ...
    | .var x, _ =>
      match Γ.get? x with
      | some T => check fuel Γ T b
      | none => some false
    -- [Lam]: both sides are lambdas
    | .lam A body₁, .lam B body₂ =>
      match check fuel Γ B A with
      | some true =>
        check fuel (B :: Γ) body₁ body₂
      | other => other
    -- [App]: LHS is an application
    | .app f arg, _ =>
      -- Helper: try [App] with specific D, R witnesses
      let tryWithDR (D R : Expr) : Option Bool :=
        match check fuel Γ f (.lam D R) with
        | some true =>
          match check fuel Γ arg D with
          | some true => check fuel Γ (R.subst 0 arg) b
          | other => other
        | other => other
      ...
      let tryUnfolded : Option Bool :=
        match f with
        | .mu _A bodyF =>
          match synth fuel Γ (bodyF.subst 0 f) with
    ...
    | .asc e τ, _ =>
      match check fuel Γ e τ with
      | some true => check fuel Γ τ b
      | other => other
    -- [Mu on LHS]: try annotation first, then muR (if RHS is mu), then unfold
    | .mu A bodyA, _ =>
      -- Strategy 1: [Mu] use annotation: A ⊑ b
      ...
      match b with
      | .mu Ab bodyB =>
        match check fuel Γ (.mu A bodyA) Ab with
        | some true =>
          match check fuel Γ (.mu A bodyA) (bodyB.subst 0 (.mu A bodyA)) with
          | some true => some true
          | _ =>
            -- muR failed, try muUnfoldL
            check fuel Γ (bodyA.subst 0 (.mu A bodyA)) b
        | _ =>
    ...
    | _, .asc e τ =>
      ...
      match check fuel Γ e τ with
      | some true => check fuel Γ a e
      | other => other
    | _, .mu Ab bodyB =>
      -- [Mu-R]: self-type intro on RHS
      match check fuel Γ a Ab with
      | some true =>
        check fuel Γ a (bodyB.subst 0 a)
      | other => other
    | _, _ => some false
