...
-- ============================================================
-- Category A: Self-type intro (needs Mu-R + mu-unfold-L)
-- ============================================================
-- RESEARCH FINDING: Sub.mu (LHS only) CANNOT handle self-type intro.
--
-- The fundamental problem: when target is mu(A).body (e.g. dBool),
-- no existing rule can unfold the mu on the RHS. Specifically:
-- - Sub.mu requires LHS to be mu, handles mu on LEFT only
-- - Sub.refl requires exact equality (dtrue != dBool)
-- - Sub.lam requires both sides to be lambdas (dBool is mu)
-- - Sub.top requires RHS to be ⊤
--
-- TWO NEW RULES are needed:
--   Mu-R:        a ⊑ mu(A).b  if  a ⊑ b[0 := a]
--   mu-unfold-L: mu(A).b ⊑ c  if  b[0 := mu(A).b] ⊑ c
--
-- Full derivation traces verified in DBool.lean and DNat.lean.
-- All Category A derivations have been shown to WORK with these rules.

-- A1: dtrue ⊑ dBool (full derivation in DBool.lean)
-- Flip condition: Add Mu-R + mu-unfold-L to Subtype.lean
-- example : Sub [] dtrue dBool := ...

-- A2: dfalse ⊑ dBool (full derivation in DBool.lean)
-- Flip condition: Same
-- example : Sub [] dfalse dBool := ...

-- A3: dzero ⊑ dNat (full derivation in DNat.lean)
-- Flip condition: Same
-- example : Sub [] dzero dNat := ...

-- A4: done_ ⊑ dNat (dsucc dzero ⊑ dNat)
-- Needs Mu-R + mu-unfold-L + Sub.app for dsucc application
...
-- A6: dnot : dBool → dBool
-- Needs A1 + A2 for the body to typecheck
-- Flip condition: Mu-R + mu-unfold-L
...
-- A8: disZero : dNat → BOOL
-- Needs A3 + Mu-R + mu-unfold-L
-- Flip condition: Same
