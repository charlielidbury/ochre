...
example : Och.checkSubtype 200 pair_1_2 (och{ Sigma Nat_ constNat }) = .ok true := by native_decide

-- dpair : its full type
example : Och.checkSubtype 200 dpair (och{
...
example : Och.checkSubtype 100 (och{ Sigma Nat_ constNat }) Nat_ = .ok false := by native_decide

-- dpair is not itself a Sigma type (it's a constructor, not a value of Sigma)
example : Och.checkSubtype 100 dpair (och{ Sigma Nat_ constNat }) = .ok false := by native_decide
