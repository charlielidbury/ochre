...
example : SubstEval.subCheckT 1000 p12 (och{ Pair Nat_ Nat_ }) = .ok true := by
  native_decide

-- pair_ Bool Bool true false : Pair Bool Bool
example : SubstEval.subCheckT 100
...
example : SubstEval.subCheckT 100 fst_ (och{ Pair Nat_ Nat_ → Type })
  = .ok true := by native_decide

example : SubstEval.subCheckT 100 snd_ (och{ Pair Nat_ Nat_ → Type })
  = .ok true := by native_decide
...
example : SubstEval.subCheckT 100 fst_ Nat_ = .ok false := by native_decide

example : SubstEval.subCheckT 1000 p12 Bool = .ok false := by native_decide
