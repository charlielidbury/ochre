...
example : SubstEval.subCheckT 50 unit_ Unit_ = .ok true := by native_decide
example : SubstEval.subCheckT 50 Unit_ .type = .ok true := by native_decide

-- subtype checking (negative)
-- Unit_ is wider than unit_, so Unit_ ⊑ unit_ should fail
example : SubstEval.subCheckT 50 Unit_ unit_ = .ok false := by native_decide
-- Type is not a subtype of Unit_ (Type is wider)
example : SubstEval.subCheckT 50 .type Unit_ = .ok false := by native_decide
