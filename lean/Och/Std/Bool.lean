...
example : subCheck 50 false_ Bool = true := by native_decide
example : subCheck 50 not' (och{ Bool → Bool }) = true := by native_decide
example : subCheck 50 and' (och{ Bool → Bool → Bool }) = true := by native_decide
example : subCheck 50 or (och{ Bool → Bool → Bool }) = true := by native_decide

-- subtype checking (negative)
-- Bool is not a subtype of true or false (it's wider)
example : subCheck 50 Bool true_ = false := by native_decide
example : subCheck 50 Bool false_ = false := by native_decide
-- not : Bool → Bool is not a Bool
example : subCheck 50 not' Bool = false := by native_decide
-- and/or need two args, not one
example : subCheck 50 and' (och{ Bool → Bool }) = false := by native_decide
example : subCheck 50 or (och{ Bool → Bool }) = false := by native_decide
