...
example : Och.checkSubtype 50 true_ Bool = .ok true := by native_decide
example : Och.checkSubtype 50 false_ Bool = .ok true := by native_decide
example : Och.checkSubtype 50 not' (och{ Bool → Bool }) = .ok true := by native_decide
example : Och.checkSubtype 50 and' (och{ Bool → Bool → Bool }) = .ok true := by native_decide
example : Och.checkSubtype 50 or (och{ Bool → Bool → Bool }) = .ok true := by native_decide

-- subtype checking (negative)
-- Bool is not a subtype of true or false (it's wider)
example : Och.checkSubtype 50 Bool true_ = .ok false := by native_decide
example : Och.checkSubtype 50 Bool false_ = .ok false := by native_decide
-- not : Bool → Bool is not a Bool
example : Och.checkSubtype 50 not' Bool = .ok false := by native_decide
-- and/or need two args, not one
example : Och.checkSubtype 50 and' (och{ Bool → Bool }) = .ok false := by native_decide
example : Och.checkSubtype 50 or (och{ Bool → Bool }) = .ok false := by native_decide
